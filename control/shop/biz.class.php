...
class biz{
	...
	function __construct($DB,$usersid){
		...
		$this->table = "biz";
		$this->skin_table = "biz_skin";
		$this->home_table = "biz_home";
		$this->cat_table = "biz_category";
	...
	public function get_apply_list($condition){
		$lists = array();
		$this->db->getPage("biz_apply","*",$condition,20);
		while($r = $this->db->fetch_assoc()){
			$lists[] = $r;
		}
		return $lists;
	...
	public function delete_apply($itemid){
		$flag=$this->db->Del("biz_apply","Users_ID='".$this->usersid."' and `ItemID`=".$itemid);
		return $flag;
	}
	
	public function edit_apply($data,$itemid){
		$flag=$this->db->Set("biz_apply",$data,"where Users_ID='".$this->usersid."' and ItemID=".$itemid);
		return $flag;
	}
