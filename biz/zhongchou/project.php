    ...
    <div id="project" class="r_con_wrap">
      ...
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <tbody>
          <?php
		  	$lists = array();
          	$DB->getPage("zhongchou_project","*","WHERE usersid='{$UsersID}' AND Biz_ID={$BizID} order by status asc,addtime desc",10);
		  	while($r=$DB->fetch_assoc()){
            	$lists[] = $r;
			}
			foreach($lists as $k=>$v){
				$item = $DB->GetRs("user_order","count(*) as num,sum(Order_TotalPrice) as amount","WHERE Order_Type='zhongchou_".$v["itemid"]."' and Users_ID='{$UsersID}' AND Biz_ID={$BizID} AND Order_Status = 2 ");
