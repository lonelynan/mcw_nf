...
$OrderID=I("OrderID", 1);
$UsersID = S("Users_ID");
$rsOrder = DB::table("user_order")->where(["Users_ID" => $UsersID, "Order_ID" => $OrderID])->first();

if($_POST){
	if($rsOrder["Order_Status"]<2){
	    $status = I("Status",1);
        $sql = "update user_order as o inner join pintuan_order as p on o.Order_ID = p.order_id set o.Order_Status = {$status}},p
.Order_Status={$status} where o.Order_ID=:OrderID and o.Biz_ID = " . $BizID;
