...
$Type_Arr = $DB->Get('shop_virtual_card_type', 'Type_Id, Type_Name', "WHERE `User_ID` = '".$rsBiz["Users_ID"]."' and Biz_ID='" .$_SESSION['BIZ_ID']. "'");
        ...
        ?>
          <tr>
            ...
            <td nowrap="nowrap"><?php 
              if (!empty($v['Type_Id'])) {
                $type = $DB->GetRs('shop_virtual_card_type', 'Type_Name', 'WHERE Biz_ID=\'' .$_SESSION['BIZ_ID']. '\' and Type_Id='.$v['Type_Id']);
                echo $type['Type_Name'];
              }
            ?></td>
            <td nowrap="nowrap" width="25%" style="text-align:left; padding-left:10px;"><?php 
            	if(empty($v['Products_Relation_ID'])){ 
            		...
            		$One = $DB->GetRs('shop_products', 'Products_Name', 'WHERE Biz_ID=\'' .$_SESSION['BIZ_ID']. '\' and  Products_ID='.$v['Products_Relation_ID']);
            		echo !empty($One) ? $One['Products_Name'] : '<span class="red">'."产品不存在".'</span>';
            	} ?>
            </td>
