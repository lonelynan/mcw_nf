...
if($_POST){
	...
	$rsOrder = $DB->GetRs("user_order","Order_ID,Order_Type,Order_Code,Order_Status","where Order_ID=".$orderid." and Order_Code='".$code."' and Users_ID='".$rsBiz["Users_ID"]."' and Biz_ID=".$_SESSION["BIZ_ID"]);
	if ($rsOrder['Order_Type'] == 'pintuan') {
		$rsOrder = $DB->GetRs("user_order","uo.Order_Status,uo.Order_ID,pt.teamstatus,pt.id,ptd.teamid,ptd.order_id"," as uo left join pintuan_teamdetail as ptd on ptd.order_id = uo.Order_ID left join pintuan_team as pt on pt.id=ptd.teamid where uo.Order_ID=".$orderid." and uo.Order_Code='".$code."' and uo.Users_ID='".$rsBiz["Users_ID"]."' and uo.Biz_ID=".$_SESSION["BIZ_ID"]);
	}
	...
	if(isset($rsOrder['teamstatus']) && $rsOrder['teamstatus'] != 1){
		echo '<script language="javascript">alert("该订单为拼团订单,但尚未拼团成功,不能进行消费验证!");history.back();</script>';
		exit;
	}
