...
$rsShippingsar = $DB->getPage("shop_shipping_template","*",$condition,$pageSize=10);
$template_list = $DB->toArray($rsShippingsar);
...
$rsShippings = $DB->get("shop_shipping_company","Shipping_ID,Shipping_Name",$condition);
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="user" class="r_con_wrap">
      ...
      <?php else: ?>
      <a href="javascript:void(0)" id="create_shipping_template_btn" class="btn_green btn_w_120" add_com = "1" disabled="true">添加</a> 
	  <?php endif;?>
      </div>
	...
	<?php foreach($template_list as $key=>$template):?>
           <tr UserID="<?php echo $template['Template_ID'] ?>">
            ...
            <td>
            	<?php echo isset($company_dropdown[$template['Shipping_ID']]) ? $company_dropdown[$template['Shipping_ID']] : '快递公司不存在！'?>
            </td>
...
<script>
	$("#create_shipping_template_btn").click(function(){
		var add_com = $(this).attr("add_com");
		if(add_com == 1){
			alert("无法添加快递模板，请先添加快递公司。");
		}
	});
</script>