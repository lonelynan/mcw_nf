...
if ($_POST) {
    if(!$_POST["ShippingID"]){
        echo '<script language="javascript">alert("快递单号不能为空");window.location="javascript:history.go(-1);";</script>';
        exit;
    }
    $Data = array(
		...
		<div class="iframe_content">
			...
			<div id="gift_orders" class="r_con_wrap">
				<form id="orders_mod_form" method="post"
					...
					<div class="rows">
						<label>配送方式</label> <span class="input"> 
                        <? if(!empty($PayShipping)){ ?>
                        <select name="Shipping[Express]">
                          <?php 
                            
                            foreach($PayShipping as $key=>$value){?>
                        ...
                        <?php }else{
                        ?>
                        <span style="color:#f00;">商家没有配置物流信息</span>
                        <?php
                        } ?>
                        &nbsp;&nbsp;快递单号： <input name="ShippingID"
							value=""
