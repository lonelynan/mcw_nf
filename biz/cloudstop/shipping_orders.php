...
$condition = "WHERE Users_ID='{$UsersID}' AND Biz_ID = " . $_SESSION["BIZ_ID"] . " AND Orders_Status<4";
if(isset($_GET["search"])){
	if($_GET["search"]==1){
		...
		if(!empty($_GET["AccTime_S"])){
			$condition .= " AND Orders_CreateTime>=".(strtotime($_GET["AccTime_S"])==-1 || strtotime($_GET["AccTime_S"])==false?0:strtotime($_GET["AccTime_S"]));
		}
		if(!empty($_GET["AccTime_E"])){
			$condition .= " AND Orders_CreateTime<=".(strtotime($_GET["AccTime_E"])==-1 || strtotime($_GET["AccTime_E"])==false?0:strtotime($_GET["AccTime_E"]));
		}
	}
}
...
<style>#gift_orders .search .search_btn { width:100px;}</style>
</head>
  ...
  <div class="iframe_content">
    ...
    <div id="gift_orders" class="r_con_wrap"> 
      ...
      <form class="search" id="search_form" method="get" action="">
		...
		<input type="button" class="recieve_btn" value="批量发货" style="background:#1584D5; color:white; border:none; height:22px; cursor:pointer; line-height:22px; border-radius:5px; width:120px;" />
      </form>
