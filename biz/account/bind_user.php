    ...
    <div id="bizs" class="r_con_wrap">
      <form class="r_con_form" method="post" action="?" id="biz_edit">
        <div class="rows">
          <label>手机</label>
          <span class="input">
          <input type="text" name="User_Mobile" value="<?=empty($rsBiz['User_Mobile'])?'':$rsBiz['User_Mobile'];?>" class="form_input" size="35" maxlength="50" notnull/>
          <font class="fc_red">*</font>如果您没有该商城会员,请先关注公众号“<?php echo !empty($rsUsers['Users_WechatAccount'])?$rsUsers['Users_WechatAccount']:$rsUsers['Users_WechatName'];?>”,进入商城个人中心,查看您的手机。</span>
          <div class="clear"></div>
        </div>
        <?php 
        if(!empty($rsBiz['User_Mobile'])){
            $rsUser = $DB->GetRs('user','User_ID,User_NickName,User_Mobile',"where Users_ID='".$rsBiz["Users_ID"]."' and User_Mobile=".$rsBiz['User_Mobile']);
        ?>
            <div class="rows">
                <label>绑定信息</label>
                <span class="input">
					id：<?=empty($rsUser['User_ID'])?'':$rsUser['User_ID']?><br>
                    手机：<?=empty($rsUser['User_Mobile'])?'':$rsBiz['User_Mobile']?><br>
                    昵称&nbsp;:&nbsp;<?=!empty($rsUser['User_NickName'])?$rsUser['User_NickName']:'未填写'?>
                </span>
                <div class="clear"></div>
            </div>
