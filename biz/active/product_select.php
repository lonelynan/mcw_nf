...
$time = time();
$sql = "SELECT * FROM active WHERE Users_ID='{$UsersID}' AND Active_ID='{$active_id}' AND Status=1";
...
/*if ($rsActive['module'] == 'pintuan') { // 拼团
    ...
    $time = time();
    $condition .= " `Users_ID` = '{$UsersID}' AND Biz_ID={$BizID} AND starttime<={$time} AND stoptime>={$time}";
} elseif ($rsActive['module'] == 'cloud') { // 云购
    $table = "cloud_products";
    $condition .= " `Users_ID` = '{$UsersID}' AND Biz_ID={$BizID} AND Products_SoldOut=0";
...
$table = "shop_products";
$time = time();
$condition .= " `Users_ID` = '{$UsersID}' AND Biz_ID={$BizID} ";
  ...
  <div class="iframe_content">
    ...
    <div id="products" class="r_con_wrap"> 
      <form class="search" method="get" action="?" style="display:block">
        ...
        <input type="hidden" name="activeid" value="<?=$active_id ?>" />
        <input type="submit" class="search_btn" value="搜索" />
      </form> 
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        <thead>
          <tr>
            <td width="8%" nowrap="nowrap"><label>全选</label>&nbsp;&nbsp;<input type="checkbox" id="chose" class="listNum" value="" style="vertical-align:middle; margin:5px;"></td>
        ...
        ?>
	        <tr>
	          ...
	          <td nowrap="nowrap" vkname="<?=$v['Products_Name'] ?>">
	          	<input type="checkbox" name="select[]" id="n<?=$v['Products_ID'] ?>" class="listNum<?=$v['Products_ID'] ?> pdtwo" value="<?=$v['Products_ID'] ?>" >
	          </td>
	          ...
	          <td nowrap="nowrap" vkname="<?=$v['title'] ?>">
	          	<input type="checkbox" name="select[]" id="n<?=$v['itemid'] ?>" class="listNum<?=$v['itemid'] ?> pdtwo" value="<?=$v['itemid'] ?>" >
	          </td>
	          <td><?=$v['title'] ?></td>
      ...
      <?php $DB->showPage(); ?>
	    <div id="actionBox">
            <button class="btn_green" style="float:right;margin-right:40%;" href="javascript:void(0);" id="addInsert">选择</button>
      	</div>
    </div>
  </div>
</div>
