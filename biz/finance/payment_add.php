...
$BizRs = $DB->GetRS('biz','Users_ID,User_Mobile',"where Users_ID='".$rsBiz["Users_ID"]."' and BIZ_ID=".$_SESSION["BIZ_ID"]);
if(empty($BizRs['User_Mobile'])){
...
if ($_POST) {
    ...
    switch ($_POST['PaymentID']) {
        case 1:
            {
                $TData['OpenID'] = $_POST["OpenID"];
                break;
            }
        case 2:
            {
                $TData['aliPayNo'] = $_POST["aliPayNo"];
                $TData['aliPayName'] = $_POST["aliPayName"];
                break;
            }
        case 3:
            {
                $TData['Bank'] = $_POST["Bank"];
                $TData['BankNo'] = $_POST["BankNo"];
                $TData['BankName'] = $_POST["BankName"];
                $TData['BankMobile'] = $_POST["BankMobile"];
                break;
            }
    }
    
    if(empty($TData)){
        echo '<script language="javascript">alert("您所选择的结算方式没有配置！");location.href="/biz/account/account_payconfig.php";</script>';
        exit;
    }
...
<![endif]-->
	<style type="text/css">
body, html {
	background: url(/static/member/images/main/main-bgg.jpg) left top fixed
		no-repeat;
}
</style>
	<div id="iframe_page">
                ...
                <div class="clear"></div>
					...
					<div class="rows">
						<label>付款账户</label> <span class="input"> <input name="aliPayNo"
							value="<?php if(!empty($biz_PayConfig) && $biz_PayConfig['PaymentID'] ==2){ echo isset($biz_PayConfig['config']['aliPayNo'])?$biz_PayConfig['config']['aliPayNo']:""; } ?>"
							type="text" class="form_input" size="40" maxlength="100" notnull>
							<font class="fc_red">*</font></span>
						<div class="clear"></div>
					</div>
					<div class="rows">
						<label>付款账户名</label> <span class="input"> <input name="aliPayName"
							value="<?php if(!empty($biz_PayConfig) && $biz_PayConfig['PaymentID'] ==2){ echo isset($biz_PayConfig['config']['aliPayName'])?$biz_PayConfig['config']['aliPayName']:""; } ?>"
							type="text" class="form_input" size="40" maxlength="100" notnull>
							<font class="fc_red">*</font></span>
						<div class="clear"></div>
					</div>
