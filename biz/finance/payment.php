...
if (isset($_GET["search"])) {
    if ($_GET["search"] == 1) {
        ...
        if (! empty($_GET["AccTime_S"])) {
            $condition .= " AND CreateTime>=" . (strtotime($_GET["AccTime_S"])==-1 || strtotime($_GET["AccTime_S"])==false?0:strtotime($_GET["AccTime_S"]));
        }
        if (! empty($_GET["AccTime_E"])) {
            $condition .= " AND CreateTime<=" . (strtotime($_GET["AccTime_E"])==-1 || strtotime($_GET["AccTime_E"])==false?0:strtotime($_GET["AccTime_E"]));
        }
    }
}
$BizPayRate = array();
...
if (isset($_GET["action"])) {
    if ($_GET["action"] == "getpay") {
        ...
        $usermoney = $Biz_ID["Total"]-($Biz_ID["Total"]*$BizPayRate[$Biz_ID["Biz_ID"]]/100);
                ...
                $User_Mobile = $DB->GetRs("biz",'User_Mobile',"where Biz_ID=".$Biz_ID['Biz_ID']);
                if (empty($User_Mobile["User_Mobile"])) {
                    echo '<script language="javascript">alert("您没有绑定前台会员,暂不能结款!");history.back();</script>';
                    exit();
                }
                $flag = $DB->Set("user",'User_Money=User_Money+'.$usermoney,"where User_Mobile=".$User_Mobile['User_Mobile']);
                $rsUser=$DB->GetRs("user","User_Money,User_ID","where Users_ID='".$rsBiz["Users_ID"]."' and User_Mobile='".$User_Mobile["User_Mobile"]."'");
                if($flag){
                    //增加资金流水
                    $Data=array(
                    ...
                    $flaggo = $Add=$DB->Add('user_money_record',$Data);
                }        
		if($flag&&$flaggo){
    ...
    if ($_GET["action"] == "del") {
        ...
        $flag = $DB->Set("shop_sales_record", array(
            "Record_Status" => 0,
            "Payment_ID" => 0
        ), "WHERE Payment_ID=" . $paymentid);
        $flaggo = $DB->Del("shop_sales_payment", "Payment_ID=" . $paymentid);        
		if($flag&&$flaggo){
...
echo "+转向余额";
            ...
            <td nowrap="nowrap"> 
                ...
                <?php if($value["Status"]==2){?>
                    <a href="?action=getpay&paymentid=<?php echo $paymentid;?>" onClick="if(!confirm('确定收款？')){return false};">[确定收款]</a>&nbsp;<?php } ?>
                <?php if($value["Status"]==0 || $value["Status"]==3){?>
                    <a href="?action=del&paymentid=<?php echo $paymentid; ?>" onClick="if(!confirm('删除后不可恢复，继续吗？')){return false};">[删除]</a>
                <?php }?>
<?php
?>
            </td>
