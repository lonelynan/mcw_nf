...
class web_apiController extends controllController {
	...
	public function recommend_listOp() {
		...
		if ($gc_id > 0) {
			$istop = model('shop_category')->where(array('Users_ID'=>$_SESSION['Users_ID'],'Category_ID'=>$gc_id))->find();
				if ($istop['category_parentid'] == 0) {
				$ischiled = model('shop_category')->field('Category_ID')->where(array('Users_ID'=>$_SESSION['Users_ID'],'category_parentid'=>$gc_id))->select();
				$catgoryid = array();
				$n = 0;
				$catgoryid[0] = 'in';
				foreach($ischiled as $key => $val){
					$n++;
					foreach($val as $k => $v){
					...
					$condition['Products_Category'] = $catgoryid;
				}else{
	 ...
	 * 保存设置
	 */
	public function code_updatecayOp() {
		$code_id = intval($_POST['code_id']);
		$web_id = intval($_POST['web_id']);
		if (isset($_POST['category_list'])) {
			if (count($_POST['category_list']['goods_class']) < 12) {
		$model_web_code = model('web_code');
		$code = $model_web_code->where(array('code_id'=>$code_id,'web_id'=>$web_id))->find();
