		...
		editor.loadPlugin('image', function() {
			editor.plugin.imageDialog({
				clickFn : function(url, title, width, height, border, align) {
					K('#PicDetail').append('<div><a href="'+url+'" target="_blank"><img src="'+url+'" /></a> <span onclick="$(this).parent().remove();">删除</span><input type="hidden" name="pic" value="'+url+'" /></div>');
					editor.hideDialog();
				}
			});
		});
			...
			<form class="r_con_form" method="post" action="<?php echo url('pc_diy/focus_edit');?>">
				...
				<div class="rows">
					...
					<div>
						...
						<div class="tips">共可上传<span id="pic_count">1</span>张图片 建议尺寸：1920*360</div>
						<div class="clear"></div>
					</div>
					</span>
