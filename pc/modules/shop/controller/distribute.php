		...
		if(IS_AJAX) {
			...
			if($rsRecords) {
				foreach($rsRecords as $key => $val) {
					...
					$rsRecords[$key]['Productsattr'] = json_decode($val['skuvaljosn'], TRUE);
					$rsRecords[$key]['Productsattrkeystrval'] = $rsRecords[$key]['Productsattr'][$val['skukey']];
					$rsRecords[$key]['skustr'] = $val['skustr'];
			    }
				$data = array(
					'list' => $rsRecords,
					'totalpage' => $totalpage,
