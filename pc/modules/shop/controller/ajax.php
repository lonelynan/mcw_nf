...
class ajaxController extends controllController {
    ...
    private function _add_to_cart() {
		...
		$skuvaljosn = json_decode($rsProducts['skuvaljosn'], true); 
		...
		$attrrkey = substr($_POST['attrkeystrval'],0,strlen($_POST['attrkeystrval'])-1); 
		if (!empty($rsProducts['skuvaljosn'])) {		
		//属性值
		$Property['shu_value'] = $_POST['attrstrval'];
		$Property['shu_vid'] = $attrrkey;
		$Property['shu_price'] = $skuvaljosn[$attrrkey]['Txt_PriceSon'] * $_POST["Qty"];
		$Property['shu_pricesimp'] = $skuvaljosn[$attrrkey]['Txt_PriceSon'];
		$Property['shu_priceg'] = $skuvaljosn[$attrrkey]['Txt_PriceG'];
		$Property['shu_conut'] = $skuvaljosn[$attrrkey]['Txt_CountSon'];
		$Property['showimg'] = $_POST['attrimgval'];
	}
	...
	if(!empty($_SESSION[$cart_key])){
        ...
        if ($flag) {
            ...
            $CartList[$BizID][$ProductsID][$Property['shu_vid']] = array(
				...
				'Productsattrstrval' => $_POST['attrstrval'],
				'Productsattrkeystrval' => $skuvaljosn[$attrrkey],
    ...
    private function _cart_update() {
        ...
        $total = 0;
        $rsProducts = model('shop_products')->field('Products_Count,skuvaljosn')->where(array(
            'Users_ID' => $this->UsersID,
        ...
        $Data = array();
		if (!empty($rsProducts['skuvaljosn'])) {
					$procount = $CartList[$BizID][$ProductsID][$CartID]['Productsattrkeystrval']['Txt_CountSon'];
				}else{
					$procount = $rsProducts['Products_Count'];					
				}
        switch ($Type) {
            case 'qty_less': //减少
                if ($Qty <= 1) {
            ...
            case 'qty_add': //增加				
                if ($Qty >= $procount && $procount > 0) {
                    $Data['status'] = 1;
                    $Data['qty'] = $procount;
                    $Data['msg'] = '产品库存不足，最多能购买' . $procount . '件';
                    $CartList[$BizID][$ProductsID][$CartID]['Qty'] = $procount;
                } else {
                    $CartList[$BizID][$ProductsID][$CartID]['Qty'] = $Qty + 1;
                    $Data['status'] = 2;
                    $Data['qty'] = $Qty + 1;
            ...
            case 'qty_input':
                if ($Qty < 1) {
                    ...
                    $Data['msg'] = '最小购买数量为1！';
                    $CartList[$BizID][$ProductsID][$CartID]['Qty'] = 1;
                } elseif ($Qty > $procount && $procount > 0) {
                    $Data['status'] = 1;
                    $Data['qty'] = $procount;
                    $Data['msg'] = '产品库存不足，最多能购买' . $procount . '件';
                    $CartList[$BizID][$ProductsID][$CartID]['Qty'] = $procount;
        ...
        foreach ($CartList as $bizid => $bizcart) {
            foreach ($bizcart as $productsid => $products) {
                foreach ($products as $carid => $cart) {
					if (!empty($rsProducts['skuvaljosn'])) {					
					$proprice = $cart['Productsattrkeystrval']['Txt_PriceSon'];
					}else{					
					$proprice = $cart['ProductsPriceX'];
					}
                    if ($bizid == $BizID) {
                        $heji+= $cart['Qty'] * $proprice;
                    }
                    if ($productsid == $ProductsID && $carid == $CartID) {
                        $xiaoji+= $cart['Qty'] * $proprice;
                    }
                    $total+= $cart['Qty'] * $proprice;
                }
            }
        }
        $Data['total'] = $total;
                ...
                foreach ($BizCart as $kk => $vv) {
                    ...
                    foreach ($vv as $k => $v) {
						...
						$vprice = $v['ProductsPriceX'];						
                            ...
                            add_distribute_record($this->UsersID, $v['OwnerID'], $vprice, $kk, $v['Qty'], $neworderid, $v['ProductsProfit'], $k, $v['Productsattrstrval'], $v['Property']);
                        }
                    }
                    $condition = array(
                        'Users_ID' => $this->UsersID,
