...
class memberController extends controllController {
	...
	public function indexOp() {
		...
		//我的收藏
		$sql = "select f.FAVOURITE_ID,p.Products_ID,p.Products_Name,Products_PriceX,p.Products_PriceY,Products_PriceA,Products_PriceAmax,p.Products_JSON from Shop_Products as p join user_favourite_products as f on p.Products_id = f.Products_ID and f.User_ID =".$_SESSION[$this->UsersID . 'User_ID'];
				...
				if(!empty($CartList)) {
					foreach($CartList as $k => $v){
						foreach($v as $k2 => $v2){
							...
							$rsOrders[$key]['ProductsPriceA'] = $v2['ProductsPriceA'];
							$rsOrders[$key]['ProductsPriceAmax'] = $v2['ProductsPriceAmax'];
							$rsOrders[$key]['Productsattrstrval'] = $v2['Productsattrstrval'];
							$rsOrders[$key]['Productsattrkeystrval'] = $v2['Productsattrkeystrval'];
	...
	public function shoucangOp() {
		...
		if(IS_AJAX) {
			...
			$limitpage = ($p-1) * $num;//每次查询取记录
			//获取此用户所收藏的商品
			$sql2 = "select f.FAVOURITE_ID,p.Products_ID,p.Products_Name,Products_PriceX,p.Products_PriceY,p.Products_PriceA,p.Products_PriceAmax,p.Products_JSON from Shop_Products as p join user_favourite_products as f on p.Products_id = f.Products_ID and f.User_ID =".$_SESSION[$this->UsersID . 'User_ID'] . " limit $limitpage, $num";
