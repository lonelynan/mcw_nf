		...
		if(strpos($this->orderid, 'PRE') >- 1){
			...
			if(!strpos($orderinfo['order_type'], 'zhongchou')){
				if($orderinfo['order_type'] == 'weicbd'){
					$pay_subject = $this->shopConfig['shopname'].'微商圈在线付款，订单编号:' . $this->orderid;
				}elseif($orderinfo['order_type'] == 'kanjia'){
					$pay_subject = $this->shopConfig['shopname'].'微砍价在线付款，订单编号:' . $this->orderid;
				}else{
					$pay_subject = $this->shopConfig['shopname'].'微商城在线付款，订单编号:' . $this->orderid;
				}
			}else {
				$pay_subject = $this->shopConfig['shopname'].'微众筹在线付款，订单编号:' . $this->orderid;
			}
			$data = array(
