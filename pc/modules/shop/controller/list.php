	...
	function _list(){
	    ...
	    if(IS_AJAX) {
			...
			if(!empty($_GET['Keyword'])){//搜索页面
				...
				if($this->id){
				$istop = $category_model->where(array('Users_ID'=>$this->UsersID,'Category_ID'=>$this->id))->find();
				if ($istop['category_parentid'] == 0) {
				$ischiled = $category_model->field('Category_ID')->where(array('Users_ID'=>$this->UsersID,'category_parentid'=>$this->id))->select();
				$catgoryid = array();
				$n = 0;
				$catgoryid[0] = 'in';
				foreach($ischiled as $key => $val){
					...
					$where['Products_Category'] = $catgoryid;
				}else{
