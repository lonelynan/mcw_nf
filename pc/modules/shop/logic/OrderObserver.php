...
class OrderObserver {
	...
	private function handle_dis_sha_info()
	{
            ...
            $issha = sha_sifenh($order['Users_ID'], $order['User_ID'], $this->shop_config['Sha_Money']);
            //获取各级股东和各级股东人数
				...
				if (!empty($productInfo['sha_Reward'])) 
		    		{
	    				...
	    				$sha_record_monery = $productInfo['Qty']*($productInfo['ProductsProfit']*($productInfo['platForm_Income_Reward']/100)*($productInfo['sha_Reward']/100));
                                        ...
                                        foreach ($Sha_Rate_Count as $ks => $vs) {
											...
											$cleshanum = sha_nofenhnum($order['Users_ID'], $order['User_ID'], $this->shop_config['Sha_Money']);
										if ($cleshanum) {
											$shanum = $vs['count'] - $cleshanum;
										} else {
											$shanum = $vs['count'];
										}
