	...
	<div class="itemInfo">
		...
		<div class="summary">
			...
			<?php foreach($output['specification'] as $spec_key => $spec) {?>
				...
				<div class="pub"> <span class="dt"><?=$spec['Name'][$ks1]?>：</span>
					<div class="pub_right spec">
						...
						<?php if($spec['Attr_Type'] == 1) {?>
							...
							?>
							<a class="<?=($ks1 == 0)?'':'porpers';?>"  onclick="changeAtt(this)" href="javascript:;" url="<?=($ks1 == 0)?'':'porpers';?>" name="<?=$value['id']?>" title="<?=$value['label'][$ks1]?>">
								<?php if($key == 0){
									$spec_list[] = $value['id'];
								}?>
							    <?=$value['label'][$ks1]?>
							    <input style="display:none" id="spec_value_<?=$value['id']?>" type="radio" name="spec_<?=$spec_key?>" value="<?=$value['id']?>" <?=($key == 0) ? 'checked' : '';?> />
								<input class="childrenatt" style="display:none" value="<?=$value['label'][$ks1]?>"> 
							    <i></i>
							</a>
							<?php 
							    $array_vals[$ks1][] = $value['label'][$ks1];
							?>
							<?php } ?>
						<?php }else { ?>
							    <?php foreach($spec['Values'] as $key => $value){ ?>
			...
			<input type="hidden" id="spec_list" name="spec_list" value="<?=implode(',', $spec_list)?>" />
				...
				<div class="pub count_box" style="display:none;"> <span class="dt"></span>
					<div class="pub_right"> 
					    库存<span id="stock_val"><?=$output['rsProducts']['products_count']?></span>件
				        商品编号：<span id="number_id"></span>
					</div>
				</div>
			</form>
