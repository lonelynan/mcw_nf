...
var pricemin = <?php echo $output['rsProducts']['products_pricea'] ? $output['rsProducts']['products_pricea'] : 0.00;?>;
var pricemax = <?php echo $output['rsProducts']['products_priceamax'] ? $output['rsProducts']['products_priceamax'] : 0.00;?>;
var kucuncount = <?php echo $output['rsProducts']['products_count'] ? $output['rsProducts']['products_count'] : 0;?>;
var skustr = <?php echo $output['skustr'] ? $output['skustr'] : '\'jie\'';?>;
var skuimg = <?php echo $output['skuimg'] ? $output['skuimg'] : '\'jie\'';?>;
</script>
<link href='<?php echo $output['_site_url'];?>/static/pc/shop/css/css.css' rel="stylesheet" type="text/css" />
<link href='<?php echo $output['_site_url'];?>/static/pc/shop/css/attrbase.css' rel="stylesheet" type="text/css" />
<script src='<?php echo $output['_site_url'];?>/static/pc/shop/js/goods.js?t=<?=time();?>'></script>
<script src='<?php echo $output['_site_url'];?>/static/pc/shop/js/skupc.js?t=<?=time();?>'></script>
		...
		<div class="summary">
			<div class="summary-price">
				...
				<div class="price" style="padding:8px 0;">
					<div class="now_price">
					<?php if(!empty($output['skuarr'])) { ?>
						<span class="money_icon">现价：￥</span><price id="changprice"><?php echo $output['rsProducts']['products_pricea'] == $output['rsProducts']['products_priceamax'] ? $output['rsProducts']['products_priceamax'] : $output['rsProducts']['products_pricea'].'-'.$output['rsProducts']['products_priceamax'];?></price><br />
						<?php }else{ ?>
						<span class="money_icon">现价：￥</span><price id="changprice"><?php echo $output['cur_price'];?></price><br />
						<?php } ?>
			...
			<?php $i = 1; ?>
			<?php foreach($output['skuarr'] as $key => $val){?>
			<dl class="clearfix iteminfo_parameter sys_item_specpara" data-sid="<?=$i?>">
                <dt><?=$key?></dt>
                <dd>
                    <ul class="sys_spec_text">
					<?php foreach($val as $k => $v){?>
                        <li data-aid=<?=$k?>><a href="javascript:;" title="<?=$v?>"><?=$v?></a><i></i></li>
						<?php }?>
                    </ul>
                </dd>
            </dl>
			...
			<?php }?>
				...
				<div class="pub amount"> <span class="dt">库存：<span id="kucount"><?php echo $output['rsProducts']['products_count'] ? $output['rsProducts']['products_count'] : 0;?></span></span></div>
			</form>
						...
						<div>
							...
							<div class="beibi_tui_word">
								<a href="<?php echo $val[0]['link'];?>">
									...
									<span>￥<?php echo empty(intval($val[0]['products_priceamax'])) ? $val[0]['products_pricex'] : $val[0]['products_pricea'];?></span>
								</a>
							</div>
						</div>
						<?php if(!empty($val[1])){?>
						<div>
							...
							<div class="beibi_tui_word">
								<a href="<?php echo $val[1]['link'];?>">
									...
									<span>￥<?php echo empty(intval($val[1]['products_priceamax'])) ? $val[1]['products_pricex'] : $val[1]['products_pricea'];?></span>
								</a>
							</div>
						</div>
						<?php }?>
...
<div class="control" id="control">
	...
	<div class="see_agine">
		...
		<div class="see_agine_beibi">
			...
			<div class="beibi_price">￥<?php echo empty(intval($val['products_priceamax'])) ? $val['products_pricex'] : $val['products_pricea']?></div>
		</div>
		<?php }?>
		<?php }?>
	</div>
