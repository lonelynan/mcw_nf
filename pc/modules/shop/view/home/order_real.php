					...
					<table class="wzw-table-style">
						...
						<tbody>
							...
							foreach($Product_List as $key => $product){
								if (!empty($product['Productsattrstrval'])) {
								$curprice = $product["Productsattrkeystrval"]["Txt_PriceSon"];
								$showimg = $product["Property"]["showimg"];
								}else{
								$curprice = $product["ProductsPriceX"];
								$showimg = $product['ImgPath'];
								}
								$total += $product['Qty'] * $curprice;
								$qty += $product['Qty'];
									
								$heji += $product['Qty'] * $curprice;
								$qty2 += $product['Qty'];
									
								$xiaoji += $product['Qty'] * $curprice;
							...
							<tr class="shop-list" BizID="<?php echo $Biz_ID;?>" ProductsID="<?php echo $Products_ID;?>" CartID="<?php echo $key;?>">
								<td>&nbsp;&nbsp;</td>
								<td><a class="wzw-goods-thumb" target="_blank" href="<?php echo url('goods/index', array('id'=>$Products_ID));?>"><img alt="<?php echo $product['ProductsName'];?>" src="<?=$showimg?>"></a></td>
								<td><?=$product["Productsattrstrval"]?></td>
								<td><em><?=$curprice?></em></td>
								<td><?php echo $product['Qty'];?></td>
								<td><em wzw_type="eachGoodsTotal"><?php echo $product['Qty']*$curprice;?></em></td>
								<td></td>
							</tr>
							<?php }?>
							<tr>
