...
<script src='<?php echo $output['_site_url'];?>/static/pc/shop/js/cart.js?t=1'></script>
<script>
					...
					<tbody>
						...
						foreach($BizCart as $ProductsID => $Products){							
							...
							foreach($Products as $CartID => $Cart){
								if (!empty($Cart['Productsattrstrval'])) {
								$curprice = $Cart["Productsattrkeystrval"]["Txt_PriceSon"];
								$showimg = $Cart["Property"]["showimg"];
								}else{
								$curprice = $Cart["ProductsPriceX"];
								$showimg = $Cart['ImgPath'];
								}
								$total += $Cart['Qty'] * $curprice;
								$qty += $Cart['Qty'];
								
								$heji += $Cart['Qty'] * $curprice;
								$qty2 += $Cart['Qty'];
								
								$xiaoji += $Cart['Qty'] * $curprice;
								$qty3 += $Cart['Qty'];
						...
						<tr class="shop-list" BizID="<?php echo $BizID;?>" ProductsID="<?php echo $ProductsID;?>" CartID="<?php echo $CartID;?>">
							<td>&nbsp;&nbsp;</td>
							<td class="w60"><a class="wzw-goods-thumb" target="_blank" href="<?php echo url('goods/index', array('id'=>$ProductsID));?>"><img alt="<?php echo $Cart['ProductsName'];?>" src="<?=$showimg?>"></a></td>
							<td><?=$Cart["Productsattrstrval"]?></td>
							<td class="w120"><em><?=$curprice?></em></td>
							<td class="w120 ws0">
							...
							<td class="w120"><em wzw_type="eachGoodsTotal"><?php echo $Cart['Qty'] * $curprice;?></em></td>
							<td class="w80">
