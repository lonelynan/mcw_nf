		    ...
		    <div class="wzw-main">
				...
				<div class="wzw-receipt-info">
					...
					<table class="wzw-table-style">
						...
						<?php foreach($output['cartList'] as $Products_ID => $Product_List) {?>
						<tbody>
							<?php
							foreach($Product_List as $key => $product){
								$total += $product['Qty'] * $product['ProductsPriceX'];
								$qty += $product['Qty'];
								if($product['IsShippingFree'] == 0){
									$ShippingFree = 0;
								}
							?>
							<tr class="shop-list" ProductsID="<?php echo $Products_ID;?>" CartID="<?php echo $key;?>">
								<td>&nbsp;&nbsp;</td>
								...
								<td><?php echo $qty;?></td>
								<td><em wzw_type="eachGoodsTotal"><?php echo $total;?></em></td>
								<td></td>
							</tr>
							<?php }?>
							<tr>
								...
								<td colspan="2" class="tl">买家留言：
									<textarea maxlength="150" title="选填：对本次交易的说明（建议填写已经和商家达成一致的说明）" placeholder="选填：对本次交易的说明（建议填写已经和商家达成一致的说明）" class="wzw-msg-textarea" name="Remark"></textarea></td>
							...
							<div style="">
								<div class="wzw-form-default vat_invoice_panel">
									<dl>
										...
										<dd>
										    <input type="hidden" name="Order_NeedInvoice" value="0" class="Order_NeedInvoice" />
											<input type="text" value="" name="Order_InvoiceInfo" maxlength="200" class="text Order_InvoiceInfo" size="100" />
										</dd>
									</dl>
								</div>
							</div>
				...
				<!-- 发票end -->
				<!-- 优惠券begin -->
				<?php if($output['coupon_info']){?>
				<div class="wzw-receipt-info">
				    <div class="wzw-receipt-info-title">
						<h3>优惠券</h3>&nbsp;&nbsp;
					</div>
					<ul class="coupon_box">
						<?php foreach($output['coupon_info'] as $k => $v){?>
						<li>
							<input type="radio" value="<?=$v['Coupon_ID']?>" name="CouponID" class="coupon">
							    <?=$v['Subject']?>
						</li>
						<?php }?>
					</ul>
				...
				<?php if($ShippingFree == 0) {?>
				    <div class="wzw-receipt-info-title" style="font-size:14px;line-height:22px;">
						<h3>配送方式</h3>&nbsp;&nbsp;
						<?=$output['order_total_info']['Shipping_Name']?>&nbsp;&nbsp;
						<span id="shipping_fee_txt">
						<?=$output['order_total_info']['total_shipping_fee'];?>
						元 
						</span>
						<a href="javascript:void(0)" class="shipping_method">[重新选择]</a>
						<input type="hidden" name="Shipping_ID" value="<?=$output['order_total_info']['Shipping_ID']?>"/>
					</div>
				<?php }else {?>
					<div class="wzw-receipt-info-title">
				...
				<!-- 配送方式end -->
				<!-- 积分begin -->
				<?php if($output['shopConfig']['integral_convert']){?>
				<div class="wzw-receipt-info">
				    <div class="wzw-receipt-info-title">
						<h3>此次购物可获得<span id="total_integral" style="color:#F60;"><?=$output['order_total_info']['integral']?></span>个积分</h3>
						<h3>&nbsp;&nbsp;&nbsp;&nbsp;此次购物符合满<span id="reach_txt" style="color:#F60;"><?=$output['order_total_info']['man_info']['reach']?></span>减<span id="award_txt" style="color:#F60;">￥<?=$output['order_total_info']['man_info']['award']?></span></h3>
					</div>
				</div>
				<?php }?>
				<!-- 积分end -->
