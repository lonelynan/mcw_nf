...
$pay_total = intval(floatval($pay_fee)*100);
...
$jsApi = new JsApi_pub();
if (!isset($_GET['code'])){
	$url = $jsApi->createOauthUrlForCode("http://".$_SERVER['HTTP_HOST']."/pay/wxpay2/sendto_qiammi.php?UsersID=".$UsersID."_".$OrderID);
	Header("Location: $url");
}else{
	$code = $_GET['code'];
	$jsApi->setCode($code);
...
$unifiedOrder->setParameter("notify_url","http://".$_SERVER['HTTP_HOST']."/pay/wxpay2/notify_qiammi.php");
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					...
					if(res.err_msg=='get_brand_wcpay_request:ok'){
						window.location.href = 'http://<?php echo $_SERVER['HTTP_HOST'];?>/pay/wxpay2/notify_qiammi.php?OrderID=<?php echo $OrderID;?>';
					}
				}
			);
		}
