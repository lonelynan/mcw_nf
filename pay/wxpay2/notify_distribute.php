...
if(isset($GLOBALS['HTTP_RAW_POST_DATA'])){
    file_put_contents('xhh_distribute.txt', $GLOBALS['HTTP_RAW_POST_DATA']);
	...
	$rsOrder=$DB->GetRs("distribute_order","Users_ID,User_ID,Order_Status,Level_ID,Order_Present,Order_TotalPrice","where Order_ID='".$OrderID."'");
	if(!$rsOrder){
		echo "订单不存在";
		exit;
	}
			...
			if($Status==1){
				...
				if($data["status"]==1){
						...
						if($rsLevel['Present_type'] == 2){
							$User_Money = $rsUser['User_Money'] + $rsOrder['Order_Present'] + $rsOrder['Order_TotalPrice'];
							$DB->set('user', array('User_Money' =>$User_Money), ' WHERE `Users_ID`="' .$rsOrder['Users_ID']. '" AND `User_ID` = '.$rsOrder['User_ID']);	
						}
					echo "SUCCESS";
					exit;
				}else{
