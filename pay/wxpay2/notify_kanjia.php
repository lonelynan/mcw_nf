	...
	if($notify->checkSign() == TRUE){
		if ($notify->data["return_code"] == "FAIL") {
			...
			if($Status==1){
				...
				if($rsOrder['Order_CartList']){
					$Order_Product = json_decode($rsOrder['Order_CartList']);
					foreach($Order_Product as $key=>$value){
						$Product_ID = $key;
					}
					$rsProduct = $DB->GetRs("shop_products","*","where Products_ID='".$Product_ID."'");
					$products_data['Products_Count'] = $rsProduct['Products_Count'] - 1;
					$DB->Set("shop_products",$products_data,"where Users_ID='".$UsersID."' and Products_ID=".$Product_ID);
				}
				if($data["status"]==1){
					echo "SUCCESS";
					exit;
