...
if(strpos($OrderID,"PRE")>-1){
	...
	//$OrderID = $rsOrder["orderids"];	
}else{
	$rsOrder=$DB->GetRs("user_order","Users_ID,Order_Status,Order_Type","where Order_ID='".$OrderID."'");
	$status = $rsOrder['Order_Status'];
}
if($status>=2){
	if (isset($rsOrder['Order_Type']) && $rsOrder['Order_Type'] == 'pintuan') {
		$url = '/api/'.$UsersID.'/pintuan/orderlist/'.$status.'/';
	} else {
		$url = '/api/'.$UsersID.'/shop/member/status/'.$status.'/';
	}
	layerOpen("您已经支付过不能重复支付", 3, $url);
	exit;
}
...
$pay_total = preg_replace('/\.[0-9]+/','',strval($pay_fee*100));
