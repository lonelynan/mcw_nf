...
class weixin_message{
	...
	public function sendmessage($openid,$contentStr){//单用户发送
		...
		if($this->access_token){
			...
			curl_close($ch);
			$message = $contentStr;
			$this->db->Add("weixin_log",array("message"=>$message.' _ '.$res.' _ '.$openid,'log_CreateTime'=>time()));
		}
    }
	...
	public function sendmess($openid_array,$contentStr){//群发消息
		...
		if($this->access_token){
			...
			curl_close($ch);
			//$message = $contentStr;
			$this->db->Add("weixin_log",array("message"=>'群发'.$postdata,'log_CreateTime'=>time()));
		}
    }
	...
	public function sendmember(){
        ...
        $disRes = $this->db->GetRs("distribute_config","Dis_Level,Index_Professional_Json","where Users_ID='".$this->usersid."'");
		$show_word = json_decode($disRes['Index_Professional_Json'],true);
		if (empty($show_word)) {
		$show_word = array(1=>'一级会员',2=>'二级会员',3=>'三级会员',4=>'四级会员',5=>'五级会员',6=>'六级会员',7=>'七级会员',8=>'八级会员',9=>'九级会员');			
		}
					...
					/* if(is_array($u4)){
						...
						if(is_array($u5)){
							...
							if(is_array($u6)){
								...
								if(is_array($u8)){
									$text = "您的".$show_word['childlevelterm'][8].$name."关注了本公众号";
								...
								if(is_array($u9)){
									$text = "您的".$show_word['childlevelterm'][9].$name."关注了本公众号";
	...
	public function sendorder($money,$orderid){//佣金提醒
		$rsConfig = $this->db->GetRs("distribute_config","Withdraw_Type,Withdraw_Limit,Dis_Level,Index_Professional_Json","where Users_ID='".$this->usersid."'");
		$show_word = json_decode($rsConfig['Index_Professional_Json'],true);
		if (empty($show_word)) {
		$show_word['childlevelterm'] = array(1=>'一级会员',2=>'二级会员',3=>'三级会员',4=>'四级会员',5=>'五级会员',6=>'六级会员',7=>'七级会员',8=>'八级会员',9=>'九级会员');
		$show_word['catcommission'] = '佣金';
		}
		...
		if($self_bonus>0){
			$text = "您下单成功，支付了".$money."元，您将获取".(!empty($show_word['catcommission'])?$show_word['catcommission']:'').$self_bonus."元";
		...
		if(is_array($u1)){
			...
			$text = "您推荐的".(!empty($show_word['childlevelterm'][1])?$show_word['childlevelterm'][1]:'').$name."下单成功，支付了".$money."元，您将获取".(!empty($show_word['catcommission'])?$show_word['catcommission']:'').$bonus."元";
			$this->sendmessage($u1["User_OpenID"],$text);
			if($dis_level == 1) return;
			$u2 = $this->getownerinfo($u1["Owner_Id"]);
			if(is_array($u2)){
				$bonus = $this->get_bonus($orderid, $u1["Owner_Id"]);
				$text = "您推荐的".$show_word['childlevelterm'][2].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
				$this->sendmessage($u2["User_OpenID"],$text);
                                if($dis_level == 2) return;
				$u3 = $this->getownerinfo($u2["Owner_Id"]);
				if(is_array($u3)){
					$bonus = $this->get_bonus($orderid, $u2["Owner_Id"]);
					$text = "您推荐的".$show_word['childlevelterm'][3].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
					$this->sendmessage($u3["User_OpenID"],$text);
                                        if($dis_level == 3) return;
					$u4 = $this->getownerinfo($u3["Owner_Id"]);
					/* if(is_array($u4)){
						$bonus = $this->get_bonus($orderid, $u3["Owner_Id"]);
						$text = "您推荐的".$show_word['childlevelterm'][4].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
						$this->sendmessage($u4["User_OpenID"],$text);
                                                if($dis_level == 4) return;
						$u5 = $this->getownerinfo($u4["Owner_Id"]);
						if(is_array($u5)){
							$bonus = $this->get_bonus($orderid, $u4["Owner_Id"]);
							$text = "您推荐的".$show_word['childlevelterm'][5].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
							$this->sendmessage($u5["User_OpenID"],$text);
                                                        if($dis_level == 5) return;
							$u6 = $this->getownerinfo($u5["Owner_Id"]);
							if(is_array($u6)){
								$bonus = $this->get_bonus($orderid, $u5["Owner_Id"]);
								$text = "您推荐的".$show_word['childlevelterm'][6].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
								$this->sendmessage($u6["User_OpenID"],$text);
                                                                if($dis_level == 6) return;
								$u7 = $this->getownerinfo($u6["Owner_Id"]);
								if(is_array($u7)){
									$bonus = $this->get_bonus($orderid, $u6["Owner_Id"]);
									$text = "您推荐的".$show_word['childlevelterm'][7].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
									$this->sendmessage($u7["User_OpenID"],$text);
                                                                        if($dis_level == 7) return;
									$u8 = $this->getownerinfo($u7["Owner_Id"]);
									if(is_array($u8)){
										$bonus = $this->get_bonus($orderid, $u7["Owner_Id"]);
										$text = "您推荐的".$show_word['childlevelterm'][8].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
										$this->sendmessage($u8["User_OpenID"],$text);
                                                                                if($dis_level == 8) return;
										$u9 = $this->getownerinfo($u8["Owner_Id"]);
										if(is_array($u9)){
											$bonus = $this->get_bonus($orderid, $u8["Owner_Id"]);
											$text = "您推荐的".$show_word['childlevelterm'][9].$name."下单成功，支付了".$money."元，您将获取".$show_word['catcommission'].$bonus."元";
											$this->sendmessage($u9["User_OpenID"],$text);
										}
									}
								}
	...
	public function sendorder_distribute($money,$orderid){//购买分销商佣金提醒
		$rsConfig = $this->db->GetRs("distribute_config","Index_Professional_Json","where Users_ID='".$this->usersid."'");
		$show_word = json_decode($rsConfig['Index_Professional_Json'],true);
		if (empty($show_word)) {
			$show_word = array(1=>'一级会员',2=>'二级会员',3=>'三级会员',4=>'四级会员',5=>'五级会员',6=>'六级会员',7=>'七级会员',8=>'八级会员',9=>'九级会员');			
		}
		...
		foreach($lists as $key=>$rr){			
			$msg = "您推荐的".(!empty($show_word['childlevelterm'][$key + 1])?$show_word['childlevelterm'][$key + 1]:"").$rsUser['User_NickName'].($rsUser['Order_Type']==0 ? '购买成为' : '升级成为').$rsUser['Level_Name']."，支付了".$money."元，您将获取".(!empty($show_word['catcommission'])?$show_word['catcommission']:'').$rr['Record_Money']."元";
