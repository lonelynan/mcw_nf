...
class Distribute
{
	...
	public function update_group_num($UserID){
		...
		$this->DB->Set('shop_distribute_account',array('Group_Num'=>$posterity_count),$condition);
	...
	public function update_group_sales($Order_ID,$Product_ID){
		...
		$rsOrderSales = $this->DB->getRs('shop_distribute_record','Buyer_ID,Owner_ID,SUM(Product_Price) as sum',$condition);
	...
	private function _get_account_list(){
		...
		//$rsAccount = $this->DB->get('shop_distribute_account',$fields,$condition);	
	...
	private function _get_dis_pro_title(){
		$rsDsConfig = $this->db->GetRs('shop_distribute_config','Pro_Title_Level',"where Users_ID='".$this->Users_ID."'");
