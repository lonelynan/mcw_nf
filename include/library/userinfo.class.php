...
class userinfo
{
	...
	function getOpenid($code){
		...
		if(!empty($data['unionid'])){
			$_SESSION[$this->UsersID."unionid"] = $data['unionid'];
		}else{
			$_SESSION[$this->UsersID."unionid"] = '';
		}
	...
	function loginbyopenid(){
		if (!empty($_SESSION[$this->UsersID."unionid"])) {
			$isuser = $this->DB->GetRs("user","*","where unionid='".$_SESSION[$this->UsersID."unionid"]."' and Users_ID='".$this->UsersID."'");
			if (!$isuser) {
				$isuser = $this->DB->GetRs("user","*","where User_OpenID='".$_SESSION[$this->UsersID."OpenID"]."' and Users_ID='".$this->UsersID."'");
				if ($isuser) {
					$this->DB->Set('user', ['unionid' => $_SESSION[$this->UsersID."unionid"]], " where User_ID = " . $isuser['User_ID']);
				}
			}
		} else {
			$isuser = $this->DB->GetRs("user","*","where User_OpenID='".$_SESSION[$this->UsersID."OpenID"]."' and Users_ID='".$this->UsersID."'");
	...
	function registerbyopenid($ownerid=0,$rootid,$code = '', $userInfo = []){
        if (!empty($code)) {
		...
		if(empty($uinfo["errcode"])){
            $user = $this->DB->GetRs("user","*","where Users_ID='".$this->UsersID."' and User_OpenID='".$openid."'");
            $unionid = isset($uinfo['unionid'])?$uinfo['unionid']:'';
            if (!empty($unionid)) {
                $user2 = $this->DB->GetRs("user","*","where Users_ID='".$this->UsersID."' and unionid='".$unionid."'");
            }
            if (!empty($user) || !empty($user2)) {

                if ($user && empty($user['unionid']) && !empty($unionid)) {
                    $this->DB->Set('user', ['unionid' => $unionid], "where User_OpenID = '". $openid ."' and Users_ID = '". $this->UsersID ."'");
                }

                if (!empty($code)) {
                    $userid = !empty($user['User_ID'])?$user['User_ID']:$user2['User_ID'];
                    $_SESSION["Users_ID"] = $this->UsersID;
                    $_SESSION[$this->UsersID."User_ID"] = $userid;
                    return true;
                } else {
                    if ($user || $user2) {
			...
			} elseif ($uinfo['nickName']) {
				$Data["User_NickName"] = $this->removeEmoji1($uinfo["nickName"]);
			}
            if(isset($uinfo["sex"])){
                $Data["User_Gender"] = $uinfo["sex"]==1 ? "男" : "女";
            } else if (isset($uinfo['gender'])) {
                $Data["User_Gender"] = $uinfo["gender"]==1 ? "男" : "女";
            }
			if($uinfo["province"]){
            ...
            if(isset($uinfo["headimgurl"])){
                $Data["User_HeadImg"] = $uinfo["headimgurl"];
            } else if (isset($uinfo['avatarUrl'])) {
                $Data["User_HeadImg"] = $uinfo["avatarUrl"];
            }
			$User_No="3";
			if($ownerid){
			...
			$Data["User_Profile"] = 0;
			$Data["User_OpenID"] = !empty($_SESSION[$this->UsersID."OpenID"]) ? $_SESSION[$this->UsersID."OpenID"] : '';
			$Data["User_OpenID_Lp"] = !empty($userInfo['openId']) ? $userInfo['openId'] : '';
