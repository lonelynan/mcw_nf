...
class weixin_token{
	...
	//获取小程序的access_token
	public function get_access_token_lp($appid, $appsecret)
	{
		$redis = new Myredis($this->db, 6);
		$key = 'newo2o_' . $this->usersid . '_access_token_lp';
		$access_token = $redis->getValue($key);
		if ($access_token) {
			return $access_token;
		} else {
			...
			if(empty($data["errcode"])) {
				$redis->setValue($key, $data['access_token']);
				$expire_flag = $redis->expiresAt($key, intval($data['expires_in']) - 60);
				if ($expire_flag) {
		...
		if($item){
			...
			if($item["access_token"] && $diff>=time()){
				...
				if(!empty($weixin_ip["errcode"])){
					...
					$this->db->Add("weixin_log",array("message"=>$weixin_ip["errmsg"],'log_CreateTime'=>time()));
				}else{
		...
		if($token == 0){
			...
			if($users["Users_WechatAppSecret"] && $users["Users_WechatAppId"] && in_array($users["Users_WechatType"],array('1','2','3'))){
				...
				if(empty($data["errcode"])){
					...
					$this->db->Add("weixin_log",array("message"=>$message,'log_CreateTime'=>time()));
