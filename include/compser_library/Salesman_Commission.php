...
class Salesman_commission {
		...
		*/
		function handout_salesman_commission($orderid,$products,$qty,$prductsid=0,$cartid=0){			
			if(is_array($orderid)){
			...
			$dis_level = 3;//分销级别
			if ($products['Products_ID'] == -1) {
						$offratio = json_decode($products['Products_Dis_and_salesman'],true);
                        $dis_ratio = isset($offratio['salesman_ratio']) ? $offratio['salesman_ratio'] : 0; //业务提成
                        $salesman_level_ratio = isset($offratio['salesman_level_ratio']) ? $offratio['salesman_level_ratio'] : 0;//业务分级提成                        
						$platForm = isset($offratio['platForm_Income_Reward']) ? $offratio['platForm_Income_Reward'] : 0; //发送比例
			}else{
             ...
             if (!empty($order->muilti)) {
				$orderprice = $cartlist[$prductsid][$cartid]['web_prie_shop'];
			 } else {
				 $orderprice = $order->Web_Price;
			 }	
			...
			foreach($psars as $k=>$v){
                ...
                $nobility_commission[$v]['Sales_Money'] = $orderprice * $platForm * $dis_ratio * $salesman_level_ratio[$t] / 1000000;
