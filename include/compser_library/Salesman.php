...
class Salesman {
		...
		*/
		function up_salesman($type=''){
			$salesman_limit = 100;
			$withdraw_data = Capsule::table('distribute_smansq_record')->where(['Users_ID' => $this->Users_ID, 'User_ID' =>$this->User_ID ])->first();
		if(empty($withdraw_data)){
		    return '相应的申请记录不存在';
		}		
			$flag = '';
			if($withdraw_data['Record_Status'] == 1){
				$data['Is_Salesman'] = 1;
		...
		*/
		...
		function get_qrcode($qrcode){
			...
			if($flag){
				...
				$dis = Dis_Account::Multiwhere(array('Users_ID'=>$this->Users_ID,'User_ID'=>$this->User_ID))->first();
				return $dis->Qrcode;
			}
		}
