...
class OrderObserver {
	...
	public function confirmed($order){		
		...
		//获取本店分销配置
		if($this->shop_config['Dis_Agent_Type'] != 0 && $order['Order_Type'] != 'offline_st' && $order['Order_Type'] != 'offline_qrcode' && $order['Order_Type'] != 'kanjia'){
	...
	private function handle_user_info() {
		...
		if($user){
			...
			if($this->shop_config ['Distribute_Type']==1){//消费金额
					...
					$LevelID = get_user_distribute_level_confirmcost($level_data,$this->shop_config,$order ['Order_TotalPrice'], $user->User_ID);
					if($LevelID >= 1){
				...
				foreach ($v as $p => $productInfo) 
				{
					...
					foreach ($area_agents as $key=>$agent_area) 
					{
						...
						if($agent_area->type == 1){
							...
							if($province_rate > 0 ){
								//计算当前产品的佣金
								$product_record_money = ($orderweb_price*($productInfo['platForm_Income_Reward']/100))*($productInfo['area_Proxy_Reward']/100)*($province_rate/100);
						...
						if($agent_area->type == 2){
							...
							if($city_rate > 0 ){
								//计算当前产品的佣金
								$product_record_money = ($orderweb_price*($productInfo['platForm_Income_Reward']/100))*($productInfo['area_Proxy_Reward']/100)*($city_rate/100);
								if (isset($back_qty_arr) && isset($back_qty_arr[$k][$p])) {
									$product_record_money = $product_record_money / ($productInfo['Qty'] + $back_qty_arr[$k][$p]) * $productInfo['Qty'];
								}
						...
						if($agent_area->type == 3){
							...
							if($area_agent_rate > 0 ){
								//计算当前产品的佣金
								$product_record_money = ($orderweb_price*($productInfo['platForm_Income_Reward']/100))*($productInfo['area_Proxy_Reward']/100)*($area_agent_rate/100);
								if (isset($back_qty_arr) && isset($back_qty_arr[$k][$p])) {
									$product_record_money = $product_record_money / ($productInfo['Qty'] + $back_qty_arr[$k][$p]) * $productInfo['Qty'];
								}
    ...
    private function handle_dis_sha_info()
	{
            ...
            $order = $this->order;
            //获取各级股东和各级股东人数
	    $row = $DB1->Get('distribute_account', 'Account_ID,Real_Name,Account_Mobile,sha_level', ' where `Users_ID`="' .$order->Users_ID. '" AND `Enable_Agent`=1');
			$Sha_Rate_Objarr = $DB1->toArray($row);
            $Sha_Rate_Count = array();
            $SHA_Account_Id_List = array();
            $SHA_Account_Id_List_Sou = array();			
			$SHA_Account_Idarr = sha_nofenhnum($order['Users_ID'], $this->shop_config['Sha_Money']);
			if (count($SHA_Account_Idarr['cur']) <= 0) { return true; }
	    foreach ($Sha_Rate_Objarr as $key => $acc) {	
			$Sha_Rate_Count[$acc['sha_level']]['data'][] = $acc;			
		}
		
			foreach ($SHA_Account_Idarr['cur'] as $keysha => $accsha) {
			$SHA_Account_Id_List[$accsha['sha_level']][] = $accsha['Account_ID'];
		}
		
		foreach ($SHA_Account_Idarr['souce'] as $keyshasou => $accshasou) {
			$SHA_Account_Id_List_Sou[$accshasou['sha_level']][] = $accshasou['Account_ID'];
		}
			...
			foreach ($SHA_Account_Id_List_Sou as $ksou => $vsou) {                    
                    $Sha_Rate_Count[$ksou]['count'] = count($vsou);
            }
	    ...
	    if (!empty($cartProduct)) 
	    {
	    	foreach ($cartProduct as $k => $v) 
		    {
		    	foreach ($v as $p => $productInfo) 
		    	{
		    		...
		    		{				
						if ($Order_type == 'offline_st' || $Order_type == 'offline_qrcode') {
										$productInfo['Products_Integration'] = $productInfo['ProductsWebTotal'];
									} else {
										if (!empty($order->muilti)) {
										$productInfo['Products_Integration'] = $productInfo['web_prie_shop'];
										} else {
										$productInfo['Products_Integration'] = $order->Web_Price;
										}
									}									
									
                                    //当前商品的股东分红佣金
                                    $sha_record_monery = ($productInfo['Products_Integration']*($productInfo['platForm_Income_Reward']/100)*($productInfo['sha_Reward']/100));
                                    //遍历出各个级别对应的的股东
                                    foreach ($Sha_Rate_Count as $ks => $vs) {
                                        if(!empty($vs['data'])){
											...
											if (isset($back_qty_arr) && isset($back_qty_arr[$k][$p])) {
												$sha_record_monery_level = $sha_record_monery_level / ($productInfo['Qty'] + $back_qty_arr[$k][$p]) * $productInfo['Qty'];
											}
                                           foreach ($vs['data'] as $key => $value) {
												if (isset($SHA_Account_Id_List[$ks])) {
													if (strpos($SHA_Account_Id_List[$ks], $value['Account_ID']) !== false) {
										...
										if($order["Order_Type"]=="offline" || $order["Order_Type"]=="offline_qrcode"){
											$productInfo['ProductsName'] = "线下扫码支付";
											$productInfo['ProductsPriceX'] = $order["Order_TotalPrice"];
										}
										if($order["Order_Type"]=="offline_st"){
											$productInfo['ProductsName'] = "线下实体买单";
											$productInfo['ProductsPriceX'] = $order["Order_TotalPrice"];
										}
										if (!isset($sha_record_monery_level)) { return true; }
										if (!empty($shanum)) {
										if (isset($SHA_Account_Id_List[$ks])) {
                                        $flag_b = $this->do_sha_award_record($accountinfo, $sha_record_monery_level, $productInfo, $shanum, $SHA_Account_Id_List[$ks],$Sha_Rate['sha'][$ks]['name']);
										} else {
										$flag_b = true;
										}
										} else {
	...
	private function do_sha_award_record($accountinfo, $sha_record_monery, $productInfo, $sha_count, $SHA_Account_Id_List, $Sha_Rate_name)
	{
		global $DB1;
		...
		$dis_sha_data['Sha_level_name'] = $Sha_Rate_name;
