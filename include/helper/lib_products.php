<?php
  ...
  *产品的帮助函数
  */
 function isCoupon($BizID){
     global $DB;
     $rsCoupon = array();
     $DB->Get("user_coupon","Coupon_EndTime","where Is_Delete=0 and Biz_ID=".$BizID);
...
function build_attr_html($type_id, $product_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= $val['Attr_Name']."</td><td><input type='hidden' name='attr_id_list[". $val['Attr_ID'] ."]' value='".$val['Attr_Name']."' />";
        ...
        {           
            ...
            foreach ($attr_values AS $k => $opt)
            {
                ...
                $str = '<input style="height: 13px;" type="checkbox" name="'. $val['Attr_ID'] .'_pro[]" value="'. $k .'" class="pro_click" data-pro="'. $opt .'" />' . '<span style="padding: 0 5px;">' . $opt . '</span>&nbsp;&nbsp;';
                $html   .= $str;
            }            
        }
        $html .= '</td></tr>';
