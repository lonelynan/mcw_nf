...
function getOwner($DB, $UsersID) {
	...
	$rsConfig = $DB->GetRs("shop_config", "ShopName,ShopLogo", "where Users_ID='" . $UsersID . "'");
...
/*--20150521add--start--*/
if (!function_exists('getownerbysql')) {
	//通过url获得ownerid
...
function handle_distribute_success($DB, $Order_ID,$ProductID,$UsersID) {
	...
	foreach ($account_list as $key => $item) {
		...
		$dsAccount = $DB->GetRs('distribute_account', 'balance', "where Users_ID='" . $UsersID . "' and User_ID=" . $item['User_ID']);
		add_distribute_balance_record($UsersID, [
							'User_ID' => $item['User_ID'],							
							'Dis_Balance' => $dsAccount["balance"],
							'Record_Money' => $interest,
							'Record_Desc' => '分销账户增加提现',
							'Record_Type' => 1
						]);
	}
 ...
 *
 * */
function generate_posterepop($img, $UsersID, $owner_id) {
	...
	$data = base64_decode($img);
	$file_name = '';
	$file_path = UPLOAD_DIR . $UsersID . $owner_id . 'pop.png';
