 ...
 *获取快递公司列表
 */
function get_shipping_company($UsersID,$status = null){
    global $DB1; 
...
function handle_template_post_data($UsersID,$input){
...
function organize_template_content($template_content){
	...
	if(empty($template_content)) return array();
 ...
 */
function get_front_shiping_company_dropdown($UsersID,$rsConfig){
	
	if(empty($rsConfig['Shipping'])){
		return false;
	}else{
		$shipping_config = json_decode($rsConfig['Shipping'],TRUE);
		$shipping_ids = array_keys($shipping_config);
		$ids_string = implode(',',$shipping_ids);
		global $DB1;
		$condition = "where Users_ID='".$UsersID."' and Shipping_ID in (".$ids_string.")";
		...
		foreach($shipping_list as $key=>$item){
			$shipping_company_dropdown[$item['Shipping_ID']] = $item['Shipping_Name'];
		}
		
		return $shipping_company_dropdown;
	}
}