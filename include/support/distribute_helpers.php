...
if (!function_exists('get_owner_by_sql')) {
	...
	function get_owner_by_sql($UsersID) {
		...
		if ($user['Is_Distribute'] == 1) {
			...
			if (isset($ownerAccount['Is_Audit']) && $ownerAccount['Is_Audit'] != 1) {
...
if (!function_exists('get_dsaccount_by_id')) {
	function get_dsaccount_by_id($UsersID, $User_ID) {
		if($User_ID==0){
			...
			$where = ['Users_ID' => $UsersID, 'User_ID' => intval($User_ID)];
...
if(!function_exists('is_distribute_action')){
	
	/**
	 *判断此次购买是否为分销行为
	 */
	function is_distribute_action($BuyderID, $disself_bonus=0){
	   
		$user = User::find($BuyderID,['Owner_Id', 'Is_Distribute']);
		//购买者有介绍人是分销行为，反之不是
	
		if ($user->Owner_Id != 0) {
			$res = true;
		} elseif ($user->Is_Distribute == 1 && $disself_bonus == 1) {
			$res = true;
		} else {
			$res = false;
		}
...
if (!function_exists('get_offline_product_distribute_info')) {
	/**
	 *获取线下付款（虚拟）产品的分销信息
	 */
	function get_offline_product_distribute_info($UsersID, $BizID, $TotalPrice, $isQrcode = 0) {
		global $rsConfig, $DB;
		$rsBiz = $DB->GetRs('biz','Biz_Name,Biz_Profit',"where Users_ID='".$UsersID."' and Biz_ID=".$BizID);
		
		if(empty($rsConfig['Shop_oofCommision_Reward_Json'])){
			$rsConfig['Shop_oofCommision_Reward_Json'] = '["0","0","0","0"]';
		}
		$product = array(
		    'Products_ID' => -1,		    
			'Products_Dis_and_salesman' => $rsConfig['Shop_oofCommision_Reward_Json'],
			'Products_Profit' => $TotalPrice * ($rsBiz['Biz_Profit']/100),
			'Products_Name' => $rsBiz['Biz_Name'].($isQrcode ? '线下扫码消费' : '实体店消费'),
			'Users_ID' => $UsersID,
			'Biz_ID' => $BizID,
			'Products_PriceX' => $TotalPrice,
			'Products_Supplyprice' => $TotalPrice*((100-$rsBiz['Biz_Profit'])/100),
		);
		
		//获取此产品用于分销的总奖励钱数$seperation
		$profit = $product['Products_Profit'];
		$seperation_config = json_decode($product['Products_Dis_and_salesman'], TRUE);
		$platForm_Income_Reward = isset($seperation_config['platForm_Income_Reward']) ? $seperation_config['platForm_Income_Reward'] : 0;
		$dis_seperation_rate = isset($seperation_config['commission_Reward']) ? $seperation_config['commission_Reward'] : 0;
	    $seperation = $profit*($platForm_Income_Reward/100)*($dis_seperation_rate/100);
		

		$Products_Distributes = isset($seperation_config['Distribute']) ? $seperation_config['Distribute'] : 0;
		$distribute_bonus_list = array();
		
		if ($Products_Distributes != 0) {
		if (count($Products_Distributes) > 0) {
			foreach ($Products_Distributes as $Key => $item) {
				foreach ($item as $K => $v) {
				$distribute_bonus_list[$Key][$K] = $seperation*($v / 100);
				}
			}
		}
		}
	...
	function get_my_leiji_sales($UsersID, $UserID, $posterity) {
		...
		*/		
		...
		global $DB1;
			...
			$r2 = $DB1->GetRs('user_order','SUM(Order_TotalPrice) as money','where Owner_ID in('.implode(',',$posterityids).') and Order_Status>=2');
			$r2b = $DB1->GetRs('user_order','SUM(Back_Amount) as moneyb','where Owner_ID in('.implode(',',$posterityids).') and Is_Backup=1');
			if (!empty($r2['money'])) {
				$total_sales = $total_sales + $r2['money'] - $r2b['moneyb'];
			}
		$r3 = $DB1->GetRs('distribute_order','SUM(Order_TotalPrice) as money','where Owner_ID in('.implode(',',$posterityids).') and Order_Status>=2');
		if(!empty($r3['money'])){
			$total_sales = $total_sales + $r3['money'];
		}
...
if (!function_exists('add_distribute_record')) {
	//增加分销记录
	function add_distribute_record($UsersID, $OwnerID, $Product_Price, $ProductID, $Qty, $OrderID, $Profit, $CartID = 0, $User_ID = 0) {
		
		//此产品利润为零,不处理
		global $DB1;
		$orderInfo = $DB1->getRs('user_order','Order_ID,Web_Price,muilti,Order_CartList','WHERE Order_ID='.$OrderID);
		$Order_CartList = $orderInfo['Order_CartList'] ? json_decode($orderInfo['Order_CartList'], true) : array();
		if (!empty($orderInfo['muilti'])) {
			if ($Order_CartList[$ProductID][$CartID]['web_prie_shop'] <= 0) {
				$_SESSION['yformat'] = '';
				return false;
			}
			$Profitnew = $Order_CartList[$ProductID][$CartID]['web_prie_shop'];
		} else {
			if ($orderInfo['Web_Price'] <= 0) {
				$_SESSION['yformat'] = '';
				return false;
		}
		$Profitnew = $orderInfo['Web_Price'];
		}
		...
		$Product["Products_Profit"] = $orderInfo['Web_Price'];
		//佣金比例数组
		...
		if(!empty($Products_Distributes)){
			foreach ($Products_Distributes as $Key => $item) {
				foreach($item as $k=>$v){
					$distribute_bonus_list[$Key][$k] = $Profitnew * $Product['commission_ratio'] * $Product['platForm_Income_Reward'] * $v / 1000000;
				}
			}
		}
		...
		$dis_record->Buyer_ID = $User_ID ? $User_ID : (!empty($_SESSION[$UsersID . 'User_ID'])?$_SESSION[$UsersID . 'User_ID']:0);
		...
		DisRecordObserver::$Web_Price = $Profitnew;
		$dis_record->save();
	}
}

if (!function_exists('add_offline_distribute_record')) {
	//增加分销记录
	function add_offline_distribute_record($UsersID, $OwnerID, $Product_Price, $BizID, $Qty, $OrderID, $CartID,
                                           $User_ID = 0, $ProductID = 0) {
		$Product = get_offline_product_distribute_info($UsersID, $BizID, $Product_Price);
		...
		$dis_record = new Dis_Record();
		$dis_record->Users_ID = $UsersID;
		$dis_record->Buyer_ID = $User_ID ? $User_ID : isset($_SESSION[$UsersID . 'User_ID'])?$_SESSION[$UsersID . 'User_ID'] : 0;
		$dis_record->Owner_ID = $OwnerID;
		$dis_record->Order_ID = $OrderID;
		$dis_record->Biz_ID = $Product['Biz_ID'];
		$dis_record->Product_Price = $Product_Price;
...
if (!function_exists('get_product_distribute_info')) {
	 ...
	 *获取指定产品的分析信息
	 */
	function get_product_distribute_info($UsersID, $ProductID) {//获取产品佣金比例数组

		$fields = ['Products_ID', 'Products_Distributes', 'Products_Profit', 'Products_Name',
   		           'Products_PriceX','commission_ratio', 'nobi_ratio'];
		$product = Product::Multiwhere(array('Users_ID' => $UsersID, 'Products_ID' => $ProductID))
			->first($fields)
			->toArray();
		...
		if(!empty($Products_Distributes)){
			foreach ($Products_Distributes as $Key => $item) {
				foreach($item as $k=>$v){
					$distribute_bonus_list[$Key][$k] = $product['Products_Profit'] * $product['commission_ratio'] * $v / 1000000;
				}
			}
		}
...
if (!function_exists('add_distribute_record_pifa')) { 
    //增加分销记录 
    function add_distribute_record_pifa($UsersID, $OwnerID, $Product_Price, $ProductID, $Qty, $OrderID, $Product) { 
		global $DB1;
        //此产品利润为零,不处理 
        if ($Product['Products_Profit'] == 0) {
            return false; 
		...
		$dis_level = get_dis_level($DB1,$UsersID);
		
		$Products_Distributes = json_decode($Product['Products_Distributes'], true);
        $distribute_bonus_list = array();
		if(!empty($Products_Distributes)){
			foreach ($Products_Distributes as $Key => $item) {
				foreach($dis_level as $k=>$v){
					$distribute_bonus_list[$k][$Key] = $Product['Products_Profit'] * $Product['commission_ratio'] * $item / 1000000;
				}
			}
		}
		...
		$Product['nobi_ratio'] = 100-$Product['commission_ratio'];
		...
		$Product['Products_Price'] = $Product['Products_PriceX'] = $Product_Price;
...
if (!function_exists('add_distribute_record_bianmin')) { 
    //增加分销记录 
    function add_distribute_record_bianmin($UsersID, $OwnerID, $Product_Price, $ProductID, $Qty, $OrderID, $res) { 
		global $DB1;
		
		$Product = array(
			'Products_Profit'=>$res['Server_Profit'],
			'Products_Distributes'=>$res['Server_Distributes'],
			'commission_ratio'=>$res['commission_ratio'],
			'nobi_ratio'=>$res['nobi_ratio'],
...
if(!function_exists('deal_distribute_fanben')){//返本处理
	function deal_distribute_fanben($Fanben, $UserID){
		...
		$rsAccount = $DB1->GetRs("distribute_account","Account_ID,Fanxian_Remainder,Fanxian_Count","where User_ID=".$UserID);
		...
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/data/fanben.log',"到了判断外".(print_r($rsAccount, true))."\n",FILE_APPEND);
		if($rsAccount["Fanxian_Remainder"]==1){//返现处理		
			...
			// );
			// $DB1->Set("user",$Data,"where User_ID=".$UserID);
			
			$user_balance =  $DB1->GetRs("distribute_account","balance","where User_ID=".$UserID." and Users_ID = '".$rsUser["Users_ID"]."'");	 
			$Data = array(
				"balance"=> $user_balance['balance'] + $Fanben[1]
			);
			$DB1->Set("distribute_account",$Data,"where User_ID=".$UserID." and Users_ID = '".$rsUser["Users_ID"]."'");
			file_put_contents($_SERVER['DOCUMENT_ROOT'].'/data/fanben.log',"开始返现{$Data['balance']}\n",FILE_APPEND);
			//更新资金流水
			// $Data=array(
				...
				// 'Note'=>"您发展直接下属达到".$Fanben[0]."个，系统返现".$Fanben[1]."元",
				// 'CreateTime'=>time()	
			// );
			// $DB1->Add('user_money_record',$Data);
		}
		$flag = $DB1->Set("distribute_account",$account,"where Account_ID=".$rsAccount["Account_ID"]);
		if($flag){
		    $Data = array(
			    'Users_ID'=>$rsUser['Users_ID'],
				'User_ID'=>$UserID,
			...
			$weixin_message = new weixin_message($DB1,$rsUser['Users_ID'],$UserID);
			$weixin_message->sendscorenotice("您发展直接下属达到".$Fanben[0]."个，系统返现".$Fanben[1]."元");
		}
		return true;
	}
}
...
if (!function_exists('get_filter_cart_list')) {
	 ...
	 */
	function get_filter_cart_list($DB, $cartlist, $orderinfo = []){
		$BizList = array();
		...
		foreach($cartlist as $Biz_ID => $Biz_CartList){//第一次循环 按商家
			...
			foreach($Biz_CartList as $Products_ID => $Products){//第二次循环 按商品
				...
				foreach($Products as $Cart_ID => $Cart_Info){	
					if(!isset($Cart_Info["Property"])) continue;
					if (count($Cart_Info["Property"]) > 0) {						
						$centprice = $Cart_Info["Property"]['shu_pricesimp'];
						$centpriceg = $Cart_Info["Property"]['shu_priceg'];						
					}else{
						$centprice = $Cart_Info["ProductsPriceX"];
						$centpriceg = $Cart_Info["Products_PriceS"];						
					}
					if(!empty($orderinfo['Integral_Money'])){
						$centprice -= $orderinfo['Integral_Money'];
					}
					if(!empty($orderinfo['Coupon_Cash'])){
						$centprice -= $orderinfo['Coupon_Cash'];
					}
					$centendprice = $centprice - $centpriceg;
					$centendprice = $centprice - $centpriceg;
					if($finance_type_biz==0){//商家统一使用比例结算					
						$cartlist[$Biz_ID][$Products_ID][$Cart_ID]["ProductsProfit"] = $centprice * $BizList[$Biz_ID]["Finance_Rate"] / 100;
					}else{
						if($finance_type_products==0){//产品使用比例结算							
							$cartlist[$Biz_ID][$Products_ID][$Cart_ID]["ProductsProfit"] = $centprice * $rsProducts["Products_FinanceRate"] / 100;
						}else{//产品供货价结算
							$cartlist[$Biz_ID][$Products_ID][$Cart_ID]["ProductsProfit"] = $centendprice;
						}
					}
				}
			}
...
if (!function_exists('get_products_supply_price')) {//获取供货价
	function get_products_supply_price($DB,$ProductsID,$ProductsPriceS,$spec_list){
		if(empty($spec_list)){
			...
			$DB->Get("shop_products_attr","Supply_Price","where Products_ID=".$ProductsID." and Attr_ID in(".$spec_list.")");
			while($r = $DB->fetch_assoc()){
...
if(!function_exists('deal_user_to_distribute')){
	function deal_user_to_distribute($UsersID,$shop_config,$user_data,$level_data){
		...
		if(empty($user_data)){//用户不存在，返回设置值			
            ...
            $LevelID = get_user_distribute_level($shop_config,$level_data,$user_data['User_ID'],$UsersID);//获得该用户应得的分销级别
			if($user_data['Is_Distribute']>0){//用户已经是分销商，更改分销级别				
				...
				if(empty($dis_account->Account_ID)){//某种原因分销商账号不存在
					...
					if($shop_config['Distribute_Type'] == 1 && $LevelID > 0){
						$dis_account->Level_ID = $LevelID;
						$dis_account->save();
					}
					return true;
...
if(!function_exists('get_user_distribute_level')){//根据消费额获得分销商级别（总消费额），用于生成分销商或更新
	function get_user_distribute_level($shop_config,$level_data,$UserID,$UsersID,$orderid = 0){
        global $DB1;
        $first_level_data = reset($level_data);
        if ($first_level_data['Level_LimitType']==4) {
            $manualorder = $DB1->GetRs("dismanual_order","Order_Status","where Users_ID='".$UsersID."' and User_ID=".$UserID);
            $LevelID = $manualorder['Order_Status']==1 ? $first_level_data['Level_ID'] : 0;
        } else {
		...
		if($shop_config['Distribute_Type']==1){//消费额门槛
			...
			if(!empty($level_result)){
				...
				$rTotalPrice = 0;
				if (!empty($orderid)) {
					$rTotalPricearr = $DB1->GetRs('user_order','Order_TotalPrice','where Order_ID='.$orderid);
					$rTotalPrice = $rTotalPricearr['Order_TotalPrice'];
				}
				$r = $DB1->GetRs('user_order','SUM(Order_TotalPrice) as money','where User_ID='.$UserID.' and (Order_Status=2 or Order_Status=3 or Order_Status=4) and Is_Backup !=1');
				...
				/*if (!$yesuplevel&&$first_level_data['Level_LimitType'] != 3) {
					$DB1->Set("distribute_account",array('Is_Delete' => 1),"where User_ID=".$UserID);
				}*/
			}
		}
		return $LevelID;
...
if(!function_exists('get_user_distribute_level_cost')){//根据消费额获得分销商级别（一次性消费，状态为已付款计入）
	function get_user_distribute_level_cost($level_data,$shop_config,$cost, $User_ID = 0){
        global $DB1;
		$first_level_data = reset($level_data);
		if ($first_level_data['Level_LimitType']==4) { //如果第一级分销商是手动申请的话
		    $UID = isset($_SESSION[$shop_config['Users_ID'] . 'User_ID'])?$_SESSION[$shop_config['Users_ID'] . 'User_ID']:$User_ID;
			$manualorder = $DB1->GetRs("dismanual_order","Order_Status","where Users_ID='".$shop_config['Users_ID']."' and User_ID=".$UID);
			$LevelID = $manualorder['Order_Status']==1 ? $first_level_data['Level_ID'] : 0;
		}else{  //如果是无门槛的话,levelid等于这个无门槛的levelid
			$LevelID = $first_level_data['Level_LimitType']==3 ? $first_level_data['Level_ID'] : 0;
		}
		if($shop_config['Distribute_Type']==1){//消费额门槛
			$level_result = array();
...
if(!function_exists('get_user_distribute_level_buy')){//根据购买获得分销商级别
	function get_user_distribute_level_buy($level_data,$shop_config,$productids, $User_ID = 0){
        global $DB1;
        $first_level_data = reset($level_data);
        if ($first_level_data['Level_LimitType']==4) { //如果第一级分销商是手动申请的话
            $UID = isset($_SESSION[$shop_config['Users_ID'] . 'User_ID'])?$_SESSION[$shop_config['Users_ID'] . 'User_ID']:$User_ID;
...
if(!function_exists('get_user_distribute_level_confirmcost')){//根据消费额获得分销商级别（一次性消费，状态为已完成计入）
	function get_user_distribute_level_confirmcost($level_data,$shop_config,$cost, $User_ID = 0){
        global $DB1;
        $first_level_data = reset($level_data);
        if ($first_level_data['Level_LimitType']==4) { //如果第一级分销商是手动申请的话
            $UID = isset($_SESSION[$shop_config['Users_ID'] . 'User_ID'])?$_SESSION[$shop_config['Users_ID'] . 'User_ID']:$User_ID;
