...
if(!function_exists('get_dominfo')){//获取系统模块
	function get_dominfo($DB, $UsersID){		
		$dominfo = array();
		$DB->Get("users","domenable,domname","where Users_ID='".$UsersID."'");
		while($r = $DB->fetch_assoc()){
			$dominfo = $r;	
		}
		return $dominfo;
...
if(!function_exists('get_sys_material')){//获取图文消息
	function get_sys_material($DB, $UsersID, $type){//type为0时自定义图文，为1时系统图文
		...
		while($r = $DB->fetch_assoc()){
			...
			$r["Title"] = empty($json['Title']) ? (empty($json[0]['Title']) ? '' : $json[0]['Title']) : $json['Title'];			
			$material[] = $r;
		}
		return $material;
	}
