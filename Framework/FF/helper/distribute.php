...
if (!function_exists('add_distribute_record')) {
	//增加分销记录
	function add_distribute_record($UsersID, $OwnerID, $Product_Price, $ProductID, $Qty, $OrderID, $Profit, $CartID, $Productsattrstrval, $Property) {
		//此产品利润为零,不处理
		if ($Profit <= 0) {
			return false;
		}
		...
		$Product['skustr'] = $Productsattrstrval;
		$Product['skukey'] = $Property['shu_vid'];
...
if (!function_exists('get_my_leiji_income')) {
	...
	function get_my_leiji_income($UsersID, $UserID) {        
		$record_list = model()->query('select SUM(Record_Money) as money from distribute_account_record where User_ID='.$UserID.' and Record_Type=0 and Record_Status>=1');
		$total_income = 0;
		$total_income = floor($record_list[0]['money'] * 100) / 100;		
		return $total_income;
	}
}
...
if(!function_exists('sha_nofenhnum')){//获取当前不在参与分红的股东数
	function sha_nofenhnum($UsersID, $UserID, $Sha_Money){	
		global $DB1;
		$condition = "where Users_ID='".$UsersID."'";
		$condition .= " and Enable_Agent = 1 order by User_ID desc";
		  $DB1->get("distribute_account","User_ID,Account_ID,Real_Name,Enable_Agent",$condition);		  
		  /*获取订单列表牵扯到的分销商*/
		  $sha_list = array();		 
		  $accid = array();		 
