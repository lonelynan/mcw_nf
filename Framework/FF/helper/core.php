...
/**
 *  创建model中的数据库操作对象
 *  @param	string	$name	类名或表名
 *  @param	string	$module	 模块名,访问其他模块的model
 *  @return	object	数据库连接对象
 */
function model($name = null, $module = ''){
	global $host,$user,$pass,$data;
	$dbconfig = ['DB_HOST'=>$host,'DB_USER'=>$user,'DB_PASS'=>$pass,'DB_NAME'=>$data,'TABPREFIX'=>''];
	if($module == '') {
		$module = \base\dispatcherUrl::getModule();
	}
	if(is_null($name)) {
		$db = new \base\model($dbconfig);
	}else {
		$model_file = APP_PATH . '/' . $module . '/model/' . $name . '.php';
		if(file_exists($model_file)) {
			$class = '\\' . $module . '\\model\\' . $name;
			$db = new $class($dbconfig);
		}else {
			$db = new \base\model($dbconfig);
		}
		$db->path = APP_PATH . '/runtime/data';
		//如果表结构不存在，则获取表结构
		$db->setTable($name);
	}
	return $db;
 ...
 * @return string
 */
function _url($url = '', $vars = '', $suffix = false, $domain = false) {
    // 解析URL
    $info = parse_url($url);
    $url = !empty($info['path']) ? $info['path'] : \base\dispatcherUrl::getAction();
    ...
    if(isset($host)) {
        ...
        if(defined('APP_SUB_DOMAIN_DEPLOY')) { // 开启子域名部署
    ...
    if($url) {
        if(0 === strpos($url, '/')) {// 定义路由
            ...
            $path = explode('/', $url);
            $var = array();
            $varModule = \base\VAR_MODULE;
            $varController = \base\VAR_CONTROLLER;
            $varAction = \base\VAR_ACTION;
            $var[$varAction] = !empty($path) ? array_pop($path) : \base\dispatcherUrl::getAction();
            $var[$varController] = !empty($path) ? array_pop($path) : \base\dispatcherUrl::getController();
            $module = '';
            
            if(!empty($path)) {
                $var[$varModule] = implode('/', $path);
            }else {
                $var[$varModule] = \base\dispatcherUrl::getModule();
            }
    ...
    if(isset($route)) {
        $url = rtrim($_SERVER['SCRIPT_NAME'], '/') . '/' . rtrim($url, '/');
    }else {
        $url = rtrim($_SERVER['SCRIPT_NAME'], '/') . '/' . ($module ? $module . '/' : '') . implode('/', array_reverse($var));
    }
