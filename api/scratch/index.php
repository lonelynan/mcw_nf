...
$_SESSION[$UsersID."HTTP_REFERER"] = "/api/{$UsersID}/scratch/";
...
$base_url = base_url();
$go_url = $base_url.'/api/'.$UsersID.'/scratch/';
$rsConfig=$DB->GetRs("scratch_config","*","where Users_ID='".$UsersID."'");
//产品剩余
$PrizeCount = array(
                            0,
                            0,
        ...
        switch ($action) {
			...
			if($rsConfig['SendSms']){				
				...
				if (empty($rsSN['SN_Code'])) {
					...
					if($rsConfig['SendSms']){
					$msglevel=array('谢谢参与','一等奖','二等奖','三等奖');
					$user_phone = $_POST['MobilePhone'];
					$send_code = $rsSN['SN_Code'];
					$send_msg = '恭喜您参加活动成功，SN码为：'.$send_code;
					}
                        $Data = array(
                        ...
                        if ($Flag) {
							if($rsConfig['SendSms']){
							send_sms($user_phone, $send_msg, $UsersID);
							}
                            $Data = array(
                                "status" => 1,
								"url"=>$go_url
...
if ($rsScratch["Scratch_If_Share"]) {
    if (isset($_POST["action_do"]) && $_POST["action_do"] == 'share') {
        ...
        if (floor($Share_get_num["count"] / $rsScratch["Scratch_Share_num"]) > 0) {
			if(floor($Share_get_num["count"] / $rsScratch["Scratch_Share_num"]) >= 1){
				$num_date = array(
					"AllDayLotteryTimes_have" => floor($Share_get_num["count"] / $rsScratch["Scratch_Share_num"]) - $use_num["count"] +1,
				);
			}else{
				$num_date = array(
					"AllDayLotteryTimes_have" => floor($Share_get_num["count"] / $rsScratch["Scratch_Share_num"]) - $use_num["count"],
				);
