...
if($action == 'clear_session'){
	...
	$CartList[$rsProducts["Biz_ID"]][$_POST['Product_ID']][]=array(
	...
	$member_activity = $DB->GetRs('kanjia_member','Cur_Price',$condition);
	...
	if($Flag){		
		...
		if($rsConfig["CheckOrder"]==1){
			$url = "/api/".$UsersID."/user/payment/".$neworderid."/".$_POST["Product_ID"]."/";
		}else{
			$url = "/api/".$UsersID."/user/kanjia_order/status/0/";
		}
