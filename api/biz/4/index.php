...
$cate = $DB->GetAssoc('biz_category','*','where Biz_ID='.$BizID.' and Category_ParentID=0');
...
$(document).ready(index_obj.index_init);
 ...
 <div id="shop_skin_index">
    ...
    <div class="fenlei_list" style="width: 100%;margin: 0 auto;overflow: hidden;background: #fff;">
        <?php if(!empty($cate)){?>
        <ul>
            <?php foreach($cate as $k => $v) {
                if($k > 3) break;?>
                <li style="float: left; width: 25%;line-height: 22px;text-align: center;padding: 16px 0 12px;">
                    <a href="<?php echo $biz_url;?>products/<?php echo $v["Category_ID"];?>/"><img  style="display: block;margin: 0 auto;width: 48px;border-radius: 50%;" src="<?= $v['Category_img']?>"><?= $v['Category_Name']?></a>
                </li>
            <?php }?>
        </ul>
        <?php }?>
