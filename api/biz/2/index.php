...
$rsSkin = $DB->GetRs("biz_home","*","where Skin_ID=".$rsBiz["Skin_ID"]." and Biz_ID=".$BizID);
$Home_Json=json_decode($rsSkin['Home_Json'],true);
if (empty($Home_Json)) {
	echo '选择的模版不存在请重新选择！';exit;
}
 ...
 <div id="products">
  ...
  <?php
    foreach($pt_goods as $k => $v){
		  ...
		  $sql = "SELECT a.Users_ID,a.Active_ID,a.MaxBizCount,a.ListShowGoodsCount FROM active AS a LEFT JOIN biz_active AS b ON a
.Active_ID=b.Active_ID  WHERE a.Users_ID='{$UsersID}' AND a.Type_ID=1 AND a.starttime<={$time} AND a
.stoptime>={$time} AND a.Status = 1 AND b.Status=2 ORDER BY Active_ID DESC limit 1";
		 $result = DB::select($sql);
		 if(empty($result) || !$result){
			sendAlert("没有相关活动");
		 }
		 $ActiveID = $result[0]['Active_ID'];
		 $pintuan_url = "/api/{$UsersID}/pintuan/biz/{$BizID}/act_{$ActiveID}/";
  ?>
  <div class="item">
	<ul>
