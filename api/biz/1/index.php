...
<div id="shop_page_contents">
 ...
 <link href='/static/api/shop/skin/<?php echo $rsConfig['Biz_Skin_ID'];?>/page.css?t=<?php echo time();?>' rel='stylesheet' type='text/css' />
 <link href="/static/api/shop/skin/<?php echo $rsConfig['Biz_Skin_ID'];?>/page_media.css?t=<?php echo time();?>" type="text/css" rel="stylesheet" />
	...
	if(isset($json)){	 
	  foreach($json as $key=>$value){
        ...
        <input type="submit" class="submit" value=" " style="background:url('.$pic[0].') no-repeat left top;" />
			...
			}elseif(isset($_SESSION["Get_BizID"])){
				$BIZ_ID = $_SESSION["Get_BizID"];
			}else{
				$BIZ_ID = '';
			}
			$condition = "where Users_ID='".$UsersID."' and Biz_ID=".$BIZ_ID." and Products_SoldOut=0 and Products_Status=1";
			if(!empty($value['category'])){
				$rsCategory = $DB->Get("biz_category","Category_ID","where Users_ID='".$UsersID."' and Category_ParentID = ".$value['category']);
				$rsCategory = $DB->toArray($rsCategory);	
				$Category_ID = ''; 
				if (!empty($rsCategory)) {
				...
				$condition .= " and Products_BizCategory in (".$Category_ID.") ";
				$more_url = 'http://'.$_SERVER['HTTP_HOST'].'/api/'.$UsersID.'/biz/'.$BIZ_ID.'/products/'.$value['category'].'/';
			}else{
				$more_url = 'http://'.$_SERVER['HTTP_HOST'].'/api/'.$UsersID.'/biz/'.$BIZ_ID.'/allcate/';
			}
					...
					/*商品直接加入购物车开始*/
						...
						echo '<li>
			                ...
			                <span class="cpxq_x l">
								...
								if(!empty($v['Products_PriceA']) && $v['Products_PriceA'] != 0){
									$Products_PriceA = $v['Products_PriceA'];
								}else{
									$Products_PriceA = $v['Products_PriceX'];
								}
								if(!empty($dis2)){
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'<a style="font-size:10px; color:#fff; margin-left:5px;  background:#333; padding:0 2px; font-weight:normal;">'.$dis2.'折</a></span>';
								}else{
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'</span>';
								}
								...
								if(isset($cart2) && empty($v['skuvaljosn']) && $v['Products_IsVirtual'] == 0){
									...
									echo " <span class='r' style='margin-top:3px;'><a href='javascript:void(0)' ><img src='/static/api/shop/skin/".$rsConfig['Skin_ID']."/gwct_h.png' width='20' height='20'></a></span>";
								}	                                     
			                echo '<input type="hidden" id="spec_list_'.$v['Products_ID'].'" value="" />
					...
					/*商品直接加入购物车开始*/
						...
						echo '<li>
			               ...
			               <div class="cpxq1_x">
								...
								if(!empty($dis1)){
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'<a style="font-size:10px; color:#fff; margin-left:5px;  background:#333; padding:0 3px;">'.$dis1.'折</a></span>';
								}else{
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'</span>';
								}
								if(isset($cart1) && empty($v['skuvaljosn']) && $v['Products_IsVirtual'] == 0){
									echo " <span class='r' style='margin-top:3px;'><a href='javascript:void(0)' onclick='add_to_cart(".$v['Products_ID'].")' ><img src='/static/api/shop/skin/".$rsConfig['Skin_ID']."/gwct_x.png' width='20' height='20'></a></span>";
								}else{
									echo " <span class='r' style='margin-top:3px;'><a href='javascript:void(0)' ><img src='/static/api/shop/skin/".$rsConfig['Skin_ID']."/gwct_h.png' width='20' height='20'></a></span>";
								}             
					...
					/*商品直接加入购物车开始*/
						...
						if($k%3 == 0) {
							...
							echo '<span class="big_pr l">';
                            	...
                            	if(isset($cart1) && empty($v['skuvaljosn']) && $v['Products_IsVirtual'] == 0) {
                            		echo "<div class='name'><a href='".$v['link']."'>".$Products_Name."</a><br><span class='price l'>￥".$Products_PriceA."</span><span class='r' style='margin-top:3px; margin-right:3px'><a href='javascript:void(0)' onclick='add_to_cart(".$v['Products_ID'].")' ><img src='/static/api/shop/skin/".$rsConfig['Skin_ID']."/gwct_x.png' width='20' height='20'></a></span></div>";
                            	} else {
                            		echo "<div class='name'><a href='".$v['link']."'>".$Products_Name."</a><br><span class='price l'>￥".$Products_PriceA."</span></div>";
                            	} 
						...
						if($k%3 == 1) {
							...
							echo '<div class="small_pr">
								<a href="'.$v['link'].'">
	                          	<span class="name l"><span class="price">￥'.$Products_PriceA.'</span><br>'.$Products_Name.'</span>
			...
			if(!empty($owner['id'])){
				$more_url = 'http://'.$_SERVER['HTTP_HOST'].'/api/'.$UsersID.'/biz/'.$BIZ_ID.'/more/'.$def.'/'.$order.'/';
			}else{
				$more_url = 'http://'.$_SERVER['HTTP_HOST'].'/api/'.$UsersID.'/biz/'.$BIZ_ID.'/more/'.$def.'/'.$order.'/';
			}
			if(!empty($value['num'])){
					...
					/*商品直接加入购物车开始*/
						...
						echo '<li>
			               ...
			               <div class="cpxq1_x">
								...
								if(!empty($dis2)){
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'<a style="font-size:10px; color:#fff; margin-left:5px;  background:#333; padding:0 3px;">'.$dis2.'折</a></span>';
								}else{
									echo '<span class="jiaget_x l">￥'.$Products_PriceA.'</span>';
								}
								if(isset($cart2) && empty($v['skuvaljosn']) && $v['Products_IsVirtual'] == 0){
...
<script type='text/javascript' >
	function add_to_cart(p){
		...
		$.post(url,'ProductsID='+p+'&cattsel='+cattsel+'&Qty='+Qty+'&OwnerID='+ownerid+'&cart_key='+cart_key,
			function(data){
				if (data.status == 1) {
					alert("产品成功加入购物车!");
