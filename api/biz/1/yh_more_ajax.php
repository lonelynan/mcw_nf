<script language="javascript">
    function load_page(type, page){
		var UsersID = '<?=$UsersID?>';
		var post_url = '?UsersID='+UsersID+'&Def=<?php echo !empty($_GET['Def']) ? $_GET['Def'] : '';?>&order_by=<?php echo !empty($_GET['order_by']) ? $_GET['order_by'] : '';?>&BizID=<?=$_GET['BizID']?>';
		//首先自动加载第一页
		$.ajax({
			type:'post',
			url:post_url,
			...
			success:function(data){
				if(data['list'] != ''){
					var j = 0;
					var Products_PriceA = 0;
					$.each(data['list'],function(i){
						j++;
						v = data['list'][i];
						
						if(v["Products_PriceA"] == 0){
						...
						if(type == 1){
							$htmltmp = '<div class="item">'+
									   '<ul>'+
										...
										'<li class="name"><span>￥'+Products_PriceA+'</span></li>'+
									   '</ul>'+
									  '</div>'+
