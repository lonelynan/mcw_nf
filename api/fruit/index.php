...
$base_url = base_url();
$go_url = $base_url.'/api/'.$UsersID.'/fruit/';
...
if(isset($_POST["action"]) && $_POST["action"]!="move"){//不用判断
	...
	switch($action){
		case 'mobile':
			$rsSN=$DB->GetRs("fruit_sn","SN_Code,SN_Status,SN_ID,Fruit_PrizeID,Fruit_Prize","where Users_ID='".$UsersID."' and Open_ID='".$_SESSION[$UsersID."OpenID"]."' order by SN_CreateTime desc");
			...
			if(empty($rsSN['SN_Code'])){
				...
				if($rsSN['SN_Status']>=1){
					...
					$send_msg = '恭喜您抽中'.$msglevel[$rsSN['Fruit_PrizeID']].':'.$rsSN['Fruit_Prize'].'，SN码为：'.$send_code;
					}
					$Data=array(
...
<script type='text/javascript' src='/static/api/js/fruit.js?t=<?=time()?>'></script>
