...
if(strpos($OrderID,"PRE") !== false){
	...
	$rsOrder=$DB->GetRs("distribute_order","*","where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."' and Order_ID='".$OrderID."'");
	...
	<div id="payment">
		<div class="i-ture">
			...
			<div class="info"> 订 单 号：<?php echo $ordersn;?><br />
				...
				抱歉，余额不足<br/>
				<a href="/api/<?=$UsersID?>/distribute/" class="fc_red">返回</a> &nbsp;&nbsp;&nbsp; <a href="/api/<?=$UsersID?>/user/charge/" class="fc_red">去充值</a>
				<?php }else{ ?>
				<div class="payment_password">
					<input type="password" name="PayPassword" id="PayPassword" placeholder="请输入支付密码" value="" />
				</div>
				<?php }?>
				<?php }else{ ?>
