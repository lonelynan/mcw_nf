...
if($rsUser['Is_Distribute'] == 1){
	...
	$poster_path = $_SERVER["DOCUMENT_ROOT"].'/data/poster/'.$UsersID.$owner['id'].'pop.png';	
	$poster_web_path = '/data/poster/'.$UsersID.$owner['id'].'pop.png';	
	...
	if($need_gegerate){
		...
		//生成二维码		
		$product_url = $base_url.'api/'.$UsersID.'/shop/'.$_SESSION[$UsersID."User_ID"].'/';
...
$rsAccount['Shop_Name'] = str_replace('的店', $rsConfig['nicheng_after'], $rsAccount['Shop_Name']);
$shoplogo = strpos($rsAccount["Shop_Logo"], 'http') !== false ? $rsAccount["Shop_Logo"] : $_SERVER["DOCUMENT_ROOT"].$rsAccount["Shop_Logo"];
if (!is_file($shoplogo)) {
    $show_logo_souce = !empty($rsUser['User_HeadImg']) ? $rsUser['User_HeadImg'] : '/static/api/images/user/face.jpg';
    $show_logo_lettle = getImageUrl($show_logo_souce, 0);
    if (strpos($show_logo_lettle, 'http') === false && !is_file($_SERVER["DOCUMENT_ROOT"].$show_logo_lettle)) {
        if (strpos($show_logo_souce, 'http') === false && !is_file($_SERVER["DOCUMENT_ROOT"].$show_logo_souce)) {
            $rsAccount["Shop_Logo"] =  '/static/api/images/user/face.jpg';
        } else {
            $rsAccount["Shop_Logo"] =  $show_logo_souce;
        }
    } else {
        $rsAccount["Shop_Logo"] =  $show_logo_lettle;
    }
}
?>
<html lang="zh-cn">
    ...
    <?php if($rsAccount["status"]):?>
    	<?php if($need_gegerate):?>  
			...
			<div class="owner_info">
              <div class="content">
				<img src="<?php echo $rsAccount["Shop_Logo"];?>?t=<?=time()?>" width="100%"/>
				<p>我是<span><?=$rsAccount['Shop_Name']?></span><br />我为“<span><?=$Shop_Name?></span>”代言</p>
              </div>
			</div>
			<div class="qrcode">
			<img src="<?=$qrcode_path?>?t=<?=time() ?>" />
			</div>
		<?php else: ?>
    		<img src="<?=$poster_web_path?>?t=<?=time() ?>" width="100%"/>
		<?php endif;?>
   <?php else: ?>
        ...
        $(document).ready( function(){
            ...
            html2canvas(document.body, {  
                        ...
                        onrendered: function(canvas) {  
						    ...
						    var param = {action:'store_posterpop',dataUrl:dataUrl,owner_id:owner_id};
