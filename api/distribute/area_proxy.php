...
switch ($stepid) {
	...
	case '3':
		...
		if ($ProvinceId) { $area_id = $ProvinceId; }
		if ($CityId) { $area_id = $CityId; }
		if ($AreaId) { $area_id = $AreaId; }
		
		$resultInfo = $DB->GetRs('agent_order', 'count(*) as num', $filterCondition);
        $flag = $DB->GetRs("distribute_agent_areas","*","WHERE Users_ID='{$UsersID}' AND area_id={$area_id}");
	...
	<div class="container">
        ...
        <div class="row">
	      ...
	      <table width="100%" border="1" cellspacing="0"> 
		    <tr> 
		    	<th style="width: 16%;" class="bodytable" colspan="2">我的状态</th>		        
		    </tr>
		        ...
		        <td colspan="2">
		        	<div class="tdContent">		        		
		        		<div class="rightContent">
		        			<?php if(!empty($MY_Distribute_Level)): ?><span class="m">分销商等级：<?php echo $dis_level[$MY_Distribute_Level]['Level_Name']; ?></span><?php endif; ?>
				        	<?php if(!empty($rsAccount['Professional_Title'])): ?><span class="m">爵位等级：<?php echo $Pro_Title_Level[$rsAccount['Professional_Title']]['Name']; ?></span><?php endif; ?>
				        	<?php if(!empty($user_consue)): ?><span class="m">个人消费额：<span class="bold">&yen;<?php echo $user_consue; ?></span></span><?php endif; ?>
				        	<?php if(!empty($Sales_Group)): ?><span class="m">团队销售额：<span class="bold">&yen;<?php echo $Sales_Group; ?></span></span><?php endif; ?>
		        		</div>
		        	</div>		        	
		        </td> 
		    </tr>
