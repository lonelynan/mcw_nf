...
if (!empty($_FILES['upthumb']['error'])) {
	switch($_FILES['upthumb']['error']){
		case '1':
			$error = "上传文件大小超过" . ($max_size/1024) . "K限制。";	//'超过php.ini允许的大小。';
			break;
		case '2':
			$error = '超过表单允许的大小。';
			break;
...
/*向数据库中插入记录*/
	...
	$Account = $DB->GetRs("distribute_account", '*', "WHERE Users_ID='".$UsersID."' AND User_ID=" . (int)$_SESSION[$UsersID . 'User_ID']);
	if ($Account) {
		$Data = array(
			"Shop_Logo"=>$file_url,
			"Is_Regeposter"=>1,
...
function alert($msg) {
	global $UsersID;
	// require_once($_SERVER["DOCUMENT_ROOT"] . '/include/helper/JSON.php');
	// header('Content-type: text/html; charset=UTF-8');
	// $json = new Services_JSON();
	// echo $json->encode(array('error' => 1, 'message' => $msg));
	echo "<script>alert('" . $msg . "');location.href='/api/" . $UsersID . "/distribute/edit_headimg/';</script>";
	exit;
}