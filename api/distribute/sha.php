...
if (!empty($_POST)) 
{
	...
	if ($Flag) 
	{
       header('Location:/api/'.$UsersID.'/distribute/my_sha/?love'); exit();
	}
}
...
$roleFlag = '';
  if (!empty($Sha_Rate['sha'])) 
  {
	 ...
	 if (!empty($dis_config['Sha_People'])) {
		  $sha_people = $dis_config['Sha_People'];
		  $sha_num = $DB->GetRs('distribute_account', 'count(*) as num', ' where `Users_ID`="' .$UsersID. '" AND `Enable_Agent`=1');
	  
	  if ($sha_num['num'] >= $sha_people) {
		  $roleFlag = 'chao';
	  }
		}
...
<div class="wrap">
	<div class="container">
        ...
        <?php
			...
			<?php if($roleFlag == 'chao'): ?>
				<h1 class="tips">股东人数已满，无法申请股东了!</h1><?php exit; ?>
			<?php endif; ?>
		<div class="row">
