...
$CategoryList = array();
$DB->get("shop_category","Category_Name,Category_Img,Category_ID,Category_ParentID","where Users_ID='".$UsersID."' order by Category_ParentID asc,Category_Index asc,Category_ID asc");
...
$DB->Get("shop_category","Category_Name,Category_Img,Category_ID,Category_ParentID","where Users_ID='".$UsersID."' order by Category_ParentID asc,Category_Index asc,Category_ID asc limit 0,1 ");
while($oneCategory=$DB->fetch_assoc()){
	$oneCategorys[] =  $oneCategory;
}
// echo '<pre>';
// print_r($oneCategorys);
// exit;
//自定义分享
