...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		...
		$rsNewProducts = $DB->GetAssoc("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Count,Products_Sales","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0 and Products_Status=1 order by Products_Index asc,Products_ID desc limit $limitpage,$num");
		...
		$totalpage = ceil($total/$num);//总计页数
		$limitpage = ($p-1)*$num;//每次查询取记录
		$rsRecommendProducts = $DB->GetAssoc("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Count,Products_Sales","where Users_ID='".$UsersID."' and Products_IsRecommend=1 and Products_SoldOut=0 and Products_Status=1 order by Products_Index asc,Products_ID desc limit $limitpage,$num");
		$rsRecommendProducts = handle_product_list($rsRecommendProducts);
