...
//获取新品
$rsNewProducts = $DB->GetAssoc("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Sales","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0 and Products_Status=1 order by Products_Index asc,Products_ID desc");
...
function get_products_bycate($db,$catid){
	$new_products = array();
	$rsNewProducts = $db->GetAssoc("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_PriceY,Products_Count,Products_Sales","where Products_SoldOut=0 and Products_Status=1 and Products_IsRecommend=1 and Products_Category like '%,".$catid.",%' order by Products_Index asc,Products_ID desc limit 0,4");
	$new_products = handle_product_list($rsNewProducts);
	return $new_products;
}
?>
