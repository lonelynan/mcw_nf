	 ...
	 <div class="index-h">
	    ...
	    <?php
			...
			foreach($Category_List as $rscategory){
				//二级分类ID
				$DB->get("shop_category","Category_Name,Category_ID","where Users_ID='".$UsersID."' and Category_ParentID=".$rscategory['Category_ID']." order by Category_Index asc");
				$Category_erlist = array();
				while($rs=$DB->fetch_assoc()){
					$Category_erlist[] = $rs;
				}
				$Category_IDS = '';
				foreach($Category_erlist as $key=>$value){
				...
				$condition = "where Users_ID='".$UsersID."' and Products_SoldOut=0 and Products_Status=1 and Products_Union_ID=0";
				if (!empty($Category_IDS)) {
					$condition .= " and Products_Category in (".$Category_IDS.") order by Products_Index asc,Products_ID desc";
				} else {
					$condition .= " and Father_Category=".$rscategory['Category_ID']." order by Products_Index asc,Products_ID desc";
				}				
				...
				while($item=$DB->fetch_assoc()){
					...
					if($j==0){
						echo '<li class="big right_bor"><a href="'.$shop_url.'products/'.$item["Products_ID"].'/"><div class="name"><span class="price">￥'.($item["Products_PriceA"]==0 ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</span><br />'.$item["Products_Name"].'</div><div class="pic">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</div></a></li>';
					}elseif($j==1){
						echo '<li class="small btm_bor"><a href="'.$shop_url.'products/'.$item["Products_ID"].'/"><div class="name"><span class="price">￥'.($item["Products_PriceA"]==0 ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</span><br />'.$item["Products_Name"].'</div><div class="pic">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</div></a></li>';
					}else{
						echo '<li class="small"><a href="'.$shop_url.'products/'.$item["Products_ID"].'/"><div class="name"><span class="price">￥'.($item["Products_PriceA"]==0 ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</span><br />'.$item["Products_Name"].'</div><div class="pic">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</div></a></li>';
					}
					$j++;
				}
