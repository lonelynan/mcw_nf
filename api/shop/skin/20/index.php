...
//获取新品
$rsNewProducts = $DB->GetAssoc("shop_products","*","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0 and Products_Union_ID=0 and Products_Status=1 order by Products_Index asc,Products_ID desc");
...
function get_products_bycate($db,$catid){
    ...
    $condition = 'where Products_SoldOut=0 and Products_Status=1 and Products_IsRecommend=1 and Products_Union_ID=0 ';

    if (!empty($catid)) {
        $condition .= ' and Products_Category in (' . $catid . ')';
    }
    $condition .= ' order by Products_Index asc,Products_ID desc limit 0,4';

	$rsNewProducts = $db->GetAssoc("shop_products","*",$condition);
	$new_products = handle_product_list($rsNewProducts, $shop_url, $pintuan_url);
	return $new_products;
}
?>
...
<div id="shop_page_contents">
 ...
 <div class="index_products">
  ...
  <ul>
   <?php
   $recommend_products = get_products_bycate($DB,$Category_IDS);
   $i=0;
   ...
   <li>
    <div>
     <p><a href="<?= $item['link'] ?>"><img class="product-image" width="80%" data-url="<?=$item['ImgPath']?>" src="/static/js/plugin/lazyload/grey.gif"/></a><p>
	 <p class="products_title"><?=$item['Products_Name']?></p>
     <p class="products_price"><span><a href="<?= $item['link'] ?>">立即抢购</a></span>&nbsp;&yen;<?=$item['Products_PriceX']?><br />&nbsp;<i><?=$item['Products_PriceY']?></i></p>
