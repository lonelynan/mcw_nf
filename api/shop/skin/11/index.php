  ...
  <div id="shop_skin_index">
    ...
    <div class="shop_skin_index_list ad2" rel="edit-t08">
      ...
      <?php
			...
			$condition = "where Users_ID='".$UsersID."' and Products_SoldOut=0 and Products_Union_ID=0 and Products_Status=1 and Products_IsHot=1 order by Products_Index asc,Products_ID desc";
			$DB->get("shop_products","*",$condition,3);
			while($item=$DB->fetch_assoc()){
				$JSON=json_decode($item['Products_JSON'],true);
				if($j==0){
					echo '<li class="big right_bor"><a href="/api/'.$UsersID.'/shop/products/'.$item["Products_ID"].'/"><div class="name">'.mb_substr($item["Products_Name"],0,10,'utf-8').'<br /><span class="price">￥'.(empty($item["Products_PriceA"]) ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</span></div><div class="pic">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</div></a></li>';
				}elseif($j==1){
					echo '<li class="left_bor btm_bor"><a href="/api/'.$UsersID.'/shop/products/'.$item["Products_ID"].'/"><div class="name">'.mb_substr($item["Products_Name"],0,10,'utf-8').'<br /><span class="price">￥'.(empty($item["Products_PriceA"]) ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</span></div><div class="pic small">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</div></a></li>';
				}else{
