...
<div id="shop_page_contents">
 ...
 <link href="/static/api/shop/skin/<?php echo $rsConfig['Skin_ID'];?>/page_media.css?t=<?php echo time();?>" type="text/css" rel="stylesheet" />
 ...
 <div id="shop_skin_index">
  ...
  if(isset($json)){
	  foreach($json as $value){
        ...
        <input type="submit" class="submit" value=" " style="background:url('.$pic[0].') no-repeat left top;" />
		...
		}elseif($value['type'] == 'p13'){//分类列表
			$condition = "where Users_ID='".$UsersID."' and Products_SoldOut=0 and Products_Status=1";
			if(!empty($value['category'])){
				$rsCategory = $DB->Get("shop_category","Category_ID","where Users_ID='".$UsersID."' and Category_ParentID = ".$value['category']);
				...
				$condition .= " and Products_Category IN  (".$Category_ID.") ";
			...
			$productList = $DB->GetAssoc("shop_products","*",$condition);
			$productList = handle_product_list($productList, $shop_url, $pintuan_url);
			foreach($productList as $key => $val){
				$productList[$key]['isCoupon'] = isCoupon($val['Biz_ID']);
			}
			// echo '<pre>';
			// print_R($productList);
			// exit;
			...
			if($value['list_type'] == '1'){            
    			...
    			if(!empty($productList)){
					...
					/*商品直接加入购物车开始*/
						...
						echo '<li>
			            	<a href="'.$v['link'].'"><span class="cpt_x l"><img src="'.$v['ImgPath'].'"></span></a>
			                <span class="cpxq_x l">
                                ...
                                if($v['isCoupon']){
                                    if($v['isCoupon'] == 1){
                                        echo '<div class="clear"></div><span class="quan_x">券</span><span class="activity">可用优惠券</span>';
                                    }
                                }
								// if(isset($cart2)){
								...
								if(isset($cart2)){
									if (empty($v['skuvaljosn']) && $v['Products_IsVirtual'] == 0) {
										echo " <span class='r' style='margin-top:3px;'><a href='javascript:void(0)' onclick='add_to_cart(".$v['Products_ID'].")' ><img src='/static/api/shop/skin/".$rsConfig['Skin_ID']."/gwct_x.png' width='20' height='20'></a></span>";
									}else{
				...
				if(!empty($productList)){
					...
					/*商品直接加入购物车开始*/
						...
						echo '<li>
			            	<a href="'.$v['link'].'"><span class="cpt_x"><img src="'.$v['ImgPath'].'"></span></a>
			               <div class="cpxq1_x">
			                	<div class="cpbt_x"><a href="'.$v['link'].'">'.$v['Products_Name'].'</a></div>';
			                	if(!empty($nature)){
									if(in_array('integration', $nature)){
					...
					/*商品直接加入购物车开始*/
						...
						if($k%3 == 0) {
							...
							echo '<span class="big_pr l">';
								echo "<a href='".$v['link']."'><div class='pic'><img src='".$v['ImgPath']."'></a></div>";
						...
						if($k%3 == 1) {
							...
							echo '<div class="small_pr">
								<a href="'.$v['link'].'">
	                          	...
	                          	<span class="pic l"><img src="'.$v['ImgPath'].'"></span>
	                          	</a>
	                      	</div>';
						} 
						echo '<div class="clear"></div>';
