<script language="javascript">
    ...
    function load_page(type, page){
		var post_url = '?UsersID='+UsersID+'&CategoryID=<?php echo !empty($_GET['CategoryID']) ? $_GET['CategoryID'] : '';?>&order_by=<?php echo !empty($_GET['order_by']) ? $_GET['order_by'] : '';?>&Is_Union=<?php echo isset($_GET['Is_Union']) ? $_GET['Is_Union'] : '';?>';
		//首先自动加载第一页
		$.ajax({
			type:'post',
			url:post_url,
						    ...
						    $htmltmp = '<a href="'+v['link']+'">'+
										'<div class="item">'+
											...
											'<div class="info">'+
											  ...
											  '<h2>￥'+(v['Products_PriceA'] != 0 ? v['Products_PriceA'] : v['Products_PriceX'])+'</h2>'+
