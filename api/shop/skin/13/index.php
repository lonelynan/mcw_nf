...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		...
		//获取推荐商品
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Users_ID='".$UsersID."' and Products_IsRecommend=1 and Products_SoldOut=0 and Products_Status=1 and Products_Union_ID=0");
		$num = 4;//每页记录数
		$p = !empty($_POST['p'])?intval(trim($_POST['p'])):1;
		$total = $counts['count'];//数据记录总数
		$totalpage = ceil($total/$num);//总计页数
		$limitpage = ($p-1)*$num;//每次查询取记录
		$rsRecommendProducts = $DB->GetAssoc("shop_products","*","where Users_ID='".$UsersID."' and Products_IsRecommend=1 and Products_Union_ID=0 and Products_SoldOut=0 and Products_Status=1 order by Products_Index asc,Products_ID desc limit $limitpage,$num");
		$rsRecommendProducts = handle_product_list($rsRecommendProducts, $shop_url, $pintuan_url);
...
<div id="shop_page_contents">
 ...
 <link href='/static/api/shop/skin/<?php echo $rsConfig['Skin_ID'];?>/page.css?t=<?=time()?>' rel='stylesheet' type='text/css' />
 <link href='/static/api/shop/skin/<?php echo $rsConfig['Skin_ID'];?>/page_media.css?t=<?=time()?>' rel='stylesheet' type='text/css' />
