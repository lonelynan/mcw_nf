<?php
	if(!empty($_POST)){
		...
		foreach($productList as $key => $val){
            if ($val['pintuan_people'] >= 2 && $val['pintuan_flag'] == 1 && $val['pintuan_start_time'] <= time() && $val['pintuan_end_time'] > time() && ($val['pintuan_pricex'] > 0 || $val['flashsale_pricex'] > 0)) {
                $productList[$key]['link'] = $pintuan_url.'xiangqing/'.$val["Products_ID"].'/';
            } else {
                $productList[$key]['link'] = $shop_url.'products/'.$val["Products_ID"].'/';
            }
			$productList[$key]['JSON'] = json_decode($val['Products_JSON'],true);
			if (!empty($productList[$key]['JSON']) && !empty($productList[$key]['JSON']['ImgPath'])) {
				foreach ($productList[$key]['JSON']['ImgPath'] as $k => &$v) {
					$v = getImageUrl($v, 1);
				}
			}
		}
...
var Is_Union = "<?php echo isset($_GET['Is_Union']) ? ($_GET["Is_Union"] === '0' ? '0' : 1) : '';?>";
</script>
...
<div id="shop_page_contents">
  ...
  <link href='/static/api/shop/skin/<?php echo $rsConfig["Skin_ID"] ?>/products.css?t=<?=time()?>' rel='stylesheet' type='text/css' />
