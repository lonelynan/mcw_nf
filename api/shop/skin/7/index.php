    ...
    <?php
		...
		foreach($Category_List as $rscategory){
			...
			while($item=$DB->fetch_assoc()){
				...
				if($j==1){
					echo '<li class="big mar_b"><div><a href="'.$shop_url.'products/'.$item["Products_ID"].'/">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</a></div><div class="price"><div class="bg"></div><div class="cont">'.($item["Products_PriceA"] == 0 ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</div></div></li>';
				}else{
					echo '<li class="mar_lb"><div><a href="'.$shop_url.'products/'.$item["Products_ID"].'/">'.(empty($JSON["ImgPath"])?'暂无图片':'<img data-url="'.$JSON["ImgPath"][0].'" src="/static/js/plugin/lazyload/grey.gif" />').'</a></div><div class="price"><div class="bg"></div><div class="cont">'.($item["Products_PriceA"] == 0 ? $item["Products_PriceX"] : $item["Products_PriceA"]).'</div></div></li>';
				}
				$j++;
			}
