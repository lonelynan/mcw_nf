...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		...
		$catid = $_POST['cate_id'];
		$new_products = array();
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Products_SoldOut=0 and Products_Status=1 and Products_Union_ID=0 and Products_Category in (".$catid.")");
		$num = 2;//每页记录数
		if($counts['count'] <= $num){
			$show_load_btn = 0;
		}
...
<div id="shop_page_contents">
 ...
 <?php 
 $b = 1;
 foreach($category_list as $key=>$t){
	 //二级分类ID
		...
		$Category_IDS = '';
		foreach($Category_erlist as $k=>$value){
