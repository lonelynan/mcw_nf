...
for($no=1;$no<=1;$no++){
	$json[$no-1]=array(
		...
		"Title"=>$no==1 && isset($Home_Json[$no-1]['Title'])?json_encode($Home_Json[$no-1]['Title']):
            (isset($Home_Json[$no-1]['Title']) ? $Home_Json[$no-1]['Title'] : ''),
		"ImgPath"=>$no==1 && isset($Home_Json[$no-1]['ImgPath'])?json_encode($Home_Json[$no-1]['ImgPath']):(isset($Home_Json[$no-1]['ImgPath']) ?
            $Home_Json[$no-1]['ImgPath'] : ''),
		"Url"=>$no==1 && isset($Home_Json[$no-1]['Url'])?json_encode($Home_Json[$no-1]['Url'])
            :(isset($Home_Json[$no-1]['Url']) ? $Home_Json[$no-1]['Url'] :''),
...
.fen_x{ float: left;
 ...
 <div id="shop_skin_index">
    ...
    <div class="biz_info" style="z-index: 500;">
	 ...
	 <a class="offline_pay" href="<?php echo $shop_url;?>biz/<?php echo $BizID;?>/paymoney/">在线买单</a>
     <?php }?>
	</div>
 </div>
 <div class="dianpu_contact">
  ...
  <a href="tel:<?php echo $rsBiz["Biz_Phone"];?>" class="tel"><span></span>客服电话：<?php echo $rsBiz["Biz_Phone"];?></a>
