...
$_SESSION[$UsersID."HTTP_REFERER"] = "/api/".$UsersID."/shop/cart/checkout/?love";
...
$rsUser = $DB->GetRs("user","User_Profile,User_Integral,User_Level","where User_ID=".$User_ID);
...
$redisCart = $myRedis->getCartListValue($UsersID."CartList", $User_ID);
...
<link rel="stylesheet" href="/static/api/shop/skin/default/css/tao_checkout.css?t=<?=time()?>" />
...
<div id="wrap"> 
	...
	<form id="checkout_form" action="<?=$base_url?>api/<?=$UsersID?>/shop/cart/">
		<input type="hidden" name="AddressID"  id="AddressID" value="<?=$rsAddress['Address_ID'] ?>" />
			...
			<div class="row panel panel-default">
				...
				<div class="panel-body">
					<div class="product-list" >
						...
						$Products_Integration = $Products_Integration/$rsConfig['Integral_Buy'];
						}
						?>
    ...
    <ul class="list-group" id="man_reduce_act">
        ...
        <li class="list-group-item jieleftfive">
           可抵用积分金额为：￥<span class="fc_red" id="jin_diyong<?=$Biz_ID?>"><?=$Products_Integration?></span>元
        </li>
...
$('input[name="offset<?=$Biz_ID?>"]').click(function(){
				...
				if($(this).is(":checked")==true){
					...
					var	jin_zzk = (Math.round((jin_zz - diyong)*100)/100).toFixed(2);
