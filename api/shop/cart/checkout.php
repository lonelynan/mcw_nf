...
foreach ($CartList as $biz_id => $products) {
	$BizIDArr[] = $biz_id;
    //获取对应商家信息
    $rsBiz = $DB->GetRs('biz', 'Biz_Account, Is_BizStores, Biz_Name, Biz_Logo', 'where Biz_ID = ' . $biz_id);
    if (empty($rsBiz)) {
    ...
    foreach ($products as $pro_id => $vv) {
        foreach ($vv as $attr_id => $v) {
			...
			$Is_BizStores[$biz_id] = $DB->GetRs("shop_products", "Is_BizStores", "where Products_ID=" . $pro_id);
        }
    }
}
/********************* 计算积分 end *************************/
