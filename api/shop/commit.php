...
if(!isset($_GET['ProductsID']) && !isset($_GET['Biz_ID'])){
    echo '缺少必要的参数';
    exit;
}
...
if(isset($_GET['Biz_ID'])){
    $Biz_ID=$_GET["Biz_ID"];
    $rsProducts =$DB->GetRs("biz","*","where Users_ID='".$UsersID."' and Biz_ID=".$Biz_ID);
}
...
//调用模版
$header_title = '全部评论 - '.(isset($rsProducts["Products_Name"]) ? $rsProducts["Products_Name"] : $rsProducts['Biz_Name'] ).' - '.$rsConfig["ShopName"];
