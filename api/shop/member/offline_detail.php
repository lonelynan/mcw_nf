...
$imgs = isset($rsOrder['Order_Pzimg']) ? explode(',',$rsOrder['Order_Pzimg']) : array();
?>
...
<div id="shop_page_contents">
  ...
  <h2 style="width:100%; height:40px; line-height:38px; font-size:16px; text-align:center; font-weight:bold; border-bottom:1px #dfdfdf solid" onclick="location.href='/api/<?=$UsersID?>/shop/member/offline_status/4/'">我的实体店消费单</h2>
  <div id="order_detail">
    <div class="item">
      <ul>
    ...
    <div class="item">
      <ul>
        <li>商品发票: 需要发票</li>
		<li>发票抬头: <?php echo $rsOrder["Order_InvoiceInfo"] ?></li>
      </ul>
    </div>
	<?php }else{?>
	<div class="item">
    ...
    <div class="item">
      <ul>
        ...
        <li>收货地址: <?php echo $Province.$City.$Area.'【'.$rsOrder["Address_Name"].'，'.$rsOrder["Address_Mobile"].'】'.'&nbsp;&nbsp;&nbsp;&nbsp;详细地址: '.$rsOrder["Address_Detailed"] ?></li>
        <li>配送方式: <?php
		if(empty($Shipping)){
				echo "赞无信息";
			}else{
...
<?php }?>
<?php if(!empty($rsOrder["Order_Pzimg"])){?>
<div class="item">
      <ul>
        <li>凭证图片</li>
        <li>
		<?php
			foreach($imgs as $img){
				echo '<a href="'.$img.'" target="_blank" style="display:block; margin-top:5px;"><img src="'.$img.'" style="max-width:80%" /></a>';
			}
		?>
		</li>        
      </ul>
...
if(!empty($lists_back)){
	foreach($lists_back as $item){
		$CartList_back=json_decode($item["Back_Json"],true);
...
?>
    ...
    <div class="payment"><a href="/api/shop/member/orders.php?UsersID=<?=$UsersID?>&action=confirm_receive&OrderID=<?=$rsOrder["Order_ID"]?>">确认收货</a></div>
	<?php }?>
