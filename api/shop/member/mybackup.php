...
//获取退货单
$condition = "where Users_ID='".$UsersID."' AND Back_Type NOT IN ('pintuan','dangou')";
$condition .= " and User_ID=".$_SESSION[$UsersID.'User_ID'];
//$condition .= " and Back_Status=".$Status ;
$condition .= ' order by Back_ID desc';
...
<div id="shop_page_contents" style="color:#0F3"
  ...
  <?php $CartList_back = json_decode(htmlspecialchars_decode($item['Back_Json']),TRUE)?>
    <div class="item">
      <h1> 退货单号：<a href="/api/<?=$UsersID?>/shop/member/backup/detail/<?=$item['Back_ID']?>/?love"><?=$item['Back_Sn']?></a></h1>
      <?php
	  ...
	  echo '<div class="pro">
			...
			echo '<dd style="height:26px; color:#ff0000">'.(empty($_STATUS[$item["Back_Status"]]) ? '' : $_STATUS[$item["Back_Status"]]);
			if($item["Back_Status"]==1){
				echo '<a href="'.$base_url.'api/'.$UsersID.'/shop/member/backup/detail_send/'.$item["Back_ID"].'/?love" style="display:block; width:70px; height:26px; line-height:24px; color:#FFF; background:#F60; border-radius:8px; text-align:center; font-size:12px; font-weight:normal; float:right">我要发货</a>';
			}
			echo '</dd>';
	  echo '</dl>';
