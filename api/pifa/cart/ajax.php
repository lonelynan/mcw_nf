...
if($action=="qty"){//商品加减操作
	...
	if($rsConfig['NeedShipping']==1 && $product_info["rsProducts"]["Products_IsShippingFree"]==0){
	...
	if(empty($_POST['OrderID'])){
		...
		$ownerid = $_POST['OwnerID'];
		$total_shipping_fee = 0;
		$product_info = $DB->GetRs("pifa_products","Products_IsShippingFree","where Products_ID=".$_POST["ProductID"]);
		
		if($_POST['Need_Shipping']==1 && $product_info["Products_IsShippingFree"]==0){
			$total_info = _get_shipping_fee($_POST["ProductID"],$_POST['Qty'],$rsConfig,$Shiping_ID,$City_Code,$ownerid);
			$total_shipping_fee = $total_info['total_shipping_fee'];
		}
		
		$OrderID = _add_order($_POST['ProductID'], $_POST['Qty'], $_POST['AddressID'], $_POST['Need_Shipping'], $Order_Shipping, $_POST['OwnerID'], $total_shipping_fee);
	}else{
	...
	if($_POST['PaymentMethod']=="线下支付" || $rsOrder["Order_TotalPrice"]<=0){//线下支付
...
function _add_order($ProductID,$qty=0,$AddressID=0,$Need_Shipping=0,$Order_Shipping='',$ownerid=0,$Shipping_Price = 0){
	...
	if($is_distribute){
		...
		foreach($CartList as $ProductID=>$product_list){
			foreach($product_list as $key=>$item){
				if($item['OwnerID']>0){
					add_distribute_record_pifa($UsersID,$item['OwnerID'],$item['ProductsPriceX'],$ProductID,$item['Qty'],$neworderid,$rsProducts);
				}
			}
		}
	}
