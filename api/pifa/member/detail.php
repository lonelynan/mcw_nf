	...
	<div id="order_detail">
		...
		<div class="item">
			<ul>
				...
				<li>支付信息: <?php echo $rsOrder["Order_PaymentInfo"] ?><a href="/api/<?php echo $UsersID ?>/pifa/cart/payment/<?php echo $rsOrder["Order_ID"] ?>/" class="red"><strong>修改支付信息</strong></a></li>
				<?php }?>
			</ul>
		</div>
		<?php }?>
...
foreach($CartList as $key=>$value){
	foreach($value as $k=>$v){
		...
		echo '<div class="pro">
			<div class="img"><a href="/api/'.$UsersID.'/pifa/product/'.$key.'/"><img src="'.$v["ImgPath"].'" width="100" height="100"></a></div>
			<dl class="info">
		...
		<div class="payment"><a href="/api/<?php echo $UsersID ?>/pifa/cart/payment/<?php echo $rsOrder["Order_ID"] ?>/">付款</a></div>
		<div class="payment"><a href="/api/pifa/member/orders.php?UsersID=<?=$UsersID?>&action=delete&OrderID=<?=$rsOrder["Order_ID"]?>">取消</a></div>
		<?php }?>
		<?php if($rsOrder["Order_Status"]==3 && $rsOrder["Is_Commit"]==0){ ?>
		<div class="backup"><a href="/api/<?php echo $UsersID ?>/pifa/member/backup/<?php echo $rsOrder["Order_ID"] ?>/">去退货</a></div>
		...
		<div class="payment"><a href="/api/<?php echo $UsersID ?>/pifa/member/commit/<?php echo $rsOrder["Order_ID"] ?>/">评论</a></div>
		<?php }?>
	</div>
</div>
