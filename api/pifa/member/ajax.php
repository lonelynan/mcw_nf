...
if($action=="address"){
	...
	if(!$rsOrder){
	...
	}elseif(($rsOrder["Order_Status"]<>3 && $rsOrder["Order_IsVirtual"]==0) && ($rsOrder["Order_Status"]<>2 && $rsOrder["Order_IsVirtual"]==1)){
		$Data=array(
			"status"=>0,
			"msg"=>'只有“已发货(实物)或已付款(虚拟)”状态下的订单才可申请退款'
		);
	}else{
		...
		if(empty($CartList[$_POST["ProductsID"]][$_POST["KEY"]])){
			...
			if($item["Qty"] < $_POST["Qty"]){
				...
				$backup->add_backup($rsOrder, $_POST["ProductsID"], $_POST["KEY"], $_POST["Qty"], $_POST["Reason"], $_POST["Account"],'pifa');
				$Data=array(
					"status"=>1,
	...
	if(!$rsBack){
		...
		$Data = array(
			"status"=>1,
			"url"=>"/api/".$UsersID."/pifa/member/backup/status/0/"
		);
	}
	echo json_encode($Data,JSON_UNESCAPED_UNICODE);	
