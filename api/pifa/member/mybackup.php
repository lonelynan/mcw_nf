...
<div id="shop_page_contents" style="color:#0F3"
  ...
  <?php $CartList_back = json_decode(htmlspecialchars_decode($item['Back_Json']),TRUE)?>
    <div class="item">
      <h1> 退货单号：<a href="/api/<?=$UsersID?>/pifa/member/backup/detail/<?=$item['Back_ID']?>/"><?=$item['Back_Sn']?></a></h1>
      <?php
	  echo '<div class="pro">
			...
			if($item["Back_Status"]==1){
				echo '<a href="'.$base_url.'api/'.$UsersID.'/pifa/member/backup/detail_send/'.$item["Back_ID"].'/" style="display:block; width:70px; height:26px; line-height:24px; color:#FFF; background:#F60; border-radius:8px; text-align:center; font-size:12px; font-weight:normal; float:right">我要发货</a>';
			}
			echo '</dd>';
	  echo '</dl>';
