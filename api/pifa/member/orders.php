...
if(!empty($_GET['action'])){
	...
	if($action == 'delete'){
		...
		if($Flag_a&&$Flag_b)
		{
			...
			$url = $base_url.'api/'.$UsersID.'/pifa/member/status/0/';	
...
$(document).ready(pifa_obj.order_init);
</script>
</head>
...
<div id="shop_page_contents">
  ...
  <ul id="member_nav">
    ...
    <li class="<?php echo $Status==0?"cur":"" ?>"><a href="/api/<?php echo $UsersID ?>/pifa/member/status/0/">待确认</a></li>
    <li class="<?php echo $Status==1?"cur":"" ?>"><a href="/api/<?php echo $UsersID ?>/pifa/member/status/1/">待付款</a></li>
    <li class="<?php echo $Status==2?"cur":"" ?>"><a href="/api/<?php echo $UsersID ?>/pifa/member/status/2/">已付款</a></li>
    <li class="<?php echo $Status==3?"cur":"" ?>"><a href="/api/<?php echo $UsersID ?>/pifa/member/status/3/">已发货</a></li>
    <li class="<?php echo $Status==4?"cur":"" ?>"><a href="/api/<?php echo $UsersID ?>/pifa/member/status/4/">已完成</a></li>
   
  </ul>
  <div id="order_list">
<?php
$DB->get("user_order","*","where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."' and Order_Type='pifa' and Order_Status=".$Status." order by Order_CreateTime desc");
$lists = array();
...
foreach($lists as $rsOrder){
	...
	if($rsOrder["Is_Backup"]==1){
		$condition = "where Users_ID='".$UsersID."' and Order_ID=".$rsOrder["Order_ID"]." and Back_Type='pifa'";
...
?>
    <div class="item">
      <h1> 订单号：<a href="<?php echo '/api/'.$UsersID.'/pifa/member/detail/'.$rsOrder["Order_ID"].'/' ?>"><?php echo date("Ymd",$rsOrder["Order_CreateTime"]).$rsOrder["Order_ID"] ?></a> （<strong class="fc_red">￥<?php echo $rsOrder["Order_TotalPrice"] ?></strong>）<?php echo $rsOrder["Coupon_ID"]==0 ? '' : '<font style="color:blue; font-size:12px;">已使用优惠券</font>';?> </h1>
	...
	if(isset($CartList)){
		foreach($CartList as $key=>$value){
			foreach($value as $k=>$v){
				echo '<div class="pro">
					<div class="img"><a href="/api/'.$UsersID.'/pifa/product/'.$key.'/"><img src="'.$v["ImgPath"].'" width="70" height="70"></a></div>
					<dl class="info">
						<dd class="name"><a href="/api/'.$UsersID.'/pifa/product/'.$key.'/">'.$v["ProductsName"].'</a></dd>				
				...
				if(($rsOrder['Order_Status'] == 3 && $rsOrder['Order_IsVirtual'] == 0) || ($rsOrder['Order_Status'] == 2 && $rsOrder['Order_IsVirtual'] == 1)){
					echo '<dd style="height:26px"><a href="'.$base_url.'api/'.$UsersID.'/pifa/member/backup/'.$rsOrder["Order_ID"].'_'.$key.'_'.$k.'/" style="display:block; width:70px; height:26px; line-height:24px; color:#FFF; background:#F60; border-radius:8px; text-align:center; font-size:12px; font-weight:normal; float:right">申请退款</a></dd>';
				}
				echo '</dl>';
	...
	if(!empty($lists_back)){
		foreach($lists_back as $item){
			$CartList_back=json_decode(htmlspecialchars_decode($item["Back_Json"]),true);
				...
				echo '<div class="pro">
					<div class="img"><a href="/api/'.$UsersID.'/pifa/product/'.$item["ProductID"].'/"><img src="'.$CartList_back["ImgPath"].'" width="70" height="70"></a></div>
					<dl class="info">
						<dd class="name"><a href="/api/'.$UsersID.'/pifa/product/'.$item["ProductID"].'/">'.$CartList_back["ProductsName"].'</a></dd>				
						<dd>￥'.$CartList_back["ProductsPriceX"].'×'.$CartList_back["Qty"].'=￥'.$CartList_back["ProductsPriceX"]*$CartList_back["Qty"].'</dd>';
				echo '<dd style="height:45px; font-size:12px;">'.$_STATUS[$item["Back_Status"]].'<a href="'.$base_url.'api/'.$UsersID.'/pifa/member/backup/detail/'.$item["Back_ID"].'/" style="display:block; width:70px; height:26px; line-height:24px; color:#FFF; background:#696969; border-radius:8px; text-align:center; font-size:12px; font-weight:normal; float:right">退款详情</a></dd>';
				echo '</dl>';
...
?>
   ...
   <div class="button_panel">   
     <div class="cancel"><a href="<?=$base_url?>api/pifa/member/orders.php?UsersID=<?=$UsersID?>&action=delete&OrderID=<?=$rsOrder["Order_ID"]?>">取消</a></div>
   ...
   <?php if($rsOrder['Order_Status'] == 1): ?>
   		<div class="button_panel">   
          <div class="cancel"><a href="<?=$base_url?>api/pifa/member/orders.php?UsersID=<?=$UsersID?>&action=delete&OrderID=<?=$rsOrder["Order_ID"]?>">取消</a></div>
	 <div class="payment"><a href="<?=$base_url?>api/<?=$UsersID?>/pifa/cart/payment/<?=$rsOrder["Order_ID"]?>/">付款</a></div>
	 <div class="clear"></div>
   </div>
   ...
   <div class="button_panel">   
     <div class="payment"><a href="<?=$base_url?>api/<?php echo $UsersID ?>/pifa/member/commit/<?php echo $rsOrder["Order_ID"] ?>/">评论</a></div>
	 <div class="clear"></div>
   </div>
   <?php endif;?>
    </div>
