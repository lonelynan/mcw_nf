...
foreach($lists as $k=>$v){
	...
	if($item){
		$v["title"] = '<a href="/api/'.$UsersID.'/zhongchou/detail/'.$item["itemid"].'/"><font style="color:#0DC05D">'.$item["title"].'</font></a>';
	}else{
...
?>
  <div class="order_item">
      <table width="100%" cellpadding="0" cellspacing="0">
       ...
       <tr>
        <td class="t1">回报</td>
        <td class="t2"><font style="color:#F00">
        <?php
            if($v["Order_CartList"]){
                $cartlist = json_decode($v['Order_CartList'],true);
                if(!empty($cartlist)){
                    $bizid = $v['Biz_ID'];
                    if(isset($cartlist[$v['Biz_ID']]['prize']) && $cartlist[$v['Biz_ID']]['prize']){
                        echo  $cartlist[$v['Biz_ID']]['prize']['title'];
                    }
                }else{
                    echo $v['Order_CartList']; 
                }
            }else{
                echo "无私奉献";   
            }
       ...
       <tr>
        <td class="t1">收货地址</td>
        <td class="t3"><?php echo $v["Address_Province"].' '.$v["Address_City"].' '.$v["Address_Area"];?><?php echo $v["Address_Detailed"];?></td>
       </tr>
       <tr>
        <td class="t1">备注</td>
