...
<script language="javascript">
    var UsersID = "<?=$UsersID?>";
    $(document).ready(user_obj.coupon_init);
</script>
<div class="pop_form">
...
<div id="coupon">
  ...
  <?php if($TypeID==0){
		$DB->query("SELECT * FROM user_coupon_record where User_ID='".$_SESSION[$UsersID."User_ID"]."' and Users_ID='".$UsersID."' and Coupon_StartTime<".time()." and Coupon_EndTime>".time()." order by record_CreateTime desc");		
	}elseif($TypeID==1){
		$DB->query("SELECT * FROM user_coupon WHERE Users_ID='".$UsersID."' AND Is_Delete=0 and Coupon_StartTime<".time()." and Coupon_EndTime>".time()." and user_coupon.Coupon_ID NOT IN ( SELECT Coupon_ID FROM user_coupon_record WHERE Users_ID='".$UsersID."' and User_ID = ".$_SESSION[$UsersID."User_ID"]." ) order by Coupon_CreateTime desc");
	}else{
		$DB->query("SELECT b.*,a.Coupon_UsedTimes FROM user_coupon_record AS a LEFT JOIN user_coupon AS b ON b.Users_ID = a.Users_ID AND b.Coupon_ID = a.Coupon_ID where a.User_ID='".$_SESSION[$UsersID."User_ID"]."' AND b.Is_Delete=0 and b.Users_ID='".$UsersID."' and b.Coupon_EndTime<".time()." order by b.Coupon_CreateTime desc");
	}
	$rsCoupon_list = array();
	while($rsCou=$DB->fetch_assoc()){
		$rsCoupon_list[] = $rsCou;
	}
	foreach($rsCoupon_list as $key=>$rsCoupon){
		$rsCoupon_date =$DB->GetRs("user_coupon","*","where Users_ID='".$UsersID."' and Biz_ID=".$rsCoupon["Biz_ID"]." AND Coupon_ID=".$rsCoupon['Coupon_ID']);
		$rsCoupon['Coupon_Subject'] = $rsCoupon_date['Coupon_Subject'];
		$rsCoupon['Coupon_PhotoPath'] = $rsCoupon_date['Coupon_PhotoPath'];
		$rsCoupon['Coupon_Description'] = $rsCoupon_date['Coupon_Description'];
		if($TypeID==0){
			$coupon_url = 'href="/api/'.$UsersID.'/shop/biz_xq/'.$rsCoupon['Biz_ID'].'/"';
		}else{
			$coupon_url = 0;
		}
		echo '<div class="item">
		<h1>【'.$rsCoupon['Coupon_Subject'].'】</h1>
			<div class="p"><img src="' . $rsCoupon['Coupon_PhotoPath'] . '" /><a '.$coupon_url.'>';
			if($TypeID==0){
