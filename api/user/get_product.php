...
<div id="peas">
  <div class="t_list"> <a href="/api/<?php echo $UsersID ?>/user/get_product/" class="<?php echo $TypeID==0?'c':'' ?>">我领取的商品</a> <a href="/api/<?php echo $UsersID ?>/user/get_product/1/" class="<?php echo $TypeID==1?'c':'' ?>">领取商品</a> </div> 
  <?php if($TypeID==0){
	  ...
	  while($rsGet=$DB->fetch_assoc()){
		echo '<div class="item">
		<h1>【'.$rsGet['Get_Name'].'】</h1>
		<div class="d"><img src="'.$rsGet['Get_ImgPath'].'" /></div>
		<h2>'.(empty($rsGet['Get_Shipping'])?(empty($rsGet['Orders_Status'])?'未领取, 领取':'已领取, 领取'):(empty($rsGet['Orders_Status'])?'未发货, 领取':'已发货, 发货')).'时间: '.date("Y-m-d H:i:s",$rsGet['Orders_FinishTime']).'</h2>
		<h3>'.$rsGet['Get_BriefDescription'].'</h3>
	  </div>';
	  }
  }else{
		  ...
		  <?php foreach($dis_level as $k=>$val){
				...
				<?php foreach($get_product as $key=>$value){
					...
					<div class="item">
						...
						<h3><?=$value['Get_BriefDescription']?></h3>
					</div>
				<?php } } ?>  
		   <?php }} ?>	  
	  <?php }?>
