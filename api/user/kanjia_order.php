...
$fields = "Order_ID,Order_TotalAmount,Order_CartList,Order_CreateTime,Order_Status,Is_Commit";
...
foreach($order_list  as $key=>$item){
    ...
    foreach($item['Order_CartList']  as $bizid=>$product){
    foreach($product  as $proid=>$proval){
	$item['productid'] = $proid;
	}
	}
	if($item['Order_Status']==4 && $item['Is_Commit']==0){
		$item['show_comment_btn'] = 1;
	}else{
		$item['show_comment_btn'] = 0;
	}
	$order_list[$key] = $item;
}
