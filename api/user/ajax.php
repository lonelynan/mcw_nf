...
if(isset($_SESSION[$UsersID."User_ID"])){
	...
	if($action=='sign'){
		...
		if($rsSign){
			...
			if($Flag){
				...
				$modtype = 'integral';
				$url = "/api/".$UsersID."/shop/member/";
				$textone = "签到领取".$rsConfig['SignIntegral']."积分";
				$texttwo = '+'.$rsConfig['SignIntegral'];
				$textthree = '今日签到领取积分';
				$weixin_message->sendmessage($UserID,$contentStr,$modtype,$url,['textone' =>$textone, 'User_NickName' =>$rsUser['User_NickName'],'texttwo' => $texttwo,'User_Integral' => $rsUser['User_Integral'] + $rsConfig['SignIntegral'],'textthree' => $textthree]);
				$Data=array(
					'status'=>1,
			...
			if($rsPassword){
				...
				if($_POST['RecordType']=='获得积分'){
					foreach($UserLevel as $k=>$v){
						$score = isset($v['UpIntegral']) ? $v['UpIntegral'] : 0;
						if ($rsUser['User_TotalIntegral'] + $_POST['Integral'] >= $score) {
							$levelID=$k;
						}
					}
					$Data=array(
				...
				if($Flag){
					...
					$modtype = 'integral';
					$url = "/api/".$UsersID."/shop/member/";
					$textone = $_POST['RecordType']=='获得积分' ? "获得" : "使用";
					$textone .= $_POST['Integral']."积分";					
					$texttwo = $_POST['RecordType']=='获得积分' ? "+" : "-";
					$texttwo .= $_POST['Integral'];
					$textthree = $_POST['RecordType']=='获得积分' ? "获得积分" : "使用积分";
					$curUser_Integral = $_POST['RecordType']=='获得积分' ? $rsUser['User_Integral'] + $_POST['Integral'] : $rsUser['User_Integral'] - $_POST['Integral'];
					$weixin_message->sendmessage($UserID,$contentStr,$modtype,$url,['textone' =>$textone, 'User_NickName' =>$rsUser['User_NickName'],'texttwo' => $texttwo,'User_Integral' => $curUser_Integral,'textthree' => $textthree]);
					$Data=array(
						'status'=>1,
	    ...
	    $UserCoupon = $DB->GetRs('user_coupon_record','*',"where Users_ID='".$UsersID."' and User_ID=".$UserID." and Coupon_ID=".$_POST['CouponID']);
	    if(!empty($UserCoupon)){
	        $Data = array(
	            'status' => 0,
                'msg' => '您已经领取过该优惠券，请勿重复领取！'
            );
        }else {
	...
	}elseif($action=='peas_change'){
		$rsPeas=$DB->GetRs("user_peas","*","where Users_ID='".$UsersID."' and Peas_ID=".$_POST['PeasID']);
		if($rsPeas){
			if($rsUser['Peas_num']-$rsPeas['Peas_Integral']>=0){
				if($rsPeas['Peas_Qty']>=1){
					$Flag=$Flag&&$DB->Set("user_peas","Peas_Qty=Peas_Qty-1","where Users_ID='".$UsersID."' and Peas_ID=".$_POST['PeasID']);					
					//减掉用户表用户积分
					$Flag=$Flag&&$DB->Set("user","Peas_num=Peas_num-".$rsPeas['Peas_Integral'],"where Users_ID='".$UsersID."' and User_ID=".$UserID);
					if(empty($rsPeas['Peas_Shipping'])){
						$Data=array(
							"Users_ID"=>$UsersID,
							"User_ID"=>$UserID,
							"Address_Name"=>$rsUser['User_NickName'],
							"Address_Mobile"=>$rsUser["User_Mobile"]
						);
					}else{
					...
					$Data["Peas_ID"]=$_POST['PeasID'];
					$Data["Orders_FinishTime"]=time();
					$Data["Orders_CreateTime"]=time();
					$Flag=$Flag&&$DB->Add('user_peas_orders',$Data);
					if($Flag){
						$Data=array(
							'status'=>1,
							'msg'=>'操作成功！'
				...
				$Data=array(
					'status'=>0,
					'msg'=>'对不起，兑换此商品需'.$rsPeas['Peas_Integral'].'豆豆，您的帐户豆豆不足！'
				);
			}
		}else{
			$Data=array(
		...
		if($rsUser && is_numeric($_POST['Amount']) && $_POST['Amount']>0){
			...
			if($_POST["Operator"]==3){	
				$cartList = array(
					"1"=>array(
						"0"=>array(
							"ImgPath"=>'/static/api/shop/skin/default/xianxia.png',
							"ProductsName"=>'余额充值线下支付',
							"ProductsPriceX"=>$_POST['Amount'],	
							"Qty"=>1,
						)
					)									
				);
				...
				'Order_TotalPrice'=>$_POST['Amount'],
				'Address_Name'=>isset($rsUser['User_NickName'])?$rsUser['User_NickName']:$rsUser['User_Name'],
				'Order_Status'=>30,
				'Order_CreateTime'=>time(),	
				'Address_Detailed' =>$itemid,//地址字段兼职记录冲值记录ID
				'Order_CartList'=>$cartList,
				);
				$Flag2=$DB->Add('user_order',$Data2);
			...
			if($Flag&&$Flag2){
				if(isset($orderid)){
					$Data=array(
					'status'=>1,
					'url'=>'/api/'.$UsersID.'/shop/cart/complete_pay/huodaocharge/'.$orderid.'/',
					'type' => $_POST["Operator"],
					'amount' => $_POST['Amount'] * 100,
					'goodsname' => '余额充值订单',
					'orderid' => time() . $orderid . '_charge',
					'msg'=>'操作成功！'
					);
				}else{
					$Data=array(
...
// end sxf
		...
		if(!empty($_POST['BirthdayY']) && !empty($_POST['BirthdayM']) && !empty($_POST['BirthdayD'])){
			$Data["User_Birthday"] = $_POST['BirthdayY'].'-'.$_POST['BirthdayM'].'-'.$_POST['BirthdayD'];
		}
		if(!empty($_POST['QQ'])){
		...
		$rsUserConfig=$DB->GetRs("user_profile","*","where Users_ID='".$UsersID."'");

		if(isset($rsUserConfig['Profile_Input'])){
			$Home_Json_Input=json_decode($rsUserConfig['Profile_Input'],true);
			for($no=0;$no<=4;$no++){
		...
		if(isset($rsUserConfig['Profile_Select'])){
			$Home_Json_Select=json_decode($rsUserConfig['Profile_Select'],true);
			for($no=0;$no<=4;$no++){
		...
		$Data["User_Json_Input"] = isset($User_Json_Input) ? json_encode($User_Json_Input,JSON_UNESCAPED_UNICODE) : "";
		$Data["User_Json_Select"] = isset($User_Json_Select) ? json_encode($User_Json_Select,JSON_UNESCAPED_UNICODE) : "";
		
		$Flag=$Flag&&$DB->Set("user",$Data,"where Users_ID='".$UsersID."' and User_ID=".$UserID);
		$user = $DB->GetRs('user', '*', "WHERE Users_ID='" . $UsersID . "' AND User_ID=" . $UserID);
		if($user['Is_Distribute']){
			$DB->Set('distribute_account', ['Shop_Name' => $_POST['NickName'] . '的店'], "WHERE Users_ID='" . $UsersID . "' AND User_ID=" . intval($UserID));
		...
		exit;
		  ...
		  //发送短信功能未开启
			...
			if(isset($_SESSION[$UsersID.'mobile_time']) && !empty($_SESSION[$UsersID.'mobile_time'])){
				if(time() - $_SESSION[$UsersID.'mobile_time'] < 120){
					$Data = array(
						"status"=> 0,
						"msg"=>"发送短信过快"
					);
    ...
    if(isset($_GET['OwnerID'])){
        ...
        $url = '/api/' . $UsersID . '/user/' . $owner_id . '/login/';
    }else{
        $url = '/api/' . $UsersID . '/user/login/';
    }
}
echo json_encode(empty($Data)?array('status'=>0,'msg'=>'请先登录！','url' => isset($url) ? $url : ''):$Data,JSON_UNESCAPED_UNICODE);
?>