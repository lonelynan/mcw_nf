...
$mobile_new = $mobile_old = $unicom_new = $unicom_old = array();
$DB->Get('bianmin_server','*',"WHERE Users_ID='".$UsersID."'and Server_Type='".$Server_Type."'");
$i=0;
...
if(time()>=$expiretime){
	...
	$data_app = $refreshaccesstoken->get_access_token($paras_app,$myaccount['appsecret']);
...
<div id="bianmin_header">
	<a href="/api/<?php echo $UsersID; ?>/bianmin/" class="c">话费充值</a>
    <a href="/api/<?php echo $UsersID; ?>/bianmin/data/">流量充值</a>
    <div class="clear"></div>
</div>
<div id="gift">
		<div id="my">
		...
		if (isset($mobile_old)) {
			...
			for($i=0; $i<= $length-1; $i++){ 
				echo '<li class="li" style="width:65px; height:60px; border-radius:5px; float: left;overflow: hidden;padding-right: 8px; border: 1px solid #D94600;  margin-left: 13px; margin-right: 10px;border: 1px solid #d94600; margin-bottom: 10px;">
							<span>
									<p class="aja" style="line-height: 9px;  text-align: center;">'.$mobile_old[$i].'元<span class="little_tips">售价:'.$mobile_new[$i].'元</span></p>
									<from>
			...
			for($i=0; $i<= $length-1; $i++){ 
				echo '<li class="li" style="width:65px; height:60px; border-radius:5px; float: left;overflow: hidden;padding-right: 8px; border: 1px solid #D94600;  margin-left: 13px; margin-right: 10px;border: 1px solid #d94600; margin-bottom: 10px;">
							<span>
									<p class="aja" style="line-height: 9px;  text-align: center; ">'.$unicom_old[$i].'元<span class="little_tips">售价:9.98元</span></p>
									<from>
										<input class="put" type="hidden" value="'.$i.'" />
										<input class="type" type="hidden" value="'.$type.'" />
										<input class="st" type="hidden" value="'.$Server_Type.'" />
