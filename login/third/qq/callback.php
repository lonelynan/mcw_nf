...
if(isset($_SESSION["Users_ID"]) && !empty($_SESSION["Users_ID"])) {
	$url = '/api/' . $_SESSION["Users_ID"] . '/shop/member/?love';
	header("location:" . $url);
}
...
$qc->setRecorderValue('appkey', $config['secret']);
...
$qc1->setRecorderValue('appid', $config['appid']);
$arg = [
...
if (empty($user)) {
	...
	$DB->Set("user",array('User_No'=>$userno),'where Users_ID="'.$UsersID.'" and User_ID='.$userid);
...
if ($rsUser["User_Status"]) {
	...
	if(empty($_SESSION[$UsersID."HTTP_REFERER"])){
		$HTTP_REFERER = $url = '/api/' . $UsersID . '/shop/';
	}else{
