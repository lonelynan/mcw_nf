...
//$condition = "where Users_ID='".$_SESSION["Users_ID"]."' and Order_Type='kanjia' and User_ID =".$_SESSION[$_SESSION["Users_ID"].'User_ID'];
...
if(isset($_GET["search"])){
	...
	if($_GET["search"]==1){
		if(!empty($_GET["Keyword"])){
			...
			$condition  .= " or Order_ID like '%".substr($_GET["Keyword"],8)."%' ) ";
		}
...
$Shipping=json_decode(htmlspecialchars_decode($rsOrder["Order_Shipping"]),true);?>
          <tr class="<?php echo empty($rsOrder["Order_IsRead"])?"is_not_read":"" ?>" IsRead="<?php echo $rsOrder["Order_IsRead"] ?>" OrderId="<?php echo $rsOrder["Order_ID"] ?>">
            ...
            <td nowrap="nowrap"><?php echo isset($Order_Status[$rsOrder["Order_Status"]]) ? $Order_Status[$rsOrder["Order_Status"]] :'' ; ?></td>
            <td nowrap="nowrap"><?php echo date("Y-m-d H:i:s",$rsOrder["Order_CreateTime"]) ?></td>
            <td class="last" nowrap="nowrap"><a href="orders_view.php?OrderID=<?php echo $rsOrder["Order_ID"] ?>"><img src="/static/member/images/ico/view.gif" align="absmiddle" alt="修改" /></a><!-- <a href="orders.php?action=del&OrderID=<?php echo $rsOrder["Order_ID"] ?>" title="删除" onClick="if(!confirm('删除后不可恢复，继续吗？')){return false};"><img src="/static/member/images/ico/del.gif" align="absmiddle" /></a>-->
			</td>
          </tr>
          <?php }?>
        </tbody>
