...
if($_POST)
{
	...
	if($Flag){
		mysql_query("commit");
		echo '<script language="javascript">alert("修改成功");window.location="votes.php";</script>';
	}else{
      ...
      <form action="votes_edit.php" method="post" class="r_con_form" id="votes_form">
        ...
        <div class="rows">
          ...
          <div class="img" id="ImgDetail" style="padding-top:5px">
           <img src="<?= !empty($Material_Json["ImgPath"]) ? $Material_Json["ImgPath"] : '/static/api/images/cover_img/votes.jpg';?>" />
          </div>
          </span> </span>
          <div class="clear"></div>
        </div>
        ...
        <input type="hidden" id="ImgPath" name="ImgPath" value="<?= !empty($Material_Json["ImgPath"]) ? $Material_Json["ImgPath"] : '/static/api/images/cover_img/votes.jpg';?>" />
