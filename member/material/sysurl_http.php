...
if($_POST){
	...
	$rulreal = !empty($users_dominfo['domname']) && $users_dominfo['domenable'] == 1?$users_dominfo['domname']:$_SERVER["HTTP_HOST"];	
}
?>
<!DOCTYPE HTML>
<html>
  ...
  <div class="iframe_content">
    ...
    <div class="r_nav">
      <ul>
        ...
        <li class="cur"><a href="sysurl_http.php">系统URL查询</a></li>
      </ul>
    </div>
    <?php }else{ if($input){?>
    <script language="javascript">var parent_input = '<?php echo $input;?>'; $(document).ready(material_obj.url_select_sun);</script>
    <?php }}?>
    <div id="url" key="<?php echo $_GET['key'];?>" class="r_con_wrap" style="min-height:500px;">
      <div class="type">
       <?php foreach($modules as $m){?>
          <a href="<?php echo empty($menus[$m["moduleid"]]["type"]) ? 'sysurl_http.php?dialog='.$dialog.'&key='.$_GET['key'].'&input='.$input.'&module='.$m["module"] : '#';?>"<?php echo $module == $m["module"] ? ' class="cur"' : '';?>><?php echo $m["name"];?>
          <?php if(!empty($menus[$m["moduleid"]]["type"])){?>
          	<?php foreach($menus[$m["moduleid"]]["type"] as $t){?>
            <span onClick="window.location.href='sysurl_http.php?dialog=<?php echo $dialog;?>&key=<?=$_GET['key']?>&input=<?php echo $input;?>&module=<?php echo $m["module"];?>&type=<?php echo $t["module"];?>';"><?php echo $t["name"];?></span>
            <?php }?>
          <?php }?>
          </a>
       <?php }?>
       <?php if($dialog==1 && $input){?>
       <a href="sysurl_http.php?dialog=<?php echo $dialog;?>&key=<?=$_GET['key']?>&input=<?php echo $input;?>&module=diy"<?php echo $module == "diy" ? ' class="cur"' : '';?>>自定义URL</a>
       <?php }?>
      </div>
