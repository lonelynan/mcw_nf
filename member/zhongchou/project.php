    ...
    <div id="project" class="r_con_wrap">
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <tbody>
          <?php
		  	$lists = array();
          	$DB->getPage("zhongchou_project","*","{$condition}",10);
		  	while($r=$DB->fetch_assoc()){
            	$lists[] = $r;
			}
			foreach($lists as $k=>$v){
				$item = $DB->GetRs("user_order","count(*) as num,sum(Order_TotalPrice) as amount","WHERE Order_Type='zhongchou_".$v["itemid"]."' AND Users_ID='{$UsersID}' AND Biz_ID={$v['Biz_ID']} AND Order_Status = 2");
          ...
          <tr>
            ...
            <td nowrap="nowrap"><?php echo $v["prize"];?><br /><a href="prize.php?projectid=<?php echo $v["itemid"];?>">[查看]</a></td>
