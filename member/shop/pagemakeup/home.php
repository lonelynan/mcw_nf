...
if(isset($_GET['Makeup_Skin_ID'])){	
	$Makeup_Skin_ID = $rsConfig['Makeup_Skin_ID'];
}else{
	$Makeup_Skin_ID = 1 ;
	//echo '缺少必要的参数！';exit;
}

$rsSkin=$DB->GetRs("makeup_home","Home_Json","where Users_ID='".$_SESSION["Users_ID"]."' and Skin_ID=".$Makeup_Skin_ID);
$json=json_decode($rsSkin['Home_Json'],true);

if(!empty($rsConfig["Makeup_Skin_ID"])){
	include('skin/1.php');
	exit;
}
if($_POST){
	...
	$Flag=$DB->Set("makeup_home",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Skin_ID=".$rsConfig['Skin_ID']);
	...
	foreach($json as $k=>$value){
		...
		if(is_array(isset($value["ImgPath"]) && $value["ImgPath"])){
			$value["ImgPath"] = json_encode($value["ImgPath"]);
		}
		if(is_array(isset($value["Url"]) && $value["Url"])){
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <link href='/static/api/shop/skin/<?php echo $Makeup_Skin_ID;?>/page.css?t=<?php echo time() ?>' rel='stylesheet' type='text/css' />
