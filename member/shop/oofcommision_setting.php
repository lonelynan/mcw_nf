...
if ($_POST) 
{
  ...
  if (!is_numeric($_POST['noBi_Reward']) || !is_numeric($_POST['sha_Reward']) || !is_numeric($_POST['salesman_ratio']) || !is_numeric($_POST['commission_Reward']) || ($_POST['noBi_Reward']+$_POST['commission_Reward']+$_POST['salesman_ratio']+$_POST['sha_Reward']) > 100 || ($_POST['noBi_Reward']+$_POST['commission_Reward']+$_POST['salesman_ratio']+$_POST['sha_Reward']) <= 0 ) 
  {
...
// 判断佣金比例是否超过百分之百
  ...
  if($yongjifl==1){
	...
	$Data = array('Shop_oofCommision_Reward_Json' => json_encode($_POST, JSON_UNESCAPED_UNICODE));

  $Flag=$DB->Set("shop_config",$Data,"where Users_ID='".$_SESSION["Users_ID"]."'");
  if($Flag){
    echo '<script language="javascript">alert("修改成功");window.location="oofcommision_setting.php?cliid=\'sub11\'";</script>';
  }else{
  ...
  if (!is_null($shop_config['Shop_oofCommision_Reward_Json'])) 
  {
    $Shop_Commision_Reward_Arr = json_decode($shop_config['Shop_oofCommision_Reward_Json'], true);
  }
}
$jcurid = 1;
?>
...
$('.btn_green').on('click','',function(){
			...
			var rewardall = noBi_Reward + sha_Reward + commission_Reward + salesman_ratio;
			if(!retest.test(noBi_Reward) || !retest.test(sha_Reward) || !retest.test(commission_Reward) || !retest.test(salesman_ratio)){
					alert("佣金比例填写有误！");
					return false;
				}
			if(rewardall > 100){
					alert("爵位奖股东佣金业务佣金比例加起来不能大于100！");
					return false;
				}							
