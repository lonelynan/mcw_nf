...
$rsSkin=$DB->GetRs("shop_home_program","*","where Users_ID='".$_SESSION["Users_ID"]."' and Pro_Skin_ID=".$rsConfig['Pro_Skin_ID']);
$json=json_decode($rsSkin['Home_Json'],true);

if($rsConfig["Pro_Skin_ID"]==1){
	include('skin_program/'.$rsConfig['Pro_Skin_ID'].'.php');
	exit;
}
if($_POST){
	$no=intval($_POST["no"]);
	...
	$Flag=$DB->Set("shop_home_program",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Pro_Skin_ID=".$rsConfig['Pro_Skin_ID']);
	...
	if (empty($json)) {
		echo '<script>alert("请重新选择风格");location.href="skin_program.php";</script>';
		die();
	}	
	//重组json
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <link href='/static/api/shop/skin/<?php echo $rsConfig['Pro_Skin_ID'];?>/page.css?t=<?php echo time() ?>' rel='stylesheet' type='text/css' />
    ...
    <div id="home" class="r_con_wrap">
      <div class="m_lefter">
        <?php
			require_once('skin_program/'.$rsConfig['Pro_Skin_ID'].'.php');
		?>
      </div>
      <div class="m_righter">
        <form id="home_form">
