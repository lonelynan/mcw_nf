...
if($_POST){
	...
	echo '<script>window.location.href="switch_distribute.php";</script>';
}

require_once($_SERVER["DOCUMENT_ROOT"].'/include/yinru/defaut_switch.php');

	$perm_config = array();
	$rs = $DB->Get('permission_config','*','where Users_ID="'.$_SESSION['Users_ID'].'" AND Is_Delete = 0 AND Perm_Tyle=1 order by Perm_On  desc');
...
<script>
	$('.form_add').click(function(){
		...
		var TelHtml='<tr><td><input type="text" class="sun_input" name="name[]" value=""></td><td><div class="photo_config"><input name="LogoUpload[]" class="file_input  LogoUpload" type="button"  value="上传图标" /><input name="logo_num" type="hidden" value="'+num+'"><div class="img LogoDetail'+num+'"></div><input type="hidden" class="Logo'+num+'" name="Logo[]" value="" /></div></td><td><input type="text" name="http_url[]" value="" class="sun_input menu_url'+num+'" size="30" maxlength="200" /><img src="/static/member/images/ico/search.png" style="width:22px; height:22px; margin:0px 0px 0px 5px; vertical-align:middle; cursor:pointer" id="btn_select_url" class="http_url_sun" key="'+num+'"/></td><td><select name="Tyle_IS[]"><option value="1">分销中心</option></select></td><td><a href="javascript:void(0);" onclick="TelDel(this)"><img src="/static/member/images/ico/del.gif" /></a></td></tr>';
	...
	$(".Withdraw").click(function(){
		...
		$.get('?','action=get_Withdraw&Perm_ID='+Perm_ID+'&Perm_On='+Perm_On,function(data){
			if(data.status == 1){					
				alert('修改成功！');
				window.location.href= "switch_distribute.php";
			}else{
				alert('修改失败！');
			}
		},'json');	
