...
if(isset($_GET["action"]))
{
	...
	{		
		...
		$rsHome=$DB->GetRs("shop_skin","Skin_Json","where Skin_ID=".$_GET["Skin_ID"]);
			$Data=array(				
				"Home_Json"=>$rsHome["Skin_Json"]
			);
		$Set=$DB->Set("shop_home",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Skin_ID=".$_GET["Skin_ID"]);
		if($Set)
		{
...
function ReSkinID(Skin_ID){
	if(confirm("您确定要重置此风格吗？")){
		location.href="skin.php?action=setre&Skin_ID="+Skin_ID;
	}
}
</SCRIPT>
</head>
...
while($rsSkin=$DB->fetch_assoc()){
	$show_title = $rsSkin['Skin_ID']==1 ? 'Diy个性首页' : $i.'号模版';	
	echo '<li class="'.($rsConfig['Skin_ID']==$rsSkin['Skin_ID']?"cur":"").'">
            <div class="item" title="点击选择商城风格" onClick="SelectSkinID('.$rsSkin['Skin_ID'].');">
              <div class="img"><img src="/static/member/images/shop/skin/'.$rsSkin["Skin_ID"].'.jpg" /></div>              
            </div>
			<input type="button" onClick="ReSkinID('.$rsSkin['Skin_ID'].');" class="btn_green" name="submit_button" style="cursor:pointer;margin:5px 0px 0px 25px" value="恢复'.$show_title.'" />
          </li>';
		  $i++;
}?>
      </ul>
