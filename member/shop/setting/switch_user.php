...
if(isset($_GET["action"])){
	if($_GET["action"]=="switch"){
		...
		$Flag = $DB->Set("permission_config",$Data,"where Permission_ID=".$_GET["Perm_ID"]);
		if($Flag) {
			$Data=array("status"=>1);
			echo json_encode($Data,JSON_UNESCAPED_UNICODE);
			exit;
...
if($_POST){
	...
	echo '<script>window.location.href="switch_user.php";</script>';
}

require_once($_SERVER["DOCUMENT_ROOT"].'/include/yinru/defaut_switch.php');

	$perm_config = array();
	$rs = $DB->Get('permission_config','*','where Users_ID="'.$_SESSION['Users_ID'].'" AND Is_Delete = 0 AND Perm_Tyle=2 order by Perm_On desc');
	while($rs = $DB->fetch_assoc()){
		$perm_config[] = $rs;
	}
	$switchplace = array('非法操作','分销中心','个人中心','分销中心-二维码');
$jcurid = 7;
?>
<!DOCTYPE HTML>
<html>
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
		...
		<?php foreach($perm_config as $ky=>$value){?>
				...
				<?php if(empty($value['Perm_Field'])){?>
					<img src="/static/member/images/ico/del.gif" style="width:25px;height:25px;"  Permission_ID="<?=$value['Permission_ID']?>"class="deleted" />
				<?php }?>
				...
				<table border="0" cellpadding="5" cellspacing="0" style="width: 100%;text-align: center;" class="reverve_field_table">
					...
					<tbody class="form_list">
						<tr>
							...
							<td>
								<input type="text" name="http_url[]" value="" class="sun_input menu_url" size="30" maxlength="200"  /><img src="/static/member/images/ico/search.png" style="width:22px; height:22px; margin:0px 0px 0px 5px; vertical-align:middle; cursor:pointer" id="btn_select_url" class="http_url_sun" key=""/>
							</td>
							<td>
								<select name="Tyle_IS[]">
							...
							<td><a href="javascript:void(0);" class="form_add"><img src="/static/member/images/ico/add.gif" /></a></td>
						</tr>
					</tbody>
				</table>
			</span>
		...
		<div class="rows">
			<label>&nbsp;</label>
			<span class="input"><input type="submit" class="btn_ok" style="margin-left: 30%;" name="submit_button" value="提交保存" /><a href="index.php" class="btn_cancel">返回</a></span>
			<div class="clear"></div>
		</div>
		</form>	  
    </div>
...
<script>
	$('.form_add').click(function(){
		...
		var TelHtml='<tr><td><input type="text" class="sun_input" name="name[]" value=""></td><td><div class="photo_config"><input name="LogoUpload[]" class="file_input  LogoUpload" type="button"  value="上传图标" /><input name="logo_num" type="hidden" value="'+num+'"><div class="img LogoDetail'+num+'"></div><input type="hidden" class="Logo'+num+'" name="Logo[]" value="" /></div></td><td><input type="text" name="http_url[]" value="" class="sun_input menu_url'+num+'" size="30" maxlength="200" /><img src="/static/member/images/ico/search.png" style="width:22px; height:22px; margin:0px 0px 0px 5px; vertical-align:middle; cursor:pointer" id="btn_select_url" class="http_url_sun" key="'+num+'"/></td><td><select name="Tyle_IS[]"><option value="2">个人中心</option></select></td><td><a href="javascript:void(0);" onclick="TelDel(this)"><img src="/static/member/images/ico/del.gif" /></a></td></tr>';
	...
	$(".Withdraw").click(function(){
		...
		$.get('?','action=get_Withdraw&Perm_ID='+Perm_ID+'&Perm_On='+Perm_On,function(data){
			if(data.status == 1){					
				alert('修改成功！');
				window.location.href= "switch_user.php";
			}else{
				alert('修改失败！');
			}
		},'json');	
