...
switch($action) {
	case 'download':
		...
		$url = 'http://down.haofenxiao.net/update2.php?product='.$version['product'].'&type='.$version['type'].'&isopen='.$version['isopen'].'&release='.$release;
		...
		if(@httpcopy($code, $release_dir . '/' . $release . '.zip')) {
	...
	break;
	case 'getdown':
		$url = 'http://down.haofenxiao.net/down2.php?product=' . $version['product'].'&type='.$version['type'].'&isopen='.$version['isopen'].'&release='.$release;
		$code = @file_get_contents($url);
		if($code){
			if(substr($code, 0, 8) == 'StatusOK') {
				$code = substr($code, 8);
