...
if(isset($_GET["action"]))
{
	...
	{		
		...
		$Set=$DB->Set("web_home",$Data,"where Skin_ID=".$_GET["Skin_ID"]);
		if($Set)
		{
...
function ReSkinID(Skin_ID,Trade_ID){
	if(confirm("您确定要重置此风格吗？")){
		location.href="skin.php?action=setre&Skin_ID="+Skin_ID+"&Trade_ID="+Trade_ID;
	}
}
</SCRIPT>
</head>
        ...
        <ul>
		  ...
		  while($rsSkin=$DB->fetch_assoc()){
			...
			echo '<li class="'.($rsConfig['Skin_ID']==$rsSkin['Skin_ID']?"cur":"").'">
					<div class="item">
					  <div class="img" title="点击选择微官网风格" onClick="SelectSkinID('.$rsSkin['Skin_ID'].','.$TradeID.');"><img src="'.$rsSkin['Skin_ImgPath'].'" /></div>
					  <input type="button" onClick="ReSkinID('.$rsSkin['Skin_ID'].','.$TradeID.');" class="btn_green" name="submit_button" style="cursor:pointer;margin:5px 0px 0px 25px" value="恢复'.$show_title.'" />
					</div>					
				  </li>';
			$i++;
		  }
