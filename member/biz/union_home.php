...
$Skin_ID = $DB->GetRs("biz_config","*","where Users_ID='".$_SESSION["Users_ID"]."'")['Skin_ID'];
$flag = true;
if (empty($Skin_ID)) {
    $flag = $DB->Add('biz_config',['Pro_Skin_ID'=>2,'Users_ID'=>$_SESSION['Users_ID'],'Skin_ID'=>2]);
    $Skin_ID = 2;
}
$rsHome = $DB->GetRs("biz_union_home","*",'where Users_ID="'.$_SESSION["Users_ID"].'" and Skin_ID='.$Skin_ID);

$biz_skin = $DB->GetRs("biz_union_skin","*",'where Skin_ID='.$Skin_ID);
if(empty($biz_skin)){
...
if($_POST){
	...
	if(!empty($Home_Json[$no-1])){
		...
		$Flag = $DB->Set("biz_union_home",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Skin_ID=".$Skin_ID);
		if($Flag){
			$json=array(
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div class="r_nav">
      <ul>
        <li><a href="union.php">商家列表</a></li>
          <li><a href="union_skin.php">风格设置</a></li>
          <li class="cur"><a href="union_home.php">首页设置</a></li>
        <li><a href="category.php">行业分类</a></li>
        <li><a href="region.php">区域设置</a></li>
        <li><a href="price.php">价格筛选</a></li>
      </ul>
    </div>
