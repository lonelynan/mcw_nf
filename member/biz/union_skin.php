...
$rsConfig = $DB->GetRs("biz_config", "Skin_ID", "where Users_ID='" . $_SESSION["Users_ID"] . "'");
$flag = true;
if (empty($rsConfig)) {
...
if (isset($_GET["action"]) && $flag) {
    if ($_GET["action"] == "set") {
        ...
        $rsHome = $DB->GetRs("biz_union_skin", "*", "where Skin_ID=" . $Skin_ID);
        if (!$rsHome) {
        ...
        $biz_home = $DB->GetAssoc('biz_union_home','*','where Users_ID="'.$_SESSION["Users_ID"].'" and Skin_ID='.$Skin_ID);
        if(empty($biz_home)){
            $Data=[
                "Skin_ID" => $Skin_ID,
           ...
           $biz_home_flag = $DB->Add('biz_union_home',$Data);
...
$reSkin = $DB->GetAssoc("biz_union_skin", "*", "where Skin_Status=1 order by Skin_Index asc");
    ...
    <script type=text/javascript>
        function SelectSkinID(Skin_ID) {
                if(confirm('您确定要选择此风格吗？')){
                    location.href = "union_skin.php?action=set&Skin_ID=" + Skin_ID;
                }
        }
        $(function(){
            ...
            <ul>
                <?php if (!empty($reSkin)) {
                    foreach ($reSkin as $k => $v) {
                        $i = 0; ?>
                        <li class="<?= $v['Skin_ID'] == $rsConfig['Skin_ID'] ? "cur" : "" ?>">
                            <div class="item" <?= $v['Skin_ID'] == $rsConfig['Skin_ID'] ? '' : 'onClick="SelectSkinID(' . $v['Skin_ID'] . ');"' ?>>
                                <div class="img"><img src="/static/biz/images/skin/<?= $v["Skin_ID"] ?>.jpg"/></div>
