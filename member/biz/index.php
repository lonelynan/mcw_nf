...
$bizclass = new biz($DB,$_SESSION["Users_ID"]);
if(isset($_GET["action"])){
	...
	if($_GET["action"]=="upgrade"){
		...
		$Flag = $bizclass->edit($Data,$_GET["BizID"]);
		if($Flag){
			$DB->Set("shop_products",array("Products_Union_ID"=>1),"where Users_ID='".$_SESSION["Users_ID"]."' and Biz_ID=".$_GET["BizID"]);
			echo '<script language="javascript">alert("升级成功");window.location="'.$_SERVER['HTTP_REFERER'].'";</script>';
		}else{
			echo '<script language="javascript">alert("升级失败");history.back();</script>';
		}
		exit;
	}
}
$_GET = daddslashes($_GET,1);
$condition = "where Users_ID='".$_SESSION["Users_ID"]."' and Is_Union=0";
...
$condition .= " and is_biz = 1 order by ".$OrderBy;
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="bizs" class="r_con_wrap">
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        <thead>
          <tr>
            ...
            <td width="10%" nowrap="nowrap">到期时间</td>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td nowrap="nowrap"><?php echo !empty($rsBiz["expiredate"])?date("Y-m-d",$rsBiz["expiredate"]):'' ?></td>
            ...
            <td class="last" nowrap="nowrap"><a href="edit.php?BizID=<?php echo $rsBiz["Biz_ID"] ?>"><img src="/static/member/images/ico/mod.gif" align="absmiddle" alt="修改" /></a> <a href="?action=del&BizID=<?php echo $rsBiz["Biz_ID"] ?>" onClick="if(!confirm('将会一并删除掉该商家的产品及订单您确定删除该商家？')){return false};"><img src="/static/member/images/ico/del.gif" align="absmiddle" alt="删除" /></a><a href="?action=upgrade&BizID=<?php echo $rsBiz["Biz_ID"] ?>">[升级]</a></td>
          </tr>
          <?php }?>
        </tbody>
      </table>
      ...
      <?php $DB->showPage(); ?>
      <div style="background:#F7F7F7; border:1px #dddddd solid; height:40px; line-height:40px; font-size:12px; margin:10px 0px; padding-left:15px; color:#ff0000">提示：商家登录地址 <a href="/biz/login.php" target="_blank">http://<?php echo $_SERVER['HTTP_HOST'];?>/biz/login.php</a></div>
    </div>
  </div>
</div>
</body>
