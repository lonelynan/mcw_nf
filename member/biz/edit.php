...
if ($_POST) {
    ...
    if (! empty($_POST["PassWord"])) {
        if ($_POST["PassWord"] != $_POST["PassWordA_e"]) {
    ...
    if (!empty($_POST["PaymenteRate"])) {
        if (!is_numeric($_POST["PaymenteRate"]) || $_POST["PaymenteRate"] <= 0) {
            echo '<script language="javascript">alert("结算比例必须数字且大于零！");history.back();</script>';
            exit();
        }
    }
...
<![endif]-->
	...
	<div id="iframe_page">
		<div class="iframe_content">
			<script type='text/javascript' src='/static/member/js/biz.js?t=1'></script>
				...
				<form class="r_con_form" method="post" action="?" id="group_edit">
					...
					<div class="rows">
						<label>确认登录密码</label> <span class="input"> <input type="password"
							name="PassWordA_e" value="" class="form_input" size="35"
							maxlength="50" /> <font class="fc_red">*</font></span>
						<div class="clear"></div>
					</div>
					<div class="rows">
						...
						<label>商家logo</label> <span class="input"> <span
							class="upload_file">
								<div>
									...
									<div class="tips">图片建议尺寸：300*300px</div>
									<div class="clear"></div>
								</div>
        ...
        ?>
					...
					<div class="rows">
						<label>接受短信手机</label> <span class="input"> <input type="text"
							...
							class="form_input" size="30" pattern="[0-9]*"/><span class="tips">当用户下单时，系统会自动发短信到该手机</span>
						</span>
						<div class="clear"></div>
					</div>
					<div class="rows">
