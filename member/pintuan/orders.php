...
$condition .= "WHERE u.Users_ID='{$UsersID}' AND ( u.Order_Type='pintuan' OR u.Order_Type='dangou') ";
    ...
    if ($_GET["search"] == 1) {
        if ($_GET['Fields']) {
            ...
            if (!empty($_GET["Keyword"])) {
                if ($_GET['Fields'] == 'Order_Shipping' || $_GET['Fields'] == 'Order_CartList') {
...
<head>
    ...
    <script>
        ...
        function choujiang() {
            $.get("/api/pintuan/sync/", {"first": 1, "UsersID": "<?=$_SESSION["Users_ID"] ?>"}, function (data) {
                if (!data) {
                    alert("抽奖成功，稍后执行退款操作！");
                    window.location.href = "/api/pintuan/sync.php?UsersID=<?=$_SESSION["Users_ID"] ?>";
                }
            });
        }
    </script>
        ...
        <div id="orders" class="r_con_wrap">
            <form class="search" id="search_form" method="get" action="?">
                <select name="Fields">
                    ...
                    <option value='Address_Name'>姓名</option>
                    <option value='Address_Mobile'>手机号</option>
                    <option value='Order_Shipping'>配送方式</option>
                </select>
                ...
                时间： <input type="text" class="input" name="AccTime_S" value="" maxlength="20"/> - <input type="text" class="input" name="AccTime_E" value="" maxlength="20"/>
                <input type="hidden" value="1" name="search"/> <input type="submit" class="search_btn" value="搜索"/>
            ...
            <form id="submit_form" method="get" action="send_print.php">
                <table border="0" cellpadding="5" cellspacing="0" class="r_con_table" id="order_list">
                    ...
                    <?php
                    $i = 0;
                    $DB->getPages("user_order as u", "u.*, t.teamstatus", $condition, 10);
                    $Order_Status = array(
                        "待确认",
                        "待付款",
                        "已付款",
                    ...
                    while ($rsOrder = $DB->fetch_assoc()) {
                        ...
                        $pro_detail = [];
                        $order_process = 0;
                        foreach ($goodsinfo as $pro_id => $value) {
                            foreach ($value as $attr_id => $attr_value) {
                                $pro_detail = $attr_value;

                                if ($attr_value['Products_IsVirtual'] == 0 && $attr_value['Products_IsRecieve'] == 0) { //实物产品
                                    $order_process = 0;
                                } else if ($attr_value['Products_IsVirtual'] == 1 && $attr_value['Products_IsRecieve'] == 0) {  //虚拟产品
                                    $order_process = 1;
                                } else if ($attr_value['Products_IsVirtual'] == 1 && $attr_value['Products_IsRecieve'] == 1) {  //卡密产品
                                    $order_process = 2;
                                }
                            }
                        }
                        ...
                        <tr>
                            ...
                            <td nowrap="nowrap"><?php echo $rsOrder["Address_Name"] ?></td>
                            <td nowrap="nowrap">
                                ￥<?= $rsOrder["Order_TotalPrice"] ?>
                                <?= $rsOrder["Back_Amount"] > 0 ? '<br/><font style="text-decoration:line-through; color:#999">&nbsp;退款金额：￥' . $rsOrder["Back_Amount"] . '&nbsp;</font>' : "" ?>
                            </td>
                            ...
                            <td nowrap="nowrap"><?php
                                if ($rsOrder['Order_PaymentMethod'] == '线下支付') {
                                    echo "付款待确认";
                                } else {
                                    echo $Order_Status[$rsOrder["Order_Status"]];
                                }
                                ?>
                            </td>
                            <td nowrap="nowrap"><?= date("Y-m-d H:i:s", $rsOrder["Order_CreateTime"]) ?></td>
                            <td class="last" nowrap="nowrap">
                                <a href="<?= $rsOrder["Order_IsVirtual"] == 1 ? 'virtual_orders.php' : 'orders_view.php' ?>?OrderID=<?= $rsOrder["Order_ID"] ?>">[详情]</a>
                                <?php if ($rsOrder['teamstatus'] == 1 && $rsOrder["Order_IsVirtual"] != 1 && $rsOrder['Order_Status'] == 2) { ?>
                                    <a href="orders_send.php?OrderID=<?= $rsOrder["Order_ID"] ?>">[发货]</a>
                                <?php } ?>
                            </td>
                        </tr>
                        <?php $i++;
                ...
                <label style="display: block; width: 120px; border-radius: 5px; height: 32px; line-height: 30px; background: #3AA0EB; color: #FFF; text-align: center; font-size: 12px; cursor: pointer" id="print">打印订单</label> <input type="hidden" name="templateid" value=""/>
            </form>
