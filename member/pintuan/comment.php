...
if (isset($_GET["action"])) {
    if ($_GET["action"] == "del") {
        $Flag = $DB->Del("user_order_commit", "Users_ID='" . $_SESSION["Users_ID"] . "' and MID = 'pintuan' and Item_ID=" . $_GET["ItemID"]);
        if ($Flag) {
        ...
        $DB->Set("user_order_commit", "Status=1", "where Users_ID='" . $_SESSION["Users_ID"] . "' and MID = 'pintuan' and Item_ID=" . $_GET["ItemID"]);
        echo '<script language="javascript">alert("已通过审核");window.location="' . $_SERVER['HTTP_REFERER'] . '";</script>';
        exit;
    } elseif ($_GET["action"] == "uncheck") {
        $DB->Set("user_order_commit", "Status=0", "where Users_ID='" . $_SESSION["Users_ID"] . "' and MID = 'pintuan' and Item_ID=" . $_GET["ItemID"]);
    ...
    <div class="iframe_content">
        ...
        <div id="reviews" class="r_con_wrap">
            <table border="0" cellpadding="5" cellspacing="0" class="r_con_table" id="review_list">
                ...
                $DB->getPage("user_order_commit", "c.*, p.Products_Name", " as c left join shop_products as p on c.Product_ID = p.Products_ID where c.Users_ID='" . $_SESSION["Users_ID"] . "' and c.MID = 'pintuan' order by c.CreateTime desc", "10");
                ...
                foreach ($lists as $k => $rsCommit) {
                    ...
                    <tr>
                        ...
                        <td nowrap="nowrap"><?= $rsCommit["Products_Name"] ?></td>
                        <td nowrap="nowrap"><?= $rsCommit["Score"] ?></td>
                        <td nowrap="nowrap"><?= $rsCommit["Note"] ?></td>
