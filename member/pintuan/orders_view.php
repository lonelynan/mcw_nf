...
if ($_POST) {
    if ($rsOrder["Order_Status"] == 0) {
        ...
        echo '<script language="javascript">alert("订单不是待确认订单");history.back();</script>';
    }
    exit;
}
...
$Order_Status = array("待确认", "待付款", "已付款", "已发货", "已完成","退款中");
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="order_info">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?= $rsOrder["Back_Amount"] > 0 ? '&nbsp;&nbsp;<font style="text-decoration:line-through; color:#999">&nbsp;退款金额：￥' . $rsOrder["Back_Amount"] . '&nbsp;</font>&nbsp;&nbsp;' : "" ?>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td nowrap>支付方式：</td>
                            <td><?php echo empty($rsOrder["Order_PaymentMethod"]) ? "暂无" : $rsOrder["Order_PaymentMethod"]; ?></td>
                        </tr>
                        <tr>
                            <td nowrap>手机号码：</td>
                        ...
                        <?php if (!empty($rsOrder["Order_Shipping"])) {
                            $shipping = json_decode($rsOrder["Order_Shipping"], true);
                            if (!empty($shipping)) {
                                ?>
                                <tr>
                                    <td nowrap>物流：</td>
                                    <td><?php echo $shipping["Express"] ?>（<?= $shipping['Price']; ?>元）</td>
                                </tr>
                            <?php }
                        } ?>
                        <?php if (!empty($rsOrder["Order_ShippingID"])) { ?>
                            <tr>
                                <td nowrap>快递单号：</td>
                                <td><?php echo $rsOrder["Order_ShippingID"] ?></td>
                            </tr>
                        <?php } ?>
                        <tr>
                            <td nowrap>付款信息：</td>
                            <td><?php echo $rsOrder["Order_PaymentMethod"] ?></td>
                        </tr>
                        <tr>
                            <td nowrap>地址信息：</td>
                            <td><?php echo $Province . $City . $Area . $rsOrder["Address_Detailed"] ?></td>
                        </tr>
                        <tr>
                            <td nowrap>订单备注：</td>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="order_item_list">
                    ...
                    foreach ($CartList as $pro_id => $value) {
                        foreach ($value as $attr_id => $attr_value) {
                            ...
                            $total += $attr_value["Qty"] * $goods_price;
                            $qty += $attr_value["Qty"]; ?>
                            <tr class="item_list" align="center">
                                <td valign="top"><img src="<?= $attr_value["ImgPath"] ?>" width="100" height="100"/></td>
                                <td align="center" class="flh_180"><?= $attr_value["ProductsName"] ?></td>
                                <td>￥<?= $goods_price ?></td>
                                <td><?= $attr_value["Qty"] ?></td>
                                <td>￥<?= $goods_price * $attr_value["Qty"] ?></td>
                            </tr>
                        <?php
                        }
                    }
                            ...
                            <tr class="item_list" align="center">
                                ...
                                <td align="left" class="flh_180"><?= $CartList_back["ProductsName"] ?><br>
									...
									if(!empty($CartList_back["Property"])){
										 echo '<dd>' . $CartList_back["Property"]['shu_value'] . ': ' . $CartList_back["Property"]['shu_price'] . '</dd>';
                                ...
                                <td>
                                    ￥<?= $CartList_back["ProductsPriceX"] * $CartList_back["Qty"] ?><br/>
                                    <font style="text-decoration:line-through; font-size:12px; color:#999">&nbsp;退款金额：￥<?= $item["Back_Amount"] ?>&nbsp;</font><br/><?= $_STATUS[$item["Back_Status"]] ?>
                                </td>
                            </tr>
                        <?php 
							}
