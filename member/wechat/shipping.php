...
$rss = $DB->GetRs('shop_shipping_company','count(Shipping_ID) as num',"where Biz_ID=0 and Users_ID= '".$_SESSION['Users_ID']."'");

if($rss['num'] < 21){
	require_once($_SERVER["DOCUMENT_ROOT"].'/include/yinru/defaut_shipping.php');
}
...
{	
	if($action =='add_shipping'){
		...
		$shipping = $DB->getRs('shop_shipping_company','Shipping_ID',$condition);
		
		if($shipping){
			echo '<script language="javascript">alert("添加失败，不能重名");history.back();</script>';
			exit();
		}
		...
		$Flagadd = $DB->Add('shop_shipping_company',$data);	
		if($Flagadd)
		{
			echo '<script language="javascript">alert("添加成功");window.location="shipping.php";</script>';
		} else {
	...
	if($action == 'delete_shipping'){
		mysql_query("BEGIN");
		$Flag1=$DB->Del("shop_shipping_company","Biz_ID=0 and Users_ID='".$_SESSION["Users_ID"]."'");
		if($Flag1)
		{
			mysql_query("COMMIT");
    ...
    $shop_config_data = array();		
	  ...
	  if(!empty($Shipping_List)&&array_key_exists($_GET["Shipping_ID"],$Shipping_List)){
		...
		if($Shop_Config_Change){
			Shop_Config::where('Users_ID',$_SESSION["Users_ID"])->update($shop_config_data);
		}
	  //删除快递公司
...
$condition = "where Biz_ID=0 and Users_ID='".$_SESSION["Users_ID"]."'";
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <script type='text/javascript' src='/static/member/js/shipping.js?t=20180317'></script>
  ...
  <div id="mod_create_shipping" class="lean-modal lean-modal-form">
         ...
         <form class="form" action="shipping.php" method="post" id="create_shipping_form" name="mod_create_shipping">
            ...
            <p class="rows">
            	<label></label>
        		<input type="submit" style="border: none;padding: 10px" value="确定提交" name="submit_btn">
      		</p>
	...
	<div id="mod_delete_shipping" class="lean-modal lean-modal-form">
		...
		<form class="form" action="shipping.php" method="post" id="delete_shipping_form" name="mod_delete_shipping">
			<p class="rows">
				<span> 快递公司数据将删除</span>
			</p>			
			<p class="rows">
