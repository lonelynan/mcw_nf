...
if(isset($_GET["action"])){
	...
	if(empty($rsUsers["Users_WechatAppId"]) || empty($rsUsers["Users_WechatAppSecret"])){
		...
		if(!$ACCESS_TOKEN){
			echo '<script language="javascript">alert("通讯失败");history.back();</script>';
			exit;
		}
	}
	if($_GET["action"]=="delete"){
		...
		include_once $_SERVER["DOCUMENT_ROOT"].'/include/support/tool_helpers.php';
		$ch = curl_init();
    ...
    <div id="wechat_menu" class="r_con_wrap"> 
      ...
      <div class="m_menu">
        <div class="tips_info"> 1. 您的公众平台帐号类型必须为<span>服务号</span>或<span>已通过认证的订阅号</span>。<br />
          ...
          6. 点击"<span>取消发布的菜单</span>"操作只删除微信公众平台上的菜单，并不是删除本系统已经设置好的菜单。 </div>
        <div class="control_btn"><a href="menu_add.php" class="btn_green btn_w_120">添加菜单</a> <a href="menu.php?action=publish" class="btn_green btn_w_120">发布菜单</a>
          <input type="button" class="btn_green btn_w_120" name="del_btn" value="取消发布的菜单" onClick="location.href='menu.php?action=delete'" />
        </div>
