...
function check_right($path,$file,$file_all,$filesub,$file_allsub){	
	...
	$settingArr = $DB->GetRs('setting', 'sys_dommain', "where id = 1");
	if($_SERVER["HTTP_HOST"] != $settingArr['sys_dommain']){
	echo "<div class='un_access' ".$style1."><div ".$style2."><b style='font-size:25px;'>域名非法,请联系管理员</b></div></div>";
	exit;
	}
...
foreach($file_all as $key=>$value){
	...
	foreach ($my_rightcopy as $key=>$val){
		if (in_array($key,['kanjia', 'votes', 'zhuli'])) {
			$lskey = $key;
			$key = 'marketing';
		}
		...
		if(in_array($k,$employee_right[$key])){
			if (in_array($k,['kanjia', 'votes', 'zhuli'])) {			 
			$key = $lskey;
			}
			if(array_key_exists(substr($key,0,3).'_'.$k,$all_subright)){
			foreach($all_subright[substr($key,0,3).'_'.$k] as $ks=>$kv){					
	...
	$jiebase = Array("bt_domain","setting","config","shopping","shipping","index","renewal_record","bmconfig","sms","weixin_config","defautshiptpl","recieve","settingdianzi_print","settingdianzi_printadd","settingdianzi_printedit");
	$jiebasedir = Array("kf","update","bianmin");
	$marketingdir = Array("pintuan","cloud","zhongchou");
	$weixindir = Array("material");
	//var_dump($employee_rights);
	//例外
	if($my_dir == 'wechat'){
		if(in_array($my_file,$jiebase)){
			$my_dir = 'jiebase';
		}else{
			$my_dir = 'weixin';
		}
	}
	if(in_array($my_dir,$jiebasedir)){
			$my_dir = 'jiebase';
		}
	if(in_array($my_dir,$marketingdir)){
			$my_file = $my_dir;
			$my_dir = 'marketing';
		}
	if(in_array($my_dir,$weixindir)){
			$my_dir = 'weixin';
		}
	if($my_dir == 'shop'){			
		if(isset($_SESSION['user_type'])){
