...
if ($_POST) {
	...
	$BizRs = $DB->GetRS('biz','Users_ID,User_Mobile',"where Users_ID='".$_SESSION["Users_ID"]."' and BIZ_ID=".$_POST["BizID"]);
	...
	if (intval($paymentinfo["supplytotal"]) < 0) {
        echo '<script language="javascript">alert("结算不能为负值");history.back();</script>';
        exit();
    }
...
.r_con_form .rows .input {overflow:inherit;}
</style>
	<div id="iframe_page">
	...
	<script>
		...
		function getConfig()
		{
			...
			$.post("/member/finance/payment_add.php",{action:'ajax',Biz_ID:BizID},function(data){
				if(data.status==0){
				//$('input[name!="Time"]').val('');	
				//$('input[name!="Time"]').attr('placeholder','商家没配置');
				$('input[name="submit_btn"]').val('一键生成');
	...
	$("input[name='submit_btn']").submit(function(){
		...
		if(paymentid===1){
			...
			if(openid=="" || openid==null){
				global_obj.win_alert("微信识别码OpenID 不能为空");
				return false;
			}
		} else if (paymentid === 2){
			...
			if ( aliPayNo ==="" || aliPayNo ===null ){
				global_obj.win_alert("支付宝帐号不能为空");
				return false;
			}
			if ( aliPayName ==="" || aliPayName ===null ){
				global_obj.win_alert("支付宝账户名不能为空");
				return false;
			}
		} else if (paymentid === 3){
			...
			if ( Bank ==="" || Bank === null ){
				global_obj.win_alert("银行类型不能为空");
				return false;
			}
			if ( BankNo ==="" || BankNo === null ){
				global_obj.win_alert("银行卡号不能为空");
				return false;
			}  
			if ( BankName ==="" || BankName === null ){
				global_obj.win_alert("收款人不能为空");
				return false;
			} 
			if ( BankMobile ==="" || BankMobile === null ){
				global_obj.win_alert("收款人手机不能为空");
				return false;
			}   
		}
	});
