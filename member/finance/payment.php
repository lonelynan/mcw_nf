...
if (isset($_GET["action"])) {
    if ($_GET["action"] == "del") {
        ...
        $Flag = $DB->Del("shop_sales_payment", "Payment_ID=" . $paymentid);        
		if($Flag){
			echo '<script language="javascript">alert("删除成功");window.location.href="payment.php";</script>';
		}else{
    ...
    if ($_GET["action"] == "okpay") {
        ...
        $Flagone = $DB->Set("shop_sales_payment", array(
                "Status" => 2
            ), "WHERE Payment_ID='{$paymentid}'");
            $Flagtwo = $DB->Set("shop_sales_record",array("Record_Status"=>2),"WHERE Payment_ID='{$paymentid}'");
		if($Flagone&&$Flagtwo){
			echo '<script language="javascript">alert("操作成功");window.location.href="payment.php";</script>';
		}else{
			echo '<script language="javascript">alert("操作失败");history.back();</script>';
		}
    }
}
if(!empty($_GET['Payment_Sn'])){
	$condition .= " and Payment_Sn=".$_GET["Payment_Sn"];
}
...
if ($_POST) {
    ...
    $pay_price = round(($sPayment['Total']*$PaymenteRate/100), 2);
...
">
        ...
        foreach ($lists as $paymentid => $value) {
         ...
         <tr>
							...
							<td nowrap="nowrap"><?php echo '<a href="/member/distribute/withdraw.php?Payment_Sn='.$value["Payment_Sn"].'">'.$value["Payment_Sn"];?></a></td>
							<td nowrap="nowrap"><?php echo date("Y-m-d H:i:s",$value["FromTime"]);?><br />~<br /><?php echo date("Y-m-d H:i:s",$value["EndTime"]);?></td>
							<td nowrap="nowrap"><font style="color: #F60"><?php echo $value["Amount"] + $value["Shipping"];?></font></td>
           ...
           <?php echo"+转向余额";echo $value["Total"]-($value["Total"]*$BizPayRate[$value["Biz_ID"]]/100); echo ")"?>
							...
							<td nowrap="nowrap" style="text-align: center;" paymentid="<?php echo $value["Payment_ID"];?>">
			...
			<td nowrap="nowrap" style="display: none;"><?php echo $value["Payment_ID"];?></td>
                			...
                			<?php if($value["Status"]==0 || $value["Status"]==3){?><a
								href="?action=del&paymentid=<?php echo $paymentid;?>" onClick="if(!confirm('删除后不可恢复，继续吗？')){return false};">[删除]</a><?php }?>
							<?php if($value["Status"]==3 && ($value['Payment_Type']==2 || $value['Payment_Type']==3)){?>
                <a
								href="?action=okpay&paymentid=<?php echo $paymentid;?>" onClick="if(!confirm('确定要打款？')){return false};">[确认打款]</a>&nbsp;
							<?php } ?>
            </td>
						</tr>
         <?php }?>
...
<script type='text/javascript'>
	$(".weixin").click(function(){
		  var Payment_ID = $(this).parent().attr("paymentid");
		  if(confirm("您确定需要支付吗？")){
			$.ajax({
               type: "POST",
               url: "?",
