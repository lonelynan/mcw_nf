...
if($_POST){
	if(!isset($_POST["price"])){
		echo '<script language="javascript">alert("请设置价格");history.back();</script>';
		exit;
	}
	if(!isset($_POST["JSON"])){
		...
		<div id="products" class="r_con_wrap">
			...
			<form class="r_con_form" id="product_edit_form" method="post" action="products_edit.php">
				<div class="rows">
				  <label>排序</label>
				  <span class="input">
				  <input type="text" name="Index" value="<?php echo $rsProducts["Products_Index"] ?>" class="form_input" size="20" maxlength="100" notnull />
...
?>
				...
				<div class="rows">
					...
					<span>%</span>
					<input type="text" name="commission_ratio" value="<?=$rsProducts["commission_ratio"]?>" class="form_input" size="5" maxlength="10" notnull />
               		...
               		<?php 
						...
						<tr>
							<td><?php if($i==$level-1 && $shop_config['Dis_Self_Bonus']){?>自销&nbsp;&nbsp;%<?php }else{?><?php echo $arr[$i]?>级&nbsp;&nbsp;%<?php }?>
								<input name="Distribute[<?php echo $i;?>]" value="<?=!empty($distribute_list[$i])?$distribute_list[$i]:0?>" class="form_input" size="5" maxlength="10" type="text">
								(佣金利润的百分比)
							</td>
						</tr>
					<?php }?>
