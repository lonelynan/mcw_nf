...
$template_dir = $_SERVER["DOCUMENT_ROOT"].'/member/pifa/html';
...
if($_POST)
{
	...
	$Data=array(
		...
		"Products_Index"=>$_POST['Index']
	);
	
  	
	$Flag=$DB->Add("pifa_products",$Data);
	$product_id = mysql_insert_id();
	
	$product_url = base_url().'api/'.$_SESSION['Users_ID'].'/pifa/products/'.$product_id.'/';
	$qrcode = array(
	...
	$DB->set("pifa_products",$qrcode,$condition);
...
$(document).ready(pifa_obj.products_init);
...
$(document).ready(pifa_obj.products_add_init);
		...
		<div id="products" class="r_con_wrap">
			...
			<form id="product_add_form" class="r_con_form" method="post" action="products_add.php">
				<div class="rows">
				  <label>排序</label>
				  <span class="input">
				  <input type="text" name="Index" value="1" class="form_input" size="20" maxlength="100" notnull />
				  <font class="fc_red">*</font>填写数字</span>
				  <div class="clear"></div>
				</div>
				<div class="rows">
					<label>产品名称</label>
...
?>
				...
				<div class="rows">
					<label>库存</label>
					<span class="input">
					<input type="text" name="Count" value="10000" class="form_input" size="5" maxlength="10" />
				...
				<div class="rows">
					<label>产品单位</label>
					<span class="input">
					<input type="text" name="unit" value="件" notnull class="form_input" size="5" />
					</span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					...
					价格:
					<input type="text" name="price[]" value="" class="form_input" size="5" maxlength="10" />￥&nbsp;<a><img hspace="5" src="/static/member/images/ico/del.gif"></a>
					</li>
					</ul>
					</span>
					<div class="clear"></div>
				...
				<div class="rows">
				    ...
				    <span>(佣金所占利润百分比，利润分为佣金和爵位奖励两部分)</span>
				    </span>
				    <div class="clear"></div>
				</div>
				<div class="rows">
					...
					for($i=0;$i<$level;$i++){?>
					<tr>
						<td><?php if($i==$level-1 && $shop_config['Dis_Self_Bonus']){?>自销&nbsp;&nbsp;%<?php }else{?><?php echo $arr[$i]?>级&nbsp;&nbsp;%<?php }?>
				...
				<div class="rows">
					<label>批发须知</label>
					<span class="input">
					<textarea name="BriefDescription" class="ckeditor" style="width:600px; height:300px;"></textarea>
					</span>
					<div class="clear"></div>
				</div>
