...
if($action == 'reject_withdraw'){//申请提现驳回
	...
	$rsRecord = $DB->GetRs("distribute_withdraw_record","Record_Total,User_ID,Record_Status,Method_Name,Record_Yue",$condition);
	$conditionuser = "where Users_ID='".$_SESSION["Users_ID"]."' and User_ID='".$rsRecord['User_ID']."'";
	$rsUser = $DB->GetRs("user","User_Money",$conditionuser);
	...
	//将钱退回
	if($rsRecord['Method_Name']=='余额提现'){
		$money = $rsRecord['Record_Total'];
	}elseif(empty($rsRecord['Record_Yue'])){
		$money = $rsRecord['Record_Total'];
	}else{
		$money = $rsRecord['Record_Total'];
		$Data = array(
		"User_Money"=>$rsUser["User_Money"] - $rsRecord['Record_Yue']
	);
	$SETU = $DB->set('user',$Data,$conditionuser);
	}
//jiechuli
	...
	echo json_encode($response,JSON_UNESCAPED_UNICODE);
}elseif($action == 'reject_salesmansq'){//创始人申请驳回
	$Record_ID = $_POST['Record_ID'];

	//获取此次提现记录内容
	$condition = "where Users_ID='".$_SESSION["Users_ID"]."' and Record_ID='".$Record_ID."'";
	$rsRecord = $DB->GetRs("distribute_smansq_record","Record_Status",$condition);
	...
	//将此次提现状态改为驳回
	$data = array("Record_Status"=>2,"No_Record_Desc"=>$_POST['Reject_Reason'],"Record_Notime"=>time());	
	$Flag = $DB->Set("distribute_smansq_record",$data,$condition);
	if($Flag){
		$response = array("status"=>1,"msg"=>"驳回成功");
	}else{
		$response = array("status"=>0,"msg"=>'驳回失败，出现未知错误');
	}
	...
	// 判断是否有其他人代理（一个地区只能有一个代理）
    $res = agent_area_check($area_id_list, $Account_ID, $UsersID);
    if (!isset($res['status']) || $res['status'] != 1) {
        $response = [
            'status' => 0,
            'msg' => empty($res['msg']) ? '有地区已被代理，不能选择' : $res['msg']
        ];
        echo json_encode($response,JSON_UNESCAPED_UNICODE);
    }
	...
	$DB->Set("distribute_config","Distribute_Type=".$type,"where Users_ID='".$UsersID."'");
