...
$rsOrder = $DB->GetRs('agent_order','o.*, d.Account_ID',' as o left join distribute_account as d on o.User_ID = d.User_ID where o.Order_ID='.$OrderID);
...
if($_POST){
	if($_POST["refuse"] == 1){
        // 检查此地区是否已被代理
        $res = agent_area_check([ empty($rsOrder['AreaId']) ? (empty($rsOrder['CityId']) ? $rsOrder['ProvinceId'] : $rsOrder['CityId']) : $rsOrder['AreaId'] ],$rsOrder['Account_ID'], $_SESSION['Users_ID']);
        if (!isset($res['status']) || $res['status'] != 1) {
            echo '<script language="javascript">alert("' . (empty($res['msg']) ? '检查地区是否已被代理失败' : $res['msg']) . '");</script>';
            exit;
        }
		$Data=array(		
            "Order_Status"=>1
