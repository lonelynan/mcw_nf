...
{	
	...
	if($action == "fullfill"){
		...
		begin_trans();
		$withdraw_data = $capsule->table('distribute_withdraw_record')->where(['Users_ID' => $UsersID, 'Record_ID' =>$RecordID ])->first();
		if(empty($withdraw_data)){
		    die('<script language="javascript">alert("相应的提现记录不存在");history.back();</script>');
		}
        ...
        $rsUser = $objUser->toArray();
        $Flag = true;
		if($withdraw_data['Method_Name'] == '余额提现'){
    		$objUser->User_Money = $rsUser['User_Money']+$amount;
    		$Flag_user = $objUser->save();
    		$Flag = $Flag && $Flag_user;
    		$Data=array(
    		    'Users_ID'=>$UsersID,
    		    'User_ID'=>$UserID,
			...
			$Flag_UserMoneyRecord = $capsule->table('user_money_record')->insert($Data);
    		$Flag = $Flag && $Flag_UserMoneyRecord;
		}else{
		    if($withdraw_data['Record_Yue']>0){
		        $objUser->User_Money = $rsUser['User_Money']+$withdraw_data['Record_Yue'];
		        $Flag_user = $objUser->save();
		        $Flag = $Flag && $Flag_user;
		    }
		}
		$FlagDis = $capsule->table('distribute_withdraw_record')->where(['Users_ID' => $UsersID, 'Record_ID' =>$RecordID ])->update(['Record_Status' => 1]);
		$Flag = $Flag && $FlagDis;
		if($Flag)
		{
		    commit_trans();
		...
		if($action == "wx_hongbao"){
			...
			if($rsRecord["Record_Money"]<0){
				echo '<script language="javascript">alert("金额必须大于零才可使用微信转账");history.back();</script>';
				exit;
			}
		}
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        <thead>
          <tr>
			...
			<td width="8%" nowrap="nowrap">提现描述</td>
