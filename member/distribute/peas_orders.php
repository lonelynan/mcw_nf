...
if(isset($_GET["action"]))
{
	...
	if($action=="del")
	{
		$Flag=true;
		$msg="";
		$Del=$DB->Del("user_peas_orders","Users_ID='".$_SESSION["Users_ID"]."' and Orders_ID=".$_GET["OrderId"]);
		$Flag=$Flag&&$Del;
		if($Flag){
...
if(isset($_GET["search"])){
	if($_GET["search"]==1){
		if(!empty($_GET["Keyword"])){
			if($_GET["Fields"] == 'Peas_Name'){
				$peas_data = $DB->GetRs("user_peas","*","where Peas_Name like '%".$_GET["Keyword"]."%'");
				$condition .= " and Peas_ID =".$peas_data["Peas_ID"];
			}else{
   ...
   <?php require_once($_SERVER["DOCUMENT_ROOT"].'/member/distribute/distribute_menubar.php');?>
    <div id="peas_orders" class="r_con_wrap"> 
	 ...
	 <form class="search" id="search_form" method="get" action="?">
        <div class="l">
		  <select name="Fields">
		    <option value="Peas_Name">产品名称</option>
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        <thead>
          <tr>
            ...
            <td width="10%" nowrap="nowrap">姓名/微信名</td>
        ...
        <tbody>
        <?php $DB->getPage("user_peas_orders","*",$condition,$pageSize=10);
		$i=1;
		$lists = array();
		...
		foreach($lists as $k=>$v){
			$peas = $DB->GetRs("user_peas","*","where Peas_ID=".$v['Peas_ID']."");
			$v['Peas_Name'] = $peas['Peas_Name'];
			$v['Peas_ImgPath'] = $peas['Peas_ImgPath'];
			?>
          <tr>
            <td nowrap="nowrap"><?php echo $pageSize*($DB->pageNo-1)+$i; ?></td>
            <td><?php echo $v['Peas_Name'];?></td>
            <td nowrap="nowrap"><img src="<?php echo $v['Peas_ImgPath']?>" class="img" /></a></td>
