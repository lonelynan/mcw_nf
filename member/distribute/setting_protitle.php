...
if($_POST){	
	...
	$distribute_config->Title_Dislevel = $_POST['TitleDis_Level'];
	...
	$rsConfig = $DB->GetRs('distribute_config','Pro_Title_Level,Pro_Title_Status,Title_Dislevel','where Users_ID="'.$_SESSION['Users_ID'].'"');
	if(!$rsConfig){
		$Data = array(
...
<link href='/static/member/css/main.css?t=2' rel='stylesheet' type='text/css' />
    ...
    <form method="post" action="?" id="distribute_config_form">
	   <table class="level_table" id="dis_pro_title_table" border="0" cellpadding="5" cellspacing="0">
			...
			<tr>
                ...
                <td colspan="6">
                	<select name="TitleDis_Level" class="tiledis">
                    <?php for($i=1;$i<=9;$i++):?>
                    	<option value="<?=$i?>" <?=($rsConfig["Title_Dislevel"]==$i)?'selected':''?>><?=$i?>级</option>
                    <?php endfor;?>
           		</select>
                </td>
            </tr>
