      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        <thead>
          <tr>
            ...
            <td width="20%" nowrap="nowrap">微信昵称</td>           
		...
		<?php
		  ...
		  $DB->query("select Account_ID,area_id from distribute_agent_areas ".$agentition);
		  $agent_rec = array();		  
		  $agentrecarr = array();		 
		  $agentrecarr = $DB->toArray();
		  foreach($agentrecarr as $key=>$aeid){			  
			  $aemoney = $DB->GetRs("distribute_agent_rec","SUM(Record_Money) as money",$agentition." and Account_ID=".$aeid['Account_ID']." group by Account_ID");
			$agent_rec[$aeid['area_id']] = $aemoney['money'];
		}
