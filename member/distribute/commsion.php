...
$condition = ' as r left join distribute_account as dis on r.User_ID = dis.User_ID WHERE r.Users_ID="' .$_SESSION['Users_ID']. '"';
	...
	if ($_GET['search'] == 1){
		if(!empty($_GET["Keyword"])){
			...
			if($_GET['Fields']=='User_ID'){
				...
				if(!empty($user_ids)){
					$condition .= " and r.User_ID in(".implode(',',$user_ids).")";
				}
				...
				if(!empty($user_ids)){
					$condition .= " and r.Buyer_ID in(".implode(',',$user_ids).")";
				}
			}
	    ...
	    if(!empty($_GET["AccTime_S"])){
	      $condition .= " and r.Record_CreateTime>=".strtotime($_GET["AccTime_S"]);
	    }
	    if(!empty($_GET["AccTime_E"])){
	      $condition .= " and r.Record_CreateTime<=".strtotime($_GET["AccTime_E"]);
	    }
	}
}

$condition .= ' ORDER BY r.Order_ID DESC, r.level ASC';

$recordList = array();//分销记录数组
$userids = array();//用户ID集
$orderids = array();//订单ID集
$DB->getPage('distribute_order_record', 'r.*, dis.Level_ID', $condition,10);
while($r = $DB->fetch_assoc()){
	$recordList[] = $r;
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
		...
		<?php foreach ($recordList as $key => $value) : if($orderid!=$value['Order_ID']){$orderid=$value['Order_ID']; $i++;}?>
			<tr>
				...
				<td><?php if (in_array($value['Level_ID'], array_keys($levelList))) { echo $levelList[$value['Level_ID']]['Level_Name']; } else { echo "无级别"; } ?></td>
