...
if($_POST){
    ...
    if($_POST['Sha_Agent_Type'] == 0){
        $Sha_Rate = '';
        $distribute_config->Sha_People = 0;
        $distribute_config->Sha_Money = 0.00;
    }elseif($_POST['Sha_Agent_Type'] == 1){
        $Sha_Rate = json_encode($_POST['Sha_Rate'],JSON_UNESCAPED_UNICODE);
        $distribute_config->Sha_People = $_POST['Sha_People'];
        $distribute_config->Sha_Money = $_POST['Sha_Money'];
    }
...
<head>
    ...
    <script type='text/javascript' src='/static/member/js/distribute/config.js?data=2'></script>
        ...
        <div class="r_con_wrap">
           ...
           <span class="tips">&nbsp;&nbsp;(设置分销商能否自定义店名与头像)</span>
                ...
                <div class="rows">
                    <label>昵称后的“的店”</label>
                    <span class="input">
            	<input type="text" name="nicheng_after" value="<?php echo $rsConfig["nicheng_after"];?>" class="form_input" size="8" maxlength="10" /> <span class="tips">&nbsp;注:图片上昵称后的字符.</span>
            </span>
                    <div class="clear"></div>
                </div>
           ...
           <input type="radio" id="d_1" name="Agent_Rate[Nameshow]"  value="0" <?php echo (isset($Agent_Rate_list['Nameshow'])?$Agent_Rate_list['Nameshow']:'') == 0 ? ' checked' : '';?>/><label for="d_1">隐藏</label>	   			
                ...
                <div class="rows" id="Sha_People" <?php echo $rsConfig["Sha_Agent_Type"] > 0 ? ' style="display:block"' : ' style="display:none"'?>>
                    <label>股东最大人数</label>
                    <span class="input">
            	<input type="text" name="Sha_People" value="<?php echo $rsConfig["Sha_People"];?>" class="form_input" size="8" maxlength="10" /> <span class="tips">&nbsp;注:超过此数值将无法再申请股东.</span>
            </span>
                    <div class="clear"></div>
                </div>

                <div class="rows" id="Sha_Money" <?php echo $rsConfig["Sha_Agent_Type"] > 0 ? ' style="display:block"' : ' style="display:none"'?>>
                    <label>最大金额</label>
                    <span class="input">
            	<input type="text" name="Sha_Money" value="<?php echo $rsConfig["Sha_Money"];?>" class="form_input" size="8" maxlength="10" /> <span class="tips">&nbsp;注:股东所得分红累计的最大值.单位元.</span>
            </span>
                    <div class="clear"></div>
                </div>
