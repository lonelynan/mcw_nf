...
if($_POST){
	...
	if(!empty($_POST['Level_Description'])){
	...
	if($type==0){//直接购买
		...
		if($_POST['Fanwei']==1){
			if(empty($_POST['BuyIDs']) && $_POST['Update_switch'] != 0){
      ...
      if(empty($_POST['product_id']) && $_POST['Update_switch'] != 0){
        echo '<script type="text/javascript">alert("请选择商品! ");history.back();</script>';
        exit;
      }
	...
	//升级相关
	if ($type != 1) {
	if(($_POST['Update_Type']==0 || $_POST['Update_Type']==2) && $_POST['Update_switch'] != 0 ){//价格
	...
	$Data['Update_switch'] = $_POST['Update_switch'] ? $_POST['Update_switch'] : 0;
	}	
...
$firstone = $DB->GetRs('distribute_level','Level_LimitType,Present_type','where Users_ID="'.$_SESSION["Users_ID"].'" order by Level_ID asc');//获取第一个分销商级别
$first = $firstone;
$firsttwo = [];
if ($firstone['Level_LimitType'] == 3) {
	$firsttwo = $DB->GetRs('distribute_level','Level_ID,Level_LimitType,Present_type','where Level_LimitType !=3 and Users_ID="'.$_SESSION["Users_ID"].'" order by Level_ID asc');//获取第一个分销商级别
	$first = $firsttwo;
}
$Present_type = array(0=>'直接购买会员',1=>'送产品',2=>'存入余额');
$level_data = get_dis_level($DB, $_SESSION["Users_ID"]);
$realtwo['Update_switch'] = 1;
if (count($level_data) > 1) {
	$realtwo = next($level_data);
}
?>
<!DOCTYPE HTML>
<html>
...
<script type='text/javascript' src='/static/member/js/distribute/dis_level.js?t=1'></script>
    ...
    <div id="level" class="r_con_wrap">
      <form id="level_form" class="r_con_form" method="post" action="?">
			...
			<div class="rows">
			  ...
			  <span class="input">
					<select name="Present_type" id="Present_type">
						<?php if (isset($firsttwo)) { ?>
						<?php if ($firsttwo) { ?>
						<?php foreach($Present_type as $key=>$value){							
							if($firsttwo['Present_type'] == $key){ ?>
        ...
        <div class="rows">
		  ...
		  <?php if($first['Present_type'] == 2){?>
				<input name='Price' value='' type='text' class='form_input' size='5'  > 元 赠送 <input type='text' name='Present_Price' value='' class='form_input' size='5' maxlength='10' /> 元<span "+"class='tips'>&nbsp;&nbsp;注：用户购买此级别的价格
		  <?php }else{ ?>
      ...
      <div class="rows">
        ...
        <div class="rows">
               ...
               <span class="input">
                ...
                <div class="products_one" style="display:none">
                  ...
                  <div class="select_item" >
                     <select class="select_product" name="product_id" style="float:left" ></select>
                  </div>
        ...
        <?php }
		if ($realtwo['Update_switch'] && $type != 1) {
		...
		<input name="Update_switch" value="0" id="upswi_0" type="radio" checked ><label for="upswi_0">关闭</label>&nbsp;&nbsp;<input name="Update_switch" value="1" id="upswi_1" type="radio" ><label for="upswi_1">开启</label>&nbsp;&nbsp;
		<?=$tipupswitch?></h2>
        <div class="rows"id="update_zd">
          <label>升级门槛</label>
          <span class="input">
           <input name="Update_Type" value="0" id="update_0" type="radio" checked><label for="update_0">补差</label>&nbsp;&nbsp;
		   <input name="Update_Type" value="2" id="update_2" type="radio" checked><label for="update_2">指定</label>&nbsp;&nbsp;
	   <?php if($type == 2){?>
	   ...
	   <?php }?>
        ...
        <div id="update_div_0">
        	<div class="rows">
			  ...
			  <?php if($first['Present_type'] == 2){?>
					<input name='UpdatePrice' value='' type='text' class='form_input' size='5' > 元 赠送 <input type='text' name='Present_UpdatePrice' value='' class='form_input' size='5' maxlength='10' /> 元<span class='tips'>&nbsp;&nbsp;注：会员升级到本级别时所需支付的金额
			  <?php }else{?>
...
<script  type='text/javascript'>
	...
	$("#Present_type").change(function(){
		...
		if(Present_type == 2){
			$("#Prices").empty();
			$("#UpdatePrices").empty();
			$("#Prices").append("<input name='Price' value='' type='text' class='form_input' size='5'  > 元 赠送 <input type='text' name='Present_Price' value='' class='form_input' size='5' maxlength='10' /> 元<span "+"class='tips'>&nbsp;&nbsp;注：用户购买此级别的价格");	
			$("#UpdatePrices").append("<input name='UpdatePrice' value='' type='text' class='form_input' size='5' > 元 赠送 <input type='text' name='Present_UpdatePrice' value='' class='form_input' size='5' maxlength='10' /> 元<span class='tips'>&nbsp;&nbsp;注：会员升级到本级别时所需支付的金额");
		}else{
			$("#Prices").empty();
			$("#UpdatePrices").empty();
			$("#Prices").append("<input name='Price' value='' type='text' class='form_input' size='5' > 元<span "+"class='tips'>&nbsp;&nbsp;注：用户购买此级别的价格");	
			$("#UpdatePrices").append("<input name='UpdatePrice' value='' type='text' class='form_input' size='5' > 元 <span class='tips'>&nbsp;&nbsp;注：会员升级到本级别时所需支付的金额");
		}
	});
</script>
