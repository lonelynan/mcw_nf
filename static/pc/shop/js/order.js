...
var order_obj = {
	order_init : function() {
		...
		$('.add-substract-key').click(function() { //更新购物车
            ...
            $.post(shop_ajax_url, {action:'cart_update',ProductsID:ProductsID,CartID:CartID,Type:type,Qty:qty},
            function(data) {
        ...
        $('input[rel=qty_input]').keyup(function() {
            ...
            $.post(shop_ajax_url, {action:'cart_update',ProductsID:ProductsID,CartID:CartID,Type:'qty_input',Qty:qty},
            function(data) {
                if (data.status == 2) { //更新购物车成功                    
                    obj.parent().children('input').attr('value', data.qty);
					obj.parents('.shop-list').find('em[wzw_type=eachGoodsTotal]').html(data.xiaoji);
        ...
        $('.delCart').click(function() {
            ...
            var ProductsID = obj.parents('.shop-list').attr("ProductsID");
            var CartID = obj.parents('.shop-list').attr("CartID");
            $.post(shop_ajax_url, {action:'cart_del',ProductsID:ProductsID,CartID:CartID},
            function(data) {
		...
		$(".shipping_method").click(function(){
			$.post(shop_ajax_url,{action:'get_shipping_list'},function(data){
				if(data.status == 1) {
					var htmlTmp = '';
					$.each(data['shiping_company_dropdown'],function(i) {
						var v = data['shiping_company_dropdown'][i];
						   htmlTmp += '<label>'+
									'<div class="pull-left shipping-company-name">'+
										v+
									'</div>'+
									'<div class="pull-right">'+
										'<input type="radio" '+((data['Default_Shipping'] == i) ? 'checked' : '')+' class="Shiping_ID_Val" name="Shiping_ID" value="'+i+'" />'+
									'</div>'+
									'<div class="clearfix"></div>'+
								'</label>';
						});
						var button = '<div id="shipping_ok">确定</div>';
						show_dialog('shipping', '配送方式', function() {
				            $('#shipping_dialog').html(htmlTmp+button);
							$('#shipping_dialog').on('click','#shipping_ok',function(){
								order_obj.change_shipping_method();
								DialogManager.close('shipping');
							});
			            });
					};
				}, 'json');
	...
	change_shipping_method : function(){
		...
		var CouponID = order_obj.getCouponID();
		var param = {
		...
		$.post(shop_ajax_url, param, function(data) {
			if(data.status == 1) {
				if (parseFloat(data.total_shipping_fee) == 0) {
                    $('#shipping_fee_txt').html('免运费');
                } else {
                    $('#shipping_fee_txt').html(data.total_shipping_fee + '元');
                }
				$('#shipping_name').html(data.Shipping_Name);
				$('#Shipping_ID').val(data.total_shipping_fee);
                $(".wzw-all-account em").html(parseFloat(data.total) + parseFloat(data.total_shipping_fee));
				if (data.man_info.reach && data.man_info.award) {
                    $("#reach_txt").html(data.man_info.reach);
					$("#award_txt").html('&yen' + data.man_info.award);
                }
	...
	change_coupon : function(){
		var CouponID = order_obj.getCouponID();
		var action = 'change_coupon';
		var param = {
			CouponID:CouponID,
			cart_key:$('input[name=cart_key]').val(),
			Shipping_ID:order_obj.getShippingID(),
		...
		$.post(shop_ajax_url, param, function(data) {
			if(data.status == 1) {
                $('.wzw-all-account em').html(parseFloat(data.total + data.total_shipping_fee));
			}
		}, 'json');
	},
