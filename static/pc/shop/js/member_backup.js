				...
				success:function(data){
					if(data['list'] != '') {
						...
						$.each(data['list'],function(i) {
							...
							if (v['CartList_back']['ProductsPriceAamx'] == 0) {
								vprice = v['CartList_back']['ProductsPriceX'];
							}else{
								vprice = v['CartList_back']['Productsattrkeystrval']['Txt_PriceSon'];
							}
							
							htmltmp +=  '<div class="dingdan"><div class="dd_more"> <span class="name"><img title="'+v['CartList_back']['ProductsName']+'" src="'+v['CartList_back']['ImgPath']+'"><div><a href="'+v['P_URL']+'">'+v['CartList_back']['ProductsName']+'</a><br>'+v['CartList_back']['Productsattrstrval']+'</div></span> <span class="price"><i>'+v['CartList_back']['ProductsPriceY']+'</i><br>'+vprice+'</span> <span class="num">'+v['CartList_back']['Qty']+'</span><span class="truepay">￥'+v['Back_Amount']+'</span> <span class="on"><i>'+v['status_arr'][v['Back_Status']]+'</i><br><a href="'+v['detail_url']+'">订单详情</a></span> <span class="paydo">'+paydo_html+'</span> </div></div>';
						});
