$(document).ready(function(){
	...
	//属性特效
	/*$('#addtocart_form').find('.spec').each(function(){
	    var that = this;
		var names = '';
		if($(that).find('a').is(':hidden')){
		    $(that).find('a').each(function(){
			     names += (',' + $(this).attr('name') + ',');
			     $(that).find('a:first').attr('name', names);
			});
		}
	});*/
});
...
var goods_obj = {
	goods_init : function(){
        ...
        function add_to_cart(cart_key) {			
			...
			var selectbe = 0;
			var msg = '';
			$('.sys_item_spec dl').each(function () {
				keystr = $(this).attr("data-attrval");
				attrkey = $(this).find('dt').html();
				attrval = $(this).find('dd ul li[class=selected] a').html();
				attrstr += attrkey + ':' + attrval + ' ';
				attrkeystr += keystr + ';';
				selectbe += 1;
				});				
				if (attrkeystr != '') {
					selectbe *= 2;
				if (selectbe != attrkeystr.length) {
					msg = '请选择商品属性';
					status = false;
				}
				if(msg.length > 0){
				new Toast({
                        context: $("body"),
                        message: msg,                        
                        top: $("#bold_num").offset().top + 20,
                        time: 4000
                    }).show();
					return status;
					}
					}
					attrimg = $('.bigImg img').attr("src");
            $.post(shop_ajax_url, {action:'add_to_cart',ProductsID:Products_ID,Qty:$('input[name=Qty]').val(),spec_list:$('input[name=spec_list]').val(),attrkeystrval:attrkeystr,attrstrval:attrstr,attrimgval:attrimg,cart_key:cart_key},
            function(data) {
                if (data.status == 1) {
