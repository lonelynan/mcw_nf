				...
				success:function(data){
					if(data['list'] != '') {
						...
						$.each(data['list'],function(i) {
							v = data['list'][i];
							if (v['ProductsPriceAmax'] == 0) {
								vprice = v['ProductsPriceX'];
							}else{
								vprice = v['Productsattrkeystrval']['Txt_PriceSon'];
							}
							//操作按钮
							var paydo_html = '';
							if(v['Order_Status'] == 0) {
								...
								if (v['product_num'] > 1) {
									paydo_html = '<a href="'+v['detail_url']+'">查看</a>&nbsp;&nbsp;<a href="'+v['detail_url']+'">申请退款</a>';
                                                               
                                                            } else {
                            paydo_html = '<a href="'+v['detail_url']+'">查看</a>&nbsp;&nbsp;<a href="'+v['backup_url']+'">申请退款</a>';
                                                            }
							...
							if(v['ProductsPriceAmax'] != 0) {								
									htmltmp +=	'<span>'+v['Productsattrstrval']+'</span>';								
							}
							htmltmp +=  '</div>'+
												'</span> <span class="price"><i>'+v['ProductsPriceY']+'</i><br />'+
												''+vprice+'</span> <span class="num">'+v['ProductsQty']+'</span><span class="truepay">￥'+v['Order_TotalPrice']+'</span> <span class="on"><i>'+v['status_arr'][v['Order_Status']]+'</i><br />'+
