...
function update_datacay(id) {//更新	
	var get_textcay = $.ajax({
		type: "POST",
		url: SITE_URL+'/pc.php/member/web_api/code_updatecay',
		data: $("#"+id+"_form").serialize(),
		async: false
		}).responseText;
	return get_textcay;
...
function update_category() {//更新分类
	var get_textcay = update_datacay("category_list");
	switch (get_textcay) {
		case '1':
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id > 0) {
		...
		$("#show_recommend_goods_list").load(SITE_URL+'/pc.php/member/web_api/recommend_list/id/' + gc_id);	
		}
	}
}
