	...
	user_profile_init:function(){
		$('#user_form .submit input').click(function(){
			...
			$.post('../../ajax/', $('#user_form').serialize(), function(data){
				if(data.status==1){
					global_obj.win_alert(data.msg, function(){
						window.location='../../../shop/member/setting/?love';
					});
				}else{
					global_obj.win_alert(data.msg, function(){
						$('#user_form .submit input').attr('disabled', false);
	...
	user_paymoney_init:function(){
		$('#user_form .submit input').click(function(){
			...
			$.post('../ajax/', $('#user_form').serialize(), function(data){
				if(data.status==1){
					global_obj.win_alert(data.msg, function(){
						window.location='../../shop/member/?love';
					});
				}else{
					global_obj.win_alert(data.msg, function(){
						$('#user_form .submit input').attr('disabled', false);
	...
	my_init:function(){
		$('.modify_password').click(function(){
			...
			$('#modify_password_form .submit').off().click(function(){
				...
				$(this).attr('disabled', true);
				$.post('../../../user/ajax/', $('#modify_password_form').serialize()+'&action=modify_password', function(data){
					if(data.status==1){
						global_obj.win_alert(data.msg, function(){
							global_obj.div_mask(1);
							$('#modify_password_div').hide();
	...
	coupon_init:function(){
		$('#coupon .use').click(function(){
			...
			$('#coupon_use_form .submit').off().click(function(){
				if(global_obj.check_form($('*[notnull]'))){return false};
				$(this).attr('disabled', true);
				$.post('/api/'+UsersID+'/user/ajax/', $('#coupon_use_form').serialize()+'&action=use_coupon&CouponID='+o.attr('CouponID'), function(data){
					if(data.status==1){
						global_obj.win_alert(data.msg, function(){
		...
		$('#coupon, .layer_coupon').on('click','.get',function(){
            var o=$(this);
            o.html('领取中...');
            $.post('/api/' + UsersID + '/user/ajax/', 'action=get_coupon&CouponID='+o.attr('CouponID'), function(data){
                o.html('领取');
                if(data.status==1){
                    global_obj.win_alert(data.msg, function(){
                        window.location='/api/' + UsersID + '/user/coupon/';
                    });
			...
			$.post($('#commit_form').attr('action')+'kanjia_ajax/', $('#commit_form').serialize(), function(data){
				...
				if(data.status==1){
					global_obj.win_alert('评论成功!', function(){
						window.location=$('#commit_form').attr('action')+'kanjia_order/status/4/';
					});					
				}else{
					global_obj.win_alert(data.msg);
				}
