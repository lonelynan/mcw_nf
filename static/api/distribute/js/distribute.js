...
var distribute_obj = {
	...
	withdraw_page:function(){
        ...
        $("#btn-withdraw").click(function() {
			...
			if($("input[name='money']").val().length == 0){
			   ...
			   $("#withdraw_tip").html("不可为空");
                return false;
			}
			
            var balance = parseFloat($("#balance").val());
            var money = parseFloat($("input[name='money']").val());
			Withdraw_PerLimit = parseFloat(Withdraw_PerLimit);
			...
			if(Withdraw_Type == 1){
				if(balance < Withdraw_Limit){
					alert("最低佣金为"+Withdraw_Limit+"元才能提现");
					return false;
				}
			}
			
			if(money < Withdraw_PerLimit){
				$("#withdraw-money").after("<label id=\"withdraw_tip\" class=\"fc_red\">最小为"+Withdraw_PerLimit+"元</label>");
                return false;
			}
            ...
            $.post(url, param, function(data) {
                if (data.status == 1) {
                    global_obj.win_alert(data.msg, function() {
                        ...
                        window.location.href = base_url+'api/'+UsersID+'/distribute/?love';
                    });
                }else{
          	...
          	if(flag){
            	$.post(url, param, function(data) {
                	if(data.status == 1) {
                       ...
                       window.location.href = base_url + 'api/' + UsersID + '/distribute/withdraw/?love';
    ...
    pro_file_init:function(){
	   ...
	   $("#sqsallman").click(function(){
            var url = base_url+'api/'+UsersID+'/distribute/ajax/';           
			var param = $("#apply_manual").serialize();
            $.post(url, param, function(data) {
                    if (data.status == 1) {
                    global_obj.win_alert(data.msg,function() {
                       window.location.href = base_url + 'api/' + UsersID + '/distribute/';
