...
var index_obj={
	index_init:function(){
		...
		var get_biz_list = function(filter, lat, lng){
			$('#loading').show();
					...
					$.each(data.items,function(i,n){
						...
						if(n.meter){
							if(n.meter<1000){
								...
								var met_num = n.meter/1000;
								var meter_num = decimal(met_num,2);
								meter = '约'+meter_num+'km';
							}
						}	
						html+='<div class="items"><a href="'+app_url+'biz/'+n.Biz_ID+'/"><div class="img"><img src="'+n.Biz_Logo+'" /></div><ul><li class="title">'+n.Biz_Name+'&nbsp;';
						if(n.Category_Name != ''){
							html+='<font color="#FF6600;">('+n.Category_Name+')</font>';
						}
						html+='</li><li class="category">'+n.Biz_Address+'</li><li><img src="/static/images/star_'+n['average_ico']+'.png"/>&nbsp;<span>'+n['average_num']+'分</span></li></ul><div class="buyinfo"><span class="price"><i>￥'+n.Biz_MinPrice+'</i>'+n.qiword+'</span><span class="route">'+meter+'</span></div></a></div>';
					});
					$('#bizlist').html(html);
				}else{
		...
		if(ispost==1){
			...
			if(search_params.orderby == 4){
				$.cookie('cookieflag', '', { expires: -1, path: '/' }); // 删除 cookie
			}
			get_biz_list(search_params, '', '');//加载默认地区数据
			//alert($.cookie('cookieflag'));alert(newlat);alert(newlng);
			if(($.cookie('cookieflag') == 'null' || $.cookie('cookieflag') == null) && newlat == '' && newlng == ''){
				var geolocation=new BMap.Geolocation();
				geolocation.getCurrentPosition(function(pos){
					get_biz_list(search_params, pos.point.lat, pos.point.lng);
				});
			}
			$('#list_pages #pre').on('click','span',function(){
				if(search_params.page == 1){
					return false;
				}else{
					...
					get_biz_list(filter, '', '');//加载默认地区数据
					if($.cookie('cookieflag') == 'null' || $.cookie('cookieflag') == null){
						var geolocation=new BMap.Geolocation();
						geolocation.getCurrentPosition(function(pos){
							get_biz_list(filter, pos.point.lat, pos.point.lng);
						});
					}
				}
			});
			
			$('#list_pages #next').on('click','span',function(){
	...
	search_init:function(){
		//遮罩层				
		var get_biz_list = function(lat, lng){
			search_params.lat = lat;
			search_params.lng = lng;
			$.post(search_ajax_url, search_params, function(data) {
				if(data.status == 1){
					...
					$.each(data.items,function(i,n){
						...
						html+='</li><li class="category">'+n.Biz_Address+'</li><li><img src="/static/images/star_'+n['average_ico']+'.png"/>&nbsp;<span>'+n['average_num']+'分</span></li></ul><div class="buyinfo"><span class="price"><i>￥'+n.Biz_MinPrice+'</i>起</span><span class="route">'+meter+'</span></div></a></div>';
					});
					$('#bizlist').html(html);
				}else{
					$('#bizlist').html('<div class="nodata">暂无内容</div>');
		...
		if(ispost==1){
			...
			var left_load = ($(window).width()-$('#loading').width())/2;
			$('#loading').css({"top":top_load,"left":left_load});
			get_biz_list('', '');//加载默认地区数据
			if($.cookie('cookieflag') == 'null' || $.cookie('cookieflag') == null){
				var geolocation=new BMap.Geolocation();
				geolocation.getCurrentPosition(function(pos){
					get_biz_list(pos.point.lat, pos.point.lng);
				});
			}
			$('#list_pages #pre').on('click','span',function(){
				if(search_params.page == 1){
