...
var shop_obj={
	...
	home_init:function(){
		...
		// global_obj.file_upload($('#HomeFileUpload'), $('#home_form input[name=ImgPath]'), $('#home_form .b_images'));
		for(var i=0;i<5;i++){
		...
		$('.item .url_select .btn_select_url').click(function(){
            var module = $(this).attr('module');
                global_obj.create_layer('选择链接', '/member/material/sysurl.php?dialog=1&module=' + module + '&input=shop_home_url_'+$(this).attr("ret"),1000,500);
		});
		...
		$('#shop_skin_index .shop_skin_index_list .mod').click(function(){
			...
			if(shop_skin_data[no]['ContentsType']==1){
				...
				var dataImgPath = shop_skin_data[no]['ImgPath'];
				if(dataImgPath.indexOf('http://')!=-1){
					var s='';
				}else if(dataImgPath.indexOf('/u_file/')!=-1){
					var s=domain.img;
					dataImgPath=dataImgPath.replace('/u_file', '');
				}else if(dataImgPath.indexOf('/api/')!=-1){
					var s=domain.static;
				}else{
					var s='';
				}
				var str = '<a href="'+s+shop_skin_data[no]['ImgPath']+'" target="_blank"><img src="'+s+shop_skin_data[no]['ImgPath']+'" /></a>';
				$('#home_form .b_images').html(str);
				$("#setimages").show();
	...
	products_add_init:function(){
		...
		$("#product_add_form").submit(function(){		
			if(global_obj.check_form($('*[notnull]'))){return false};
			return global_obj.check_form_ele();
		});
...
$('#search_form input[name=AccTime_S], #search_form input[name=AccTime_E]').datetimepicker({
	...
	"p13"  : {
		"packageElement"     : function(packageName){
			$str  = "";
			$str += "<div class='p13 sprite1' packageName='"+packageName+"' p_title='' check_more='' num='' type='' txt='' list_type='' nature=''  order='' category=''>";
			$str += 	"<div class='dragPart'>";
			$str +=         "<div class='w'>";
			$str += 			"<div class='biaot_x'>";
			$str +=		    		"<span class='fenlei_x l'>分类列表</span>";
			$str +=		       	 	"<span class='more r'><a href='#'>更多>></a></span>";
			$str +=		    	"</div>";
			$str +=         	"<div class='chanpint1_x'>";
			$str +=             	"<ul>";
			$str +=             		"<li>";
			$str +=                 		"<a href='#'><span class='cpt_x'><img src='/static/api/shop/skin/"+Skin_ID+"/tp01.jpg'></span></a>";
            $str += 	    		    	"<div class='cpxq1_x'>";
            $str += 				    		"<div class='cpbt_x'><a href='#'>蛋糕蛋糕蛋糕蛋糕</a></div>";
            $str +=       				   		"<span class='l' style='font-size:13px; color:#999; line-height:22px;'>积分：20</span><span class='r' style='font-size:13px; color:#999; line-height:24px;'>已售993</span>";
            $str += 				        	"<div class='clear'></div>";
            $str +=								"<span class='jiaget_x l'>￥36.5<a style='font-size:10px; color:#fff; margin-left:5px;  background:#333; padding:0 3px;'>9.5折</a></span>";
            $str +=       						"<span class='r' style='margin-top:3px;'><img src='/static/api/shop/skin/"+Skin_ID+"/gwct_x.png' width='20' height='20'></span>";                    
            $str +=    						"</div>";
			$str +=             		"</li>";
			$str +=             		"<li>";
			$str +=                 		"<a href='#'><span class='cpt_x'><img src='/static/api/shop/skin/"+Skin_ID+"/tp02.jpg'></span></a>";
            $str += 	    		    	"<div class='cpxq1_x'>";
            $str += 				    		"<div class='cpbt_x'><a href='#'>蛋糕蛋糕蛋糕蛋糕</a></div>";
            $str +=       				   		"<span class='l' style='font-size:13px; color:#999; line-height:22px;'>积分：20</span><span class='r' style='font-size:13px; color:#999; line-height:24px;'>已售993</span>";
            $str += 				        	"<div class='clear'></div>";
			...
			$str +=     	"</div>";
			$str += 	"</div>";
			$str +=		"<div class='delObj hand' onclick='shop_obj.delObjEvt(this,\"p13\");'><img "+shop_obj.Evt1+" src='"+shop_obj.cloIco+"' /></div>";
			$str +=		"<div class='clean'></div>";
			$str +=	"</div>";
			return $str;
		},
		"insertWord"     : function(packageName){
			var $num = $(".ps2_frmae_p13 .num input").val();
			var $type = $("#type_p13").val();
			var $txt = $(".ps2_frmae_p13 .more input").val();
			var $p_title = $(".ps2_frmae_p13 .p_title input").val();
			var $check_more = $("input[name='check_more']:checked").val();
			var $list_type = $("#list_type_p13").val();
			var $nature = '';
			$("input[name='nature_p13']:checked").each(function(){
				$nature += $(this).val()+",";
			});
			var $order = $("#order_p13").val();
			var $category = $("#category_p13").val();
			var re = /^(\+|-)?\d+$/;
			if(!re.test($num)){
				alert('推荐列表条数为数字!');
				return false;
			}
			$(".selectObj").attr("p_title",$p_title);
			$(".selectObj").attr("check_more",$check_more);
			$(".selectObj").attr("num",$num);
			$(".selectObj").attr("type",$type);
			$(".selectObj").attr("txt",$txt);
			$(".selectObj").attr("list_type",$list_type);
			$(".selectObj").attr("nature",$nature);
			$(".selectObj").attr("order",$order);
			$(".selectObj").attr("category",$category);
			shop_obj.btnCtrl();
		}
	},
	"p14"  : {
		"packageElement"     : function(packageName){
			$str  = "";
			$str += "<div class='p14 sprite1' packageName='"+packageName+"' p_title='' check_more='' num='' type='' txt='' list_type='' nature=''  order='' def='' >";
			$str += 	"<div class='dragPart'>";
			$str +=         "<div class='w'>";
			$str += 			"<div class='biaot_x'>";
			$str +=		    		"<span class='fenlei_x l'>属性列表</span>";
			$str +=		       	 	"<span class='more r'><a href='#'>更多>></a></span>";
			$str +=		    	"</div>";
			$str +=         	"<div class='chanpint1_x'>";
			$str +=             	"<ul>";
			...
			$str +=     	"</div>";
			$str += 	"</div>";
			$str +=		"<div class='delObj hand' onclick='shop_obj.delObjEvt(this,\"p14\");'><img "+shop_obj.Evt1+" src='"+shop_obj.cloIco+"' /></div>";
			$str +=		"<div class='clean'></div>";
			$str +=	"</div>";
			return $str;
		},
		"insertWord"     : function(packageName){
			...
			var $num = $(".ps2_frmae_p14 .num input").val();
			var $type = $("#type_p14").val();
			var $txt = $(".ps2_frmae_p14 .more input").val();
			var $p_title = $(".ps2_frmae_p14 .p_title input").val();
			var $list_type = $("#list_type_p14").val();
			var $check_more = $("input[name='check_more_p14']:checked").val();
			var $nature = '';
			$("input[name='nature_p14']:checked").each(function(){
				$nature += $(this).val()+",";
			});
			var $order = $("#order_p14").val();
			var $def = $("#def_p14").val();
			var re = /^(\+|-)?\d+$/;
			if(!re.test($num)){
				alert('推荐列表条数为数字!');
	...
	"showproElement": function(packageName){ //显示属性编辑面板
		switch(packageName)
		{
			...
			break;
			case "p13"://分类
				var $num         = $(".ipad .selectObj").attr("num");
				var $type        = $(".ipad .selectObj").attr('type');
				var $txt         = $(".ipad .selectObj").attr('txt');
				var $list_type   = $(".ipad .selectObj").attr('list_type');
				var $nature      = $(".ipad .selectObj").attr('nature');
				var $order       = $(".ipad .selectObj").attr('order');
				var $category    = $(".ipad .selectObj").attr('category'); 
				var $p_title     = $(".ipad .selectObj").attr("p_title");
				var $check_more  = $(".ipad .selectObj").attr("check_more");
				$(".ps2_frmae_p13 .num input").val($num);
				$("#type_p13").val($type);
				$(".ps2_frmae_p13 .more input").val($txt);
				$(".ps2_frmae_p13 .p_title input").val($p_title);
				$("#list_type_p13").val($list_type);
				$nature = $nature?$nature:'';
				//复选框
				$nature = $nature.replace(/,$/g,"");
				var natureArr = new Array();
				natureArr = $nature.split(',');
				$.each(natureArr,function(i,item){
					$("input[name='nature_p13']:checkbox[value="+item+"]").attr('checked',true);
				});
				//单选框
				$("input[name='check_more'][value="+$check_more+"]").attr('checked',true);
				$("#order_p13").val($order);
				$("#category_p13").val($category);
			break;
			case "p14"://属性
				var $num         = $(".ipad .selectObj").attr("num");
				var $type        = $(".ipad .selectObj").attr('type');
				...
				var $nature      = $(".ipad .selectObj").attr('nature');
				var $order       = $(".ipad .selectObj").attr('order');
				var $def_p14     = $(".ipad .selectObj").attr('def'); 
				var $p_title     = $(".ipad .selectObj").attr('p_title');
				var $check_more  = $(".ipad .selectObj").attr('check_more');
				$(".ps2_frmae_p14 .num input").val($num);
				$("#type_p14").val($type);
				$(".ps2_frmae_p14 .more input").val($txt);
				$(".ps2_frmae_p14 .p_title input").val($p_title);
				$("#list_type_p14").val($list_type);
				$nature = $nature?$nature:'';
				//复选框
				$nature = $nature.replace(/,$/g,"");
				var natureArr = new Array();
				natureArr = $nature.split(',');
				$.each(natureArr,function(i,item){
					$("input[name='nature_p14']:checkbox[value="+item+"]").attr('checked',true);
				});
				//单选框
				$("input[name='check_more_p14'][value="+$check_more+"]").attr('checked',true);
				$("#order_p14").val($order);
				$("#def_p14").val($def_p14);
			break;
		...
		$(".ipad .sprite1").each(function(index, element) {
			...
			switch($packageName){
				...
				case "p13": //分类
					...
					var $check_more  = $(this).attr('check_more');
					...
					var $category    = $(this).attr('category'); 
					    ...
					    str +=     '"top_type"   : "'+$type+'",';
					    str +=     '"txt"        : "'+$txt+'",';
					    str +=     '"list_type"  : "'+$list_type+'",';
					    str +=     '"nature"     : "'+$nature+'",';
					    str +=     '"order"      : "'+$order+'",';
					    str +=     '"p_title"    : "'+$p_title+'",';
					    str +=     '"check_more" : "'+$check_more+'",';
