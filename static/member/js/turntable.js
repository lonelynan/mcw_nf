...
var turntable_obj={
	...
	wheel_mod:function(){
		...
		if(status==0){
			...
			$('#wheel_form input[name=FirstPrize], #wheel_form input[name=SecondPrize], #wheel_form input[name=ThirdPrize], #wheel_form input[name=UsedIntegral],#wheel_form input[name=If_Share]').attr('disabled', '').css({'background': '#F5F5F5'});
		}else{
			
		}
		var date_str=new Date();
		$('#wheel_form input[name=Time]').attr('disabled', false);
